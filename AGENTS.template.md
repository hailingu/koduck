...
- The coordination board is the mutable demand source for an ADD, not approval
  ...
  deliberately.
- Reading, triaging, and routing a coordination requirement are read-only and
...
- When Web or native UI is in scope, the ADD MUST cite the specific accessible
  ...
  actors, states, transitions, and system feedback in the required Mermaid
  diagram, but MUST NOT override {{DESIGN_TOOL_NAME}}'s visual or interaction
  decisions. Missing or ambiguous {{DESIGN_TOOL_NAME}} context blocks the ADD
...
its diff proves it has no semantic change. Test source, snapshots, generated
...
| `.github/workflows/koduck-ai.yml` | `docs/README.md`, `docs/development/software-engineering-standard.md`, `docs/adr/ADR-0002-required-ai-ci-postgres-verification.md`, and this guide's Work Coordination and Decision Records sections | repository root | `npm test --prefix tools/governance-validator`; `npm run validate --prefix tools/governance-validator`; `cargo test -p koduck-ai --test architecture required_ci_maps_every_routed_command_and_postgres_boundary -- --exact` | Keep every routed governance command inside an existing required `dev` check and preserve the exact three required check contexts. Configuration changes use the deterministic architecture contract test plus governance validation. |
<!-- OPTIONAL MODULE FRAGMENT: DELIVERY_OPERATIONS -->
...
- **New ADD branches**: A task that creates a new ADD from identified
  coordination requirements MUST use a newly created task branch dedicated to
...
- **Task pull requests**: target `dev`; include the verification commands run and
  ...
  automatic-review rule and the original-thread reply rule in Work
  Coordination; passing review does not replace record acceptance or
  operational verification. Only a repository-integration or release pull
  request from `dev` may target `main`; ordinary task branches MUST NOT target
  `main`.
