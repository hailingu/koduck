...
- Decomposition review at `a7258bc`: `sqlx_executor.rs` is 742 physical lines
  and `postgres.rs` is 409 physical lines, both above the 400-line review
  threshold and below the 800-line exception limit. The executor remains
  ...
  pass-through split. Recovery-thread ownership remains extracted to
