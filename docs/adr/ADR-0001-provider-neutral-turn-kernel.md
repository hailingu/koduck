...
| --- | --- | --- | --- | --- |
| T-1 | Create the repository routing, owned domain lifecycle, application turn runner, consumer-owned ports, and one OpenAI-compatible provider adapter. | Root `AGENTS.md` Scope Routing row for `koduck-ai/**`; root Cargo workspace; `koduck-ai` domain, application, provider adapter, typed errors, unit tests, and dependency-direction test. | In Progress | Reapproval allows the existing Root Scope Routing, Cargo manifests, and first domain-lifecycle test to count toward T-1. Production source remains absent until the governed red-test build is observed. |
