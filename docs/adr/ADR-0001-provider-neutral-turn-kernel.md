...
- **Approval Time [Conditionally Required — Decision Status is or has been `Accepted`]**: 2026-08-12T15:55:49+08:00
...
- Initial acceptance and append preallocate a stable Item identity. If the
  ...
  identity; the adapter reports the durable result when present and reports
  unavailable only after absence is determined.
...
| CT-16 | A timed-out or failed initial-accept or append commit acknowledgement is reconciled by stable Item identity before the adapter reports success or unavailability. | AC-8, AC-9; `postgres::tests::timed_out_commit_returns_the_reconciled_durable_outcome`; `postgres::tests::timed_out_commit_reports_unavailable_only_after_absence_is_reconciled`; `postgres::tests::failed_commit_acknowledgement_returns_the_reconciled_durable_outcome`; `commit_reconciliation::tests::reconciliation_waits_for_the_matching_writer_identity` |
...
| Timeout and deadline | Applicable — database startup/query/commit acknowledgement or provider establishment/streaming stalls. | Runtime assembly, SQLx, and OpenAI-compatible adapters | AC-9, AC-16, and AC-17 commands plus commit-reconciliation unit tests | Database startup/query attempts stop at 2 seconds; timed-out commit acknowledgement is reconciled by stable operation identity before a result is reported; provider connect/header/idle/total deadlines are 5/30/30/120 seconds and produce typed terminal failure. | AC-9, AC-16, AC-17 | Pass — startup/query deadline behavior, commit-reconciliation tests, and `architecture::production_io_and_background_work_are_bounded`. |
...
| --- | --- | --- | --- | --- | --- |
| A-1 | ADR approved | An eligible non-author approver, approval time, and exact `Approval Evidence: Approve` are recorded; any optional Approval Context Revision is informational, non-binding, and exactly represents the approved document | ADR metadata | Complete | `@linhai` identified ADR-0001 and supplied exact `Approve` for Review `4913815805`; metadata records `2026-08-12T15:55:49+08:00`. Implementation commit `1099bfe` is stable delivery evidence; no Approval Context Revision is recorded because approval was intentionally task-context-bound rather than revision-bound. |
