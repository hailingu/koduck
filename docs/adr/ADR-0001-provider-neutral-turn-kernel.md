...
| CT-7 | Authenticated interruption wins terminal arbitration only while a `started` Turn retains an unfenced, unexpired live owner; detached `recovery-pending` and expired Turns reject interruption because no stream remains to deliver `turn.interrupted`. Unknown/non-owned Turns remain indistinguishable and cancellation remains distinct. | AC-7; `postgres_subject_ownership::production_postgres_contract` |
...
| Cancellation and interruption | Applicable — interrupt races with provider completion/failure, failed-append recovery, lease expiry, or downstream disconnect. | Application runner, PostgreSQL history, and HTTP/SSE adapter | AC-7 and AC-15 commands | A live-owner interrupt is the sole `interrupted` terminal; recovery-pending or expired owners reject interruption rather than acknowledge an SSE event they cannot deliver; dependency/disconnect cancellation remains `cancelled`; synchronous mappings are distinct 409 codes. | AC-7, AC-15 | Pass — `interrupt_and_cancel_are_distinct`, terminal-arbitration tests, production PostgreSQL recovery-pending/expired-owner rejection, and synchronous mapping regression. |
...
| AC-7 | T-2 | The interrupt contract is exact and an authenticated client stop remains distinct from platform/dependency cancellation. | One owned live SSE Turn, one recovery-pending Turn, one expired started Turn, one unknown UUID, one non-owned Turn, one already-terminal Turn, and one second Turn with an injected dependency stop. | Run `cargo test -p koduck-ai --test cand_1_contract interrupt_and_cancel_are_distinct -- --exact` and the production PostgreSQL contract test. | Exit code 0; only the unfenced/unexpired `started` owner accepts interruption and reaches exactly one durable `interrupted`; recovery-pending and expired started Turns reject interruption; unknown/non-owned cases return indistinguishable `404`; the terminal case returns `409 turn-already-terminal`; dependency cancellation remains `cancelled`; no tested Turn has another terminal. | Command output, normalized response hashes, PostgreSQL replay, and replay fixtures. | Pass | Local contract and terminal-arbitration tests pass; the production PostgreSQL test verifies live-owner interruption and deterministically rejects both `recovery-pending` and expired-started fixtures. Commits `fe3beb9` and `a7258bc` prove an accepted live interrupt wins over simultaneous provider completion or failure; commit `31ef43f` proves it remains observable during provider request establishment. |
...
- Pre-commit decomposition review of the current worktree: maintained
  ...
  (721), `provider/mod.rs` (582), `runner.rs` (591), `postgres.rs` (509), and
