...
| --- | --- | --- | --- | --- |
| T-1 | Create the repository routing, owned domain lifecycle, application turn runner, consumer-owned ports, and one OpenAI-compatible provider adapter. | Root `AGENTS.md` Scope Routing row for `koduck-ai/**`; root Cargo workspace; `koduck-ai` domain, application, provider adapter, typed errors, unit tests, and dependency-direction test. | Complete | Commit `af10ac9` adds the owned domain/application boundary, runner, provider port and deterministic OpenAI-compatible protocol adapter; review-correction commit `56073a0` makes post-accept provider setup/protocol failures durable failed terminals and consumes provider frames lazily. AC-1 through AC-3 and AC-14 pass. |
| T-2 | Implement the owned authenticated REST/SSE v1 contract and freeze its golden fixtures. | Plain-text no-tool `POST /api/v1/ai/chat`, `POST /api/v1/ai/chat/stream`, and interrupt route; trust-context handoff; request/response/header/status/SSE fixture hashes; contract tests. | Complete | Commit `4a7bf5d` adds the framework-neutral REST/SSE/interrupt adapter, resume/interruption behavior, contract copy, and three hashed fixtures; review-correction commit `56073a0` removes the turn-wide runtime mutex and streams durable SSE events incrementally while authenticated interrupts remain concurrent. AC-4 through AC-7 and AC-13 pass. |
| T-3 | Implement the AI-owned PostgreSQL history and fenced-liveness adapter and prove failure/recovery/no-fallback behavior. | Initial durable acceptance, append/replay, migrations, deadlines and buffer caps, lease acquire/renew/fence, orphan reconciliation, crash/fault tests, and proof of no legacy runtime dependency. | Complete | Commits `46f2a39` and `80fc2ff` add fail-closed policy, schema/adapter boundaries, exact lease timing, and crash/race evidence. Commit `08cc1b3` adds the concrete tenant-keyed SQLx executor, idempotent PostgreSQL migration, Reqwest provider transport, Axum routes, validated runtime configuration schema, and executable entry point. Review-correction commit `56073a0` enforces the production two-second append deadline and starts per-turn renewal plus runtime orphan reconciliation workers. AC-8 through AC-12 pass. |
