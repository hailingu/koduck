...
- **Approval Time [Conditionally Required — Decision Status is or has been `Accepted`]**: 2026-08-17T08:57:39Z
...
- Initial acceptance and append preallocate a stable Item identity. If the
  2-second attempt deadline expires during commit acknowledgement, PostgreSQL
  transaction advisory locking serializes result reconciliation on that
  identity. Reconciliation is a separate PostgreSQL attempt capped at 2
  seconds: the adapter reports the durable result when found, reports absence
  when determined, and otherwise returns `Unavailable` while retaining the
  stable identity in any durable write. The two sequential attempts therefore
  wait at most 4 seconds in total.
...
- Resume provider context is independently capped at 4096 prior Items and 1 MiB
  of canonical serialized Item payload. The PostgreSQL query reads no more
  than 4097 ordered rows, rejects an over-budget resume as owned `400
  invalid-request`, and never silently truncates durable history.
- Every production PostgreSQL operation has the same 2-second attempt deadline.
  ...
  background workers per production history instance. Connection and migration
  startup attempts use the same deadline. An append outage stops its renewal
  worker and atomically moves that worker's permit into recovery without
  decrementing admission between owners, preserving reserved recovery capacity
  even at full renewal saturation. That handoff wait is bounded by the 2-second
  renewal-attempt deadline; other saturation fails closed as
  `durability-unavailable`.
- Failed-append recovery has one 22-second total window. Before every attempt,
  the adapter computes the remaining window and caps that database attempt to
  the lesser of 2 seconds or the remaining duration; it starts no attempt once
  the window is exhausted and then defers to fenced reconciliation.
...
transport diagnostic `durability-unavailable` — the in-band SSE `error` event
or the synchronous `503` problem defined by the wire contract below. When
history returns, one conditional terminal append closes it as `failed`, except
that an already expired and fenced owner is closed as `cancelled` by
reconciliation.
...
- `POST /api/v1/ai/chat/stream` accepts the same JSON and returns `200` with
  ...
  `item.created`, exactly one of `turn.completed`, `turn.failed`,
  `turn.interrupted`, or `turn.cancelled`, and the transport-diagnostic
  `error`. Each `turn.*` or `item.created` event's JSON includes matching
  ...
  the exact `usage` object above only for `turn.completed`. A failure after
  `turn.started` that prevents a durable terminal append instead emits at most
  one `error` event whose data is the exact problem body defined below — a
  mid-turn durability outage carries code `durability-unavailable` — and then
  closes the stream without a terminal event; the Turn is closed later as
  `failed` or `cancelled` by bounded recovery or fenced reconciliation. The
  `error` event carries no `thread_id`, `turn_id`, or `sequence`, and no
  `error` event is emitted once a terminal event has been published.
...
- A resume whose ordered provider context exceeds 4096 Items or 1 MiB of
  canonical serialized Item payload returns owned `400 invalid-request`; it
  creates no new Turn or provider request and never truncates or mutates the
  prior durable history.
...
| CT-7 | Authenticated interruption wins terminal arbitration only while a `started` Turn retains an unfenced, unexpired live owner; the interrupt transaction locks ownership, appends the unique durable `interrupted` Item, and commits the terminal status before returning acceptance. Detached `recovery-pending` and expired Turns reject interruption. Unknown/non-owned Turns remain indistinguishable and cancellation remains distinct. | AC-7; `postgres_subject_ownership::production_postgres_contract` |
| CT-8 | Initial or mid-Turn durability outage, including an accepted control-state read outage, publishes no uncommitted state and returns `durability-unavailable`; after SSE stream start the diagnostic is delivered as at most one in-band `error` event carrying the exact problem body and the stream closes without a terminal event, and no `error` event is emitted once a terminal event has been published. | AC-8; `control_read_outage_enters_failed_recovery_handoff`; `runtime_wiring::mid_turn_failure_is_reported_inside_an_started_sse_stream`; `sse_terminal_consistency::replay_failure_after_sse_terminal_does_not_emit_error_event` |
| CT-9 | Serialized unpublished data is capped at 64 Items and 1 MiB and every PostgreSQL attempt, including startup connection and migration, is capped at 2 seconds. | AC-9, AC-16 |
| CT-10 | Lease renewal, expiry, skew, fencing, and reconciliation produce at most one durable orphan terminal. | AC-10, AC-11 |
| CT-11 | Lease-renewal and failed-append recovery share a 256-worker admission bound; a failed Turn moves its renewal permit directly into recovery without releasing the reservation, so the slot transfers atomically at saturation. | AC-16; `append_outage_confirms_admission_handoff_before_recovery`; `postgres::tests::renewal_recovery_handoff_retains_permit_reservation` |
...
| CT-16 | A timed-out or failed initial-accept or append commit acknowledgement starts one separate stable-Item-identity reconciliation attempt capped at 2 seconds; a found durable result is returned, confirmed absence or an indeterminate reconciliation returns `Unavailable`, and operation plus reconciliation waits at most 4 seconds. | AC-8, AC-9; `postgres::tests::timed_out_commit_returns_the_reconciled_durable_outcome`; `postgres::tests::timed_out_commit_reports_unavailable_only_after_absence_is_reconciled`; `postgres::tests::failed_commit_acknowledgement_returns_the_reconciled_durable_outcome`; `postgres::tests::commit_reconciliation_attempt_is_bounded_by_the_database_deadline`; `commit_reconciliation::tests::reconciliation_waits_for_the_matching_writer_identity` |
| CT-17 | Resume provider context is bounded to 4096 prior Items and 1 MiB of canonical serialized payload; over-budget history returns owned `400 invalid-request` without truncating or mutating history. | AC-6, AC-15; `commit_reconciliation::tests::aggregate_history_over_one_mib_is_rejected_before_provider_construction`; `commit_reconciliation::tests::aggregate_history_over_four_thousand_ninety_six_items_is_rejected`; `http::tests::context_limit_maps_to_the_owned_invalid_request_service_error`; `cand_1_contract::oversized_resume_context_uses_the_owned_invalid_request_problem` |
...
| Timeout and deadline | Applicable — database startup/query/commit acknowledgement/reconciliation or provider establishment/streaming stalls. | Runtime assembly, SQLx, and OpenAI-compatible adapters | AC-9, AC-16, and AC-17 commands plus commit-reconciliation unit tests | Each database attempt stops at 2 seconds; a failed/timed-out commit acknowledgement receives one separate stable-identity reconciliation attempt capped at 2 seconds, so both sequential attempts wait at most 4 seconds and an indeterminate result returns `Unavailable`; provider connect/header/idle/total deadlines are 5/30/30/120 seconds and produce typed terminal failure. | AC-9, AC-16, AC-17 | Pass — startup/query deadline behavior, bounded commit-reconciliation tests, and `architecture::production_io_and_background_work_are_bounded`. |
| Cancellation and interruption | Applicable — interrupt races with provider completion/failure, failed-append recovery, lease expiry, or downstream disconnect. | Application runner, PostgreSQL history, and HTTP/SSE adapter | AC-7 and AC-15 commands | Returning 202 means the live-owner interrupt is already the sole durable `interrupted` terminal; provider/recovery competitors replay it. Recovery-pending or expired owners reject interruption; dependency/disconnect cancellation remains `cancelled`; synchronous mappings are distinct 409 codes. | AC-7, AC-15 | Pass — `interrupt_and_cancel_are_distinct`, transactional terminal-arbitration tests, production PostgreSQL recovery-pending/expired-owner rejection, and synchronous mapping regression. |
| Resource bounds and backpressure | Applicable — provider floods frames, resumed history grows without bound, or active Turns exhaust background capacity. | Provider, application durability policy, and PostgreSQL history | AC-6, AC-9, AC-15, and AC-16 commands plus aggregate-history regression | Item/payload caps fail closed; resume reads at most 4097 rows and rejects context above 4096 Items or 1 MiB without truncation; provider channel stays bounded; worker 257 is rejected; append outage directly transfers its reserved permit from renewal into recovery without exceeding 256 or exposing an acquisition window. | AC-6, AC-9, AC-15, AC-16 | Pass — durability/context-cap tests, bounded-query inspection, atomic reservation-transfer unit test, and recovery-handoff regression. |
...
| AC-6 | T-2 | Resume creates a new Turn on the same Thread without mutating or silently truncating prior terminal history. | One completed Turn exists with its immutable replay hash; over-budget fixtures contain 4097 prior Items or more than 1 MiB of canonical serialized payload. | Run `cargo test -p koduck-ai --test cand_1_contract` and PostgreSQL history unit tests. | Exit code 0; an in-budget resume uses a new Turn ID on the same Thread and supplies ordered durable history exactly once; over-budget history creates no new Turn/provider call and returns owned `400 invalid-request`; prior replay is unchanged. | Command output, before/after replay hashes, bounded-query inspection, and response assertion. | Pass | Local contract and PostgreSQL history regressions pass; production query streams at most 4097 ordered rows, rejects item 4097 or serialized byte 1,048,577, and never constructs an over-budget provider request. |
| AC-7 | T-2 | The interrupt contract is exact and an authenticated client stop remains distinct from platform/dependency cancellation. | One owned live SSE Turn, one recovery-pending Turn, one expired started Turn, one unknown UUID, one non-owned Turn, one already-terminal Turn, and two provider-terminal competitors. | Run the owned contract and production PostgreSQL contract tests. | Exit code 0; 202 is returned only after the interrupt transaction has appended exactly one durable `interrupted` terminal for an unfenced/unexpired `started` owner; both provider-terminal competitors receive `AlreadyTerminal`; recovery-pending and expired started Turns reject interruption; unknown/non-owned cases remain indistinguishable; dependency cancellation remains distinct. | Command output, normalized response hashes, PostgreSQL replay, and race results. | Pass | Local contract/arbitration tests pass; production PostgreSQL proves the interrupt terminal is durable before concurrent completion/failure both lose, and rejects recovery-pending plus expired-started fixtures. Provider request establishment remains interruptible. |
| AC-8 | T-3 | Initial history failure exposes no accepted Turn, while a later durability outage exposes only the durable prefix plus `durability-unavailable` and retains failed-recovery ownership. | Fault adapter fails initial acceptance in case A, the next append after one durable delta in case B, and an accepted control-state read in case C. | Run `cargo test -p koduck-ai --test cand_1_durability`. | Exit code 0; case A has zero accepted Turn records and zero provider calls; cases B and C publish no uncommitted payload, stop provider consumption, emit `durability-unavailable` (on the SSE route, exactly one in-band `error` event carrying that problem body and no terminal event), enter recovery-pending, and schedule failed recovery through the liveness handoff. | Command output, adapter trace, and replay fixtures. | Pass | Local durability regressions prove initial zero-side-effect failure, durable-prefix-only append failure, and accepted control-read failure all fail closed; both accepted outages retain the liveness recovery reservation. |
| AC-9 | T-3 | Append, commit-reconciliation, and unpublished-buffer limits are exact and fail closed. | Virtual clock; cases for a 2.001-second append, stalled stable-identity reconciliation, 65 Items, and payload size 1,048,577 bytes. | Run `cargo test -p koduck-ai --test cand_1_durability append_deadline_and_buffer_caps -- --exact` and `cargo test -p koduck-ai adapters::history::postgres::tests`. | Exit code 0; each database attempt stops at 2 seconds; operation plus reconciliation waits at most 4 seconds; an indeterminate reconciliation returns `Unavailable`; each buffer-limit case stops provider consumption, publishes zero over-limit Items, emits `durability-unavailable`, and retains the pre-case durable prefix. | Command output and per-case trace. | Pass | Local regressions prove the operation and its separate reconciliation are each bounded by the database deadline, including a stalled reconciliation; existing 2.001-second, item 65, and byte 1,048,577 cases stop consumption, map to `durability-unavailable`, and publish no over-limit Item. Commit `a7b6faa` proves JSON escaping and payload-object overhead are included in the serialized 1-MiB boundary. |
...
| AC-11 | T-3 | Concurrent reconcilers and delayed store recovery cannot duplicate or overwrite the orphan terminal. | 4 reconcilers race on one expired Thread/Turn/generation while the store is unavailable and then recovers. | Run `cargo test -p koduck-ai --test cand_1_liveness concurrent_reconcilers_are_idempotent -- --exact`. | Exit code 0; after recovery exactly one conditional write succeeds, durable history has exactly one `cancelled` terminal, 3 reconcilers receive `ALREADY_TERMINAL` or `FENCED`, and a late `completed` append is rejected. | Command output, race summary, and replay hash. | Pass | Exit 0 at `80fc2ff`; 32 unavailable attempts fail closed, recovery yields 1 cancellation and 31 terminal/fenced results, and late completion is rejected. |
...
| AC-15 | T-2 | HTTP media-type, synchronous terminal, and context-limit mappings are exact. | Valid JSON with `Application/JSON; charset=utf-8`; completed, interrupted, cancelled, failed, and over-budget resume results. | Run `cargo test -p koduck-ai --test cand_1_contract`. | Exit 0; both chat routes accept the parameterized JSON media type; completed returns 200, interrupted/cancelled return their distinct 409 problem codes, failed returns `503 provider-unavailable`, and over-budget resume returns `400 invalid-request`. | Command output and response assertions. | Pass | Local review-correction test run passes all ten contract tests, including the owned context-limit problem mapping. |
| AC-16 | T-3 | Database calls and background liveness/recovery work are bounded and fail closed. | Slow query/startup futures, a background admission limit of one, append outage while liveness owns capacity, and recovery-thread creation failure. | Run PostgreSQL adapter/recovery tests, the startup-timeout test, `append_outage_confirms_admission_handoff_before_recovery`, and the production bound architecture test. | Exit 0; slow database calls return typed unavailable/timeout errors; worker 257 is rejected; ordinary guard drop is non-blocking; the same reserved permit moves from renewal into recovery without an acquisition window; the original observer remains attached until bounded recovery finishes or defers; a recovery-thread spawn failure executes recovery synchronously while retaining the permit. | Command output and deterministic source inspection. | Pass | Local deadline, shared-admission, atomic transfer, terminal replay, and injected spawn-failure regressions pass; production renewal/recovery remain capped at 256. |
...
| --- | --- | --- | --- | --- | --- |
| A-1 | ADR approved | An eligible non-author approver, approval time, and exact `Approval Evidence: Approve` are recorded; any optional Approval Context Revision is informational, non-binding, and exactly represents the approved document | ADR metadata | Complete | `@linhai` identified ADR-0001 and PR #2 Review thread `3764738232`, then supplied exact `Approve`; metadata records `2026-08-12T16:25:02+08:00`. Implementation commit `8de786d` is stable delivery evidence; no Approval Context Revision is recorded because approval was intentionally task-context-bound rather than revision-bound. |
...
| A-7 | Contract and baseline risks covered | Every normative contract clause maps to an explicit acceptance check or deterministic test, and every required Risk Coverage Matrix row reaches Pass before review-ready or completion | Contract-To-Check Traceability, Risk Coverage Matrix, acceptance checks | Complete | Contract-To-Check Traceability maps every clause to a check; the Risk Coverage Matrix covers all five dimensions with Pass and stable evidence. |
...
In task `01a08909-759c-78b3-948a-367161280447`, the repository owner explicitly
requested reducing the 32-way test races to four and directly updating the
affected documents. This instruction authorizes this verification-only
amendment without another decision record or approval ritual. AC-11 now uses
four reconcilers: all four report unavailability before recovery; after
recovery exactly one cancels and three observe an already-terminal or fenced
outcome. Production behavior is unchanged. Earlier approval metadata, statuses,
and dated test results describe the original implementation; they do not claim
that the original approval or 32-way evidence verifies this amendment. Current
verification evidence is recorded with the four-way amendment in
`koduck-ai/docs/adr/ADR-0004-authenticated-correction-admission.md`.
...
- Pre-commit decomposition review of the current worktree: maintained
  production files above the 400-line review threshold are `sqlx_executor.rs`
  (739), `provider/mod.rs` (582), `runner.rs` (591), `postgres.rs` (509), and
  `runtime/mod.rs` (423); maintained test files above the 600-line threshold
  are `turn_terminal_arbitration.rs` (870), `runtime_wiring.rs` (762), and
  `cand_1_durability.rs` (679). All remain below their 800/1,200-line exception
  limits. The production files retain one boundary each: transactional
  PostgreSQL state, OpenAI-compatible streaming protocol, Turn orchestration,
  PostgreSQL liveness/history adaptation, and runtime assembly. The test files
  retain one contract family each. Executable units above 60 lines are the
  Turn execution coordinator, the four transactional acceptance/append/
  recovery/reconciliation methods, and two end-to-end arbitration scenarios;
  each keeps one ordering-sensitive operation or fixture in view, and none
  exceeds the 120-line exception limit. Extracting smaller pass-through units
  would split transaction, lifecycle, or scenario invariants without reducing
  coupling. Cyclomatic complexity is `N/A — no configured complexity tool`;
  source inspection found no exception-limit nesting. No engineering exception
...
| 2026-08-12 | Follow-up correction for Review `4912786010`: an append outage releases that Turn's renewal permit before recovery scheduling so capacity transfers even when all 256 renewal slots are occupied; PostgreSQL connection and migration startup each use the 2-second database deadline. Added deterministic handoff and startup-timeout regressions; accepted scope and statuses remain unchanged. | @codex |
| 2026-08-12 | Follow-up correction for Review `4912891981`: replaced asynchronous renewal-stop signaling with an explicit recovery handoff that joins the bounded renewal worker and confirms its permit is released before scheduling recovery. Ordinary request-shutdown Drop remains non-blocking. Added regressions for both confirmed permit availability and non-blocking ordinary Drop; accepted scope and statuses remain unchanged. | @codex |
| 2026-08-12 | Follow-up correction for Review `4912970984`: removed the remaining release/reacquire window by moving the renewal worker's existing permit through a single-owner channel directly into recovery. Admission remains reserved throughout the transfer, so a concurrent Turn cannot steal the slot at saturation. Added an atomic-reservation regression and retained the ordinary non-blocking Drop regression; accepted scope and statuses remain unchanged. | @codex |
| 2026-08-12 | Pre-commit whole-PR review corrected three adjacent lifecycle gaps: an accepted control-state read outage now enters failed recovery, `recovery-pending` accepts authenticated interruption, and an `AlreadyTerminal` append race replays the durable winner like fencing. It also aligned Cargo package metadata with the repository MIT license, replaced implementation-text assertions with behavior tests, and refreshed decomposition evidence; accepted scope and statuses remain unchanged. | @codex |
| 2026-08-12 | Review `4913319728` corrected the recovery/stream boundary: only unfenced, unexpired `started` owners may accept interruption; detached recovery-pending and expired owners reject it. Atomic liveness handoff now completes bounded recovery before releasing the original observer and replays any durable terminal, while recovery-worker creation failure retains the permit and executes a bounded synchronous fallback. Added production PostgreSQL rejection and injected spawn-failure regressions; accepted scope and statuses remain unchanged. | @codex |
| 2026-08-12 | Review `4913411000` removed the last near-expiry acceptance race by making the interrupt endpoint itself the terminal transaction: it locks the owned live Turn/Lease, inserts the unique `interrupted` Item, advances status/sequence, and commits before returning 202. Concurrent provider/recovery writers therefore replay an already-durable winner even if expiry follows immediately. Updated the production PostgreSQL race regression; accepted scope and statuses remain unchanged. | @codex |
| 2026-08-12 | Approval-invalidating Review `4913815805` correction at `2026-08-12T15:55:49+08:00` added a public over-budget-resume `400 invalid-request` outcome and changed commit/recovery behavior. Preserved prior approval: Approver `@linhai`, Approval Time `2026-08-11T11:14:45+08:00`, Approval Evidence `Approve`, no Approval Context Revision. Reset Decision Status to `Proposed` and Implementation Status to `Not Started` pending reapproval. | @codex |
| 2026-08-12 | Reapproved the three Review `4913815805` corrections after repository owner `@linhai` identified ADR-0001 and supplied exact `Approve`; recorded Approval Time `2026-08-12T15:55:49+08:00`, returned Decision Status to `Accepted`, and restored Implementation Status to `Complete` after the revised checks passed. The approved correction reconciles timed-out commit acknowledgement by stable Item identity, caps each recovery attempt to the remaining 22-second window, and bounds resume context to 4096 Items/1 MiB without truncation. No Approval Context Revision is recorded because no immutable revision yet represents the approved correction. | @linhai |
| 2026-08-12 | Recorded implementation commit `1099bfe` for Review `4913815805`. Routed `cargo fmt --all --check`, strict all-target/all-feature Clippy, and the complete all-target/all-feature suite pass: 101 tests, including PostgreSQL writer/reconciliation lock serialization, failed/timed-out commit acknowledgement reconciliation, exact remaining-window recovery attempts, both aggregate resume budgets, and owned HTTP mapping. This evidence-only update does not change the approved decision, behavior, or scope. | @codex |
| 2026-08-12 | Approval-invalidating PR #2 Review thread `3764738232` revision at `2026-08-12T16:25:02+08:00` bounded the previously unbounded stable-identity reconciliation attempt. Preserved prior approval: Approver `@linhai`, Approval Time `2026-08-12T15:55:49+08:00`, Approval Evidence `Approve`, no Approval Context Revision. Reset Decision Status to `Proposed` and Implementation Status to `Not Started` pending reapproval. | @codex |
| 2026-08-12 | Reapproved the bounded commit-reconciliation correction after repository owner `@linhai` identified the ADR/review conflict and supplied exact `Approve`; recorded Approval Time `2026-08-12T16:25:02+08:00`, returned Decision Status to `Accepted`, and restored Implementation Status to `Complete` after the revised checks passed. The approved behavior caps the write and stable-identity reconciliation as two independent 2-second PostgreSQL attempts, returning `Unavailable` when reconciliation remains indeterminate. No Approval Context Revision is recorded because no immutable revision yet represents this approved correction. | @linhai |
| 2026-08-12 | Recorded implementation commit `8de786d` for PR #2 Review thread `3764738232`. Routed format, strict all-target/all-feature Clippy, and the complete all-target/all-feature suite pass: 102 tests, including the red-then-green stalled-reconciliation deadline regression. This evidence-only update does not change the approved decision, behavior, or scope. | @codex |
