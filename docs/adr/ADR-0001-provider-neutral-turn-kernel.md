...
- Every production PostgreSQL operation has the same 2-second attempt deadline.
  ...
  startup attempts use the same deadline. An append outage releases its renewal
  permit before recovery scheduling, preserving a recovery slot even at full
  renewal saturation; other saturation fails closed as `durability-unavailable`.
...
| CT-11 | Lease-renewal and failed-append recovery share a 256-worker admission bound; a failed Turn releases renewal before recovery scheduling so its slot transfers at saturation. | AC-16; `append_outage_releases_liveness_before_fallback_recovery`; `postgres::tests::background_admission_rejects_saturation_and_releases_capacity` |
...
| Resource bounds and backpressure | Applicable — provider floods frames or active Turns exhaust background capacity. | Provider, application durability policy, and PostgreSQL history | AC-9 and AC-16 commands | Item/payload caps fail closed; provider channel stays bounded; worker 257 is rejected; append outage releases renewal before recovery scheduling, transferring capacity without exceeding 256. | AC-9, AC-16 | Pass — durability-cap tests, bounded-channel inspection, background admission unit test, and recovery-handoff regression. |
