...
- Every production PostgreSQL operation has the same 2-second attempt deadline.
  ...
  worker and waits for that worker to release its permit before recovery
  scheduling, preserving confirmed recovery capacity even at full renewal
  saturation. That wait is bounded by the 2-second renewal-attempt deadline;
  other saturation fails closed as `durability-unavailable`.
...
| CT-11 | Lease-renewal and failed-append recovery share a 256-worker admission bound; a failed Turn confirms its renewal worker has released admission before recovery scheduling so the slot transfers without a race at saturation. | AC-16; `append_outage_confirms_admission_handoff_before_recovery`; `postgres::tests::renewal_recovery_shutdown_confirms_permit_release` |
...
| Resource bounds and backpressure | Applicable — provider floods frames or active Turns exhaust background capacity. | Provider, application durability policy, and PostgreSQL history | AC-9 and AC-16 commands | Item/payload caps fail closed; provider channel stays bounded; worker 257 is rejected; append outage confirms renewal admission release before recovery scheduling, transferring capacity without exceeding 256. | AC-9, AC-16 | Pass — durability-cap tests, bounded-channel inspection, synchronous permit-release unit test, and recovery-handoff regression. |
