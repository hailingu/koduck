...
| Timeout and deadline | Applicable — database startup/query or provider establishment/streaming stalls. | Runtime assembly, SQLx, and OpenAI-compatible adapters | AC-9, AC-16, and AC-17 commands | Database startup and query attempts stop at 2 seconds; provider connect/header/idle/total deadlines are 5/30/30/120 seconds and produce typed terminal failure. | AC-9, AC-16, AC-17 | Pass — startup/query deadline behavior tests plus `architecture::production_io_and_background_work_are_bounded`. |
...
| Resource bounds and backpressure | Applicable — provider floods frames or active Turns exhaust background capacity. | Provider, application durability policy, and PostgreSQL history | AC-9 and AC-16 commands | Item/payload caps fail closed; provider channel stays bounded; worker 257 is rejected; append outage directly transfers its reserved permit from renewal into recovery without exceeding 256 or exposing an acquisition window. | AC-9, AC-16 | Pass — durability-cap tests, bounded-channel inspection, atomic reservation-transfer unit test, and recovery-handoff regression. |
