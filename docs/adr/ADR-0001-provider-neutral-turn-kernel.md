...
| CT-7 | Authenticated interruption wins terminal arbitration, including while failed-append recovery is pending; unknown/non-owned Turns are indistinguishable and cancellation remains distinct. | AC-7; `postgres_subject_ownership::production_postgres_contract` |
...
| Cancellation and interruption | Applicable — interrupt races with provider completion/failure, failed-append recovery, or downstream disconnect. | Application runner, PostgreSQL history, and HTTP/SSE adapter | AC-7 and AC-15 commands | Accepted interrupt, including one recorded during `recovery-pending`, is the sole `interrupted` terminal; dependency/disconnect cancellation is `cancelled`; synchronous mappings are distinct 409 codes. | AC-7, AC-15 | Pass — `interrupt_and_cancel_are_distinct`, terminal-arbitration tests, production PostgreSQL recovery-pending interruption, and synchronous mapping regression. |
...
| AC-7 | T-2 | The interrupt contract is exact and an authenticated client stop remains distinct from platform/dependency cancellation. | One owned active SSE Turn, one recovery-pending Turn, one unknown UUID, one non-owned Turn, one already-terminal Turn, and one second Turn with an injected dependency stop. | Run `cargo test -p koduck-ai --test cand_1_contract interrupt_and_cancel_are_distinct -- --exact` and the production PostgreSQL contract test. | Exit code 0; the owned active and recovery-pending cases accept interruption and each reach exactly one durable `interrupted`; unknown and non-owned cases both return indistinguishable `404` problem bodies after correlation-ID normalization; the terminal case returns `409` with code `turn-already-terminal`; the dependency-stopped Turn has exactly one `cancelled`; no tested Turn has another terminal. | Command output, normalized response hashes, PostgreSQL replay, and replay fixtures. | Pass | Local contract and terminal-arbitration tests pass; the production PostgreSQL test deterministically drives an owned Turn into `recovery-pending`, records interruption through the public executor, and verifies one durable `interrupted`. Commits `fe3beb9` and `a7258bc` additionally prove an accepted interrupt wins over simultaneous provider completion or failure. Commit `31ef43f` proves interruption remains observable during provider request establishment. |
...
| AC-16 | T-3 | Database calls and background liveness/recovery work are bounded and fail closed. | Slow query/startup futures, a background admission limit of one, and an append outage while liveness owns capacity. | Run `cargo test -p koduck-ai adapters::history::postgres::tests`, `cargo test -p koduck-ai runtime::tests::database_setup_attempt_maps_deadline_expiration`, `cargo test -p koduck-ai --test cand_1_durability append_outage_confirms_admission_handoff_before_recovery -- --exact`, and the production bound architecture test. | Exit 0; slow database query/startup calls return typed unavailable/timeout errors; a second worker is rejected while the first permit is held; ordinary guard drop remains non-blocking; recovery handoff waits only for the bounded renewal attempt and moves the same reserved permit into recovery without an acquisition window; production uses limit 256 for both renewal and recovery. | Command output and deterministic source inspection. | Pass | Local query/startup deadline, shared admission, atomic reservation transfer, handoff, and architecture regression tests pass. |
...
- Pre-commit decomposition review of the current worktree: maintained
  ...
  (718), `provider/mod.rs` (582), `runner.rs` (576), `postgres.rs` (508), and
  ...
  `cand_1_durability.rs` (651). All remain below their 800/1,200-line exception
