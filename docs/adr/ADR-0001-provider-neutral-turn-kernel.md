...
- Decomposition review at `fe3beb9`: `sqlx_executor.rs` is 742 physical lines,
  above the 400-line review threshold and below the 800-line exception limit.
  It remains one cohesive SQLx transaction executor because acceptance,
  append, replay, lease fencing, recovery, and payload translation share the
  same transaction and row invariants; recovery-thread ownership was extracted
  to `postgres/recovery.rs`. No engineering exception is required or claimed.
