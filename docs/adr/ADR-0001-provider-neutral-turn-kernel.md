...
| --- | --- | --- | --- | --- | --- |
| A-1 | ADR approved | An eligible non-author approver, approval time, and exact `Approval Evidence: Approve` are recorded; any optional Approval Context Revision is informational, non-binding, and exactly represents the approved document | ADR metadata | Complete | `@linhai` identified ADR-0001 and PR #2 Review thread `3764738232`, then supplied exact `Approve`; metadata records `2026-08-12T16:25:02+08:00`. No Approval Context Revision is recorded because approval was intentionally task-context-bound rather than revision-bound. |
