...
- **Approval Time [Conditionally Required — Decision Status is or has been `Accepted`]**: 2026-08-20T01:23:05+08:00
- **Approval Evidence [Conditionally Required — Decision Status is or has been `Accepted`]**: Approve
- **Approval Context Revision [Optional — informational and non-binding; not approval evidence]**: `c576fbd` exactly represents the revised document content present for the 2026-08-20 reapproval
...
| --- | --- | --- | --- | --- | --- |
| A-1 | ADR approved | An eligible non-author approver, approval time, and exact `Approval Evidence: Approve` are recorded. | ADR metadata | Complete | Reapproved by `@linhai` with exact `Approve` at `2026-08-20T01:23:05+08:00` in the task context identifying this record and revision `c576fbd`; the Approval Context Revision is informational only, and the invalidated 2026-08-12 approval is retained in the Change Log. |
...
| Date | Change | Author |
| --- | --- | --- |
| 2026-08-20 | Reapproved by `@linhai` with exact `Approve` at 2026-08-20T01:23:05+08:00 in the task context identifying this record and revision `c576fbd`. Every acceptance check was then re-executed post-reapproval: workflow inspection and check-run evidence for exact SHA `ca67c1f` (run `32280948402`, all three named checks `success`, format job gating both governance commands per the revised AC-1), the PostgreSQL integration inside the test job, and the `dev` protection API re-query confirming exactly the three strict required contexts. AC-1 through AC-6 are `Pass`; the record returns to `Accepted, Verified`. | @zcode |
