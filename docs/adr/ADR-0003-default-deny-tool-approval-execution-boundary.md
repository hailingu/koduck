...
- `koduck-ai/tests/internal/cand_2_limits.rs` (crate-internal since the
  ...
  ordered boundary.
- `koduck-ai/tests/internal/cand_2_postgres.rs` is 685 physical lines,
  above the 600-line test review threshold and below the 1,200-line exception
  ...
  replay, identity-conflict, sealed-approver, and schema-regression cases.
  Splitting those cases now would duplicate the security-sensitive fixture
  rather than create an independent test boundary. Cyclomatic complexity is
  `N/A — no configured complexity tool`; the substitute review measured every
  test function at a shallow maximum nesting depth of three, below the
  nesting threshold, with the five-to-seven statement literal tables carried
  as module-level constants outside function bodies. Reassess this harness
