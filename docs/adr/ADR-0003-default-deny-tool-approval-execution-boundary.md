...
| cancellation and interruption | Turn is interrupted while approval is pending, while effect is not started, after effect started, and while cancel acknowledgement is lost. | C-2/C-5 cancellation and executor adapter | Fault-driven cases in AC-8. | Pending work dispatches zero times; acknowledged running work is `cancelled` with reported effect state; unacknowledged work is `timed_out/unknown`; one CAND-1 Turn terminal remains durable. | AC-8 | Not Started | Partially observed on the current uncommitted task revision: the route-level transport legs prove truthful D-7 cancellation terminals through the authenticated `ToolInterruptionRoute` with the canonical subject-ownership gate — pending work dispatches zero times and closes `cancelled/not_started`, acknowledged running work commits `cancelled` with the exact executor-observed state, a lost acknowledgement commits `timed_out/unknown`, non-owner/unknown/ownership-outage interruptions are indistinguishable no-ops or typed failures with zero mutations and zero tombstones, and no late executor result is delivered after the close. The harness never constructs a `TurnHistory`, so the declared single durable CAND-1 Turn terminal and its replay are NOT observed and the row stays open until the integration leg lands. ||
...
| framework or trust-boundary rejection | Missing/forged identity, cross-tenant decision, malicious MCP descriptor/result, and direct-executor bypass are attempted. | C-1/C-7, C-5, Tool/MCP/executor adapters | Axum HTTP contract, malicious adapter fixtures, and architecture inspection in AC-1/AC-6/AC-11. | Identity cases return exact 401/404 behavior with zero mutation; untrusted content cannot change authority; no forbidden direct execution path exists. | AC-1, AC-6, AC-11 | Pass | Observed on the current uncommitted task revision: missing identity returns `401` with `WWW-Authenticate`, and unscoped/cross-tenant/cross-Thread/non-owning deciders receive one indistinguishable `404` with zero store mutations (AC-6). A malicious MCP declaration is rejected before any owned action exists, untrusted result content cannot widen the immutable profile, and the disabled runtime exposes typed unavailability with zero fallback identifiers (AC-11). The architecture guard now observes the production provider-native translation entering the C-5 boundary; MCP has no production ingress in this slice and its adapter delegates to the same native translation, so AC-1 remains open only for the future MCP-ingress coverage rather than because the native call path is unwired. ||
