...
| cancellation and interruption | Turn is interrupted while approval is pending, while effect is not started, after effect started, and while cancel acknowledgement is lost. | C-2/C-5 cancellation and executor adapter | Fault-driven cases in AC-8. | Pending work dispatches zero times; acknowledged running work is `cancelled` with reported effect state; unacknowledged work is `timed_out/unknown`; one CAND-1 Turn terminal remains durable. | AC-8 | Not Started | Not run — authenticated interruption, bounded executor cancellation acknowledgement, and lost-acknowledgement fixtures are not implemented |
...
- Current decomposition review:
  `koduck-ai/src/domain/execution.rs` is 764 physical lines and
  `koduck-ai/src/application/execution.rs` is 739 physical lines. Both exceed
  ...
  `koduck-ai/src/domain/execution/authority.rs` is 54 physical lines and owns only multi-Turn lookup,
  and strong shared attempt-budget retention. Reclamation remains deferred
  ...
  path without creating an independent lifecycle. Reassess both during T-3
  when durable record mapping and executor transport provide real extraction
  boundaries. `koduck-ai/src/domain/tool.rs` is 658 physical lines, above the
  ...
  independent lifecycle. `ExecutionCoordinator::execute` is 85 physical lines,
  ...
  dispatch, effect-observation, and conditional-commit sequence; extracting a
  phase would obscure the TC-07 ordering without creating an independent owner.
  Cyclomatic complexity is `N/A — no configured complexity tool`; the required
  substitute review measured the 85-line span and maximum executable nesting
  depth of two, below the nesting threshold.
- `ToolExecutionDriver::execute` is 98 physical lines, including its intent and
  ...
  complexity tool`; the substitute review measured the 98-line span and maximum
  executable nesting depth of three (loop, match, arm), below the nesting
  threshold.
  `koduck-ai/tests/internal/cand_2_execution.rs` is 1,012 physical lines,
...
- `koduck-ai/tests/internal/cand_2_retry.rs` is 744 physical lines, above the
  ...
  measured size.
- `koduck-ai/tests/architecture.rs` is 675 physical lines, above the 600-line
  maintained-test review threshold and below the 1,200-line exception limit. It
  is one cross-cutting source-inspection harness whose dependency-direction,
  fallback, authority, single-dispatch, and ADR-evidence assertions share the
  same production-graph walkers (`collect_text`, `inspect_rust_files`, and
  `count_identifier_tokens`); splitting it would duplicate those walkers across
  files rather than create an independent test boundary. Because it is shared
  across several ADRs' structural checks, its size is recorded here as a
  point-in-time review rather than wired to an automated count. Reassess it
  when a later ADR adds a structurally distinct inspection family; no
  engineering exception is required at the measured size.
