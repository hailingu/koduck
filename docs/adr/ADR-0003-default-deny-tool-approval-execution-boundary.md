...
| framework or trust-boundary rejection | Missing/forged identity, cross-tenant decision, malicious MCP descriptor/result, and direct-executor bypass are attempted. | C-1/C-7, C-5, Tool/MCP/executor adapters | Axum HTTP contract, malicious adapter fixtures, and architecture inspection in AC-1/AC-6/AC-11. | Identity cases return exact 401/404 behavior with zero mutation; untrusted content cannot change authority; no forbidden direct execution path exists. | AC-1, AC-6, AC-11 | Not Started | Partially observed on the current uncommitted task revision: the authenticated HTTP decision contract passes (AC-6), the malicious MCP-declaration and untrusted-result fixtures fail closed with the identical-envelope and no-fallback evidence (AC-11), and the architecture no-bypass scan is green; the row stays open because AC-1's native entrypoint-delegation counting still waits on the T-2 provider Tool-call translation entrypoints. |
