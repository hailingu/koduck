...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Concurrency and ordering | The asynchronous Reqwest pump could publish clean end before the final data/usage frame or more than once | Provider transport channel and `StreamState` | Local HTTP fixture sends split/chunked terminal and usage bytes; collect the exact owned sequence | Every decoded data frame precedes exactly one clean-end frame; owned events remain delta, optional usage, then exactly one completed event | AC-2 | Pass | `reqwest_clean_eof_is_ordered_after_decoded_frames` exits 0 at final source revision `82227022b537fb2206b7481cc6df463d12a5fed6`: the chunked fixture split across byte boundaries yields exactly `Data(stop)`, `Data(usage)`, `CleanEnd` and then stream end with no error; AC-1 additionally proves one `Completed` per variant. |
...
| A-7 | Contract and baseline risks covered, when applicable | PSC-1 through PSC-7 map to explicit checks, and all five baseline risk rows are complete before approval and Pass before review-ready or completion | Contract-To-Check Traceability, Risk Coverage Matrix, acceptance checks, and stable evidence | Complete | PSC-1 through PSC-7 remain mapped to AC-1 through AC-7 unchanged; all five Risk Coverage Matrix rows are `Pass` with per-row evidence re-pinned to the final source revision `82227022b537fb2206b7481cc6df463d12a5fed6` — the framework/trust-boundary row's full twenty-case coverage was completed by the review corrections (`794eb07`, `4d6d563`, `3af3082`, `4ec1eae`, `8222702`) and is certified there, not at the initial implementation commit. |
...
- Post-implementation decomposition review, last remeasured at final source
  revision `82227022b537fb2206b7481cc6df463d12a5fed6`,
  ...
  newly extracted shared guard `adapters/unique_json.rs` (107) plus the
  correspondingly reduced `adapters/tool.rs` (249) are, below the
