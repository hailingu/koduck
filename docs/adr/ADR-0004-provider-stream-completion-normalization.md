...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Concurrency and ordering | The asynchronous Reqwest pump could publish clean end before the final data/usage frame or more than once | Provider transport channel and `StreamState` | Local HTTP fixture sends split/chunked terminal and usage bytes; collect the exact owned sequence | Every decoded data frame precedes exactly one clean-end frame; owned events remain delta, optional usage, then exactly one completed event | AC-2 | Pass | `reqwest_clean_eof_is_ordered_after_decoded_frames` exits 0 at final source revision `0a0e151c22ffb259addc08ceb32ffaa121f4b359`: the chunked fixture split across byte boundaries yields exactly `Data(stop)`, `Data(usage)`, `CleanEnd` and then stream end with no error; AC-1 additionally proves one `Completed` per variant. |
...
| A-7 | Contract and baseline risks covered, when applicable | PSC-1 through PSC-7 map to explicit checks, and all five baseline risk rows are complete before approval and Pass before review-ready or completion | Contract-To-Check Traceability, Risk Coverage Matrix, acceptance checks, and stable evidence | Complete | PSC-1 through PSC-7 remain mapped to AC-1 through AC-7 unchanged; all five Risk Coverage Matrix rows are `Pass` with per-row evidence re-pinned to the final source revision `0a0e151c22ffb259addc08ceb32ffaa121f4b359` — the framework/trust-boundary row's full twenty-case coverage was completed by the review corrections (`794eb07`, `4d6d563`, `3af3082`, `4ec1eae`, `0a0e151`) and is certified there, not at the initial implementation commit. |
...
- Post-implementation decomposition review, last remeasured at final source
  revision `0a0e151c22ffb259addc08ceb32ffaa121f4b359`,
  measuring every file changed by this record: `provider/mod.rs` is 574
  physical lines (from 559), above the 400-line review threshold and below the
  800-line exception limit; `provider/stream_state.rs` is 468 — now above
  the 400-line review threshold after folding in the duplicate-member
  guard `ensure_unique_members` beside the parsing state machine it
  validates, keeping one terminal-evidence owner in view; extracting it
  again would recreate the unapproved-scope new file or split one
  frame-validation invariant across files — and it remains far below the
  800-line exception limit. The
  ...
  from 623), and — after the review corrections — `openai_provider.rs` (710,
  ...
  `provider_stream_lifecycle.rs` (487) remains below the threshold; the
  adjacent `adapters/tool.rs` and `adapters/mod.rs` are byte-identical to
  their pre-pull-request state. All
...
| Date | Change | Author |
| --- | --- | --- |
| 2026-08-25 | Addressed the twelfth pull-request-4 automated-review finding (P1): the duplicate-member guard was folded from the new `provider/unique_json.rs` into the affected-paths-enumerated `koduck-ai/src/adapters/provider/stream_state.rs` as `ensure_unique_members`, with no behavior change, no new maintained source file, and no ADR rescope; the decomposition review records `stream_state.rs` at 468 lines crossing the 400-line review threshold with its cohesion rationale. All live evidence re-pinned to final source revision `0a0e151`. No approved decision content changed. | @zcode |
| 2026-08-24 | Addressed the eleventh pull-request-4 automated-review round: the P1 — the shared JSON extraction had changed `adapters/mod.rs`, `adapters/tool.rs`, and a new `adapters/unique_json.rs` outside this record's affected paths — was resolved by moving the guard provider-local (`koduck-ai/src/adapters/provider/unique_json.rs`, declared in the affected `provider/mod.rs`) with `adapters/mod.rs` and `adapters/tool.rs` restored byte-identical to pre-PR state; the P2 — buffered newline-less trailing bytes at body EOF were still parsed as a frame before clean end — now fails closed as `OPENAI_BODY_FAILED` with no clean end (regression observed RED first). All live evidence re-pinned to final source revision `6c0af39`; decomposition review remeasured. No approved decision content changed. | @zcode |
| 2026-08-24 | Addressed the tenth pull-request-4 automated-review P2: the C-5 duplicate-member guard was extracted into `adapters::unique_json` and applied before every provider frame parse, so duplicate JSON members — including two conflicting `finish_reason` values — fail closed as `INVALID_FRAME` instead of collapsing via serde_json's last-duplicate-wins behavior (two cases observed RED first). AC-3 and the risk row now cite the twenty-case table; all live evidence re-pinned to final source revision `8222702`; decomposition review remeasured including the new module. No approved decision content changed. | @zcode |
| 2026-08-24 | Addressed the eighth pull-request-4 automated-review P2: every Risk Coverage Matrix row, the AC-6 suite count, checklist A-2/A-7, and the decomposition-review anchor now certify the final source revision `4ec1eae` instead of the initial implementation commit, so no row claims coverage that only later review corrections completed. No approved decision content changed. | @zcode |
