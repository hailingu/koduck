...
`koduck-ai/tests/internal/cand_2_postgres_recovery.rs`;
...
The two `sqlx_executor` files, `cand_1_kernel.rs`, and the PostgreSQL-gated
`cand_2_postgres_recovery.rs` entries are the in-scope focused regressions
of the changed resource contract (Scope: "Update … focused regressions"),
added here as evidence-only enumeration after implementation; the two new
modules decompose the accumulator and the Tool-call seam out of `runner.rs`.
...
| Date | Change | Author |
| --- | --- | --- |
| 2026-08-27 | Addressed the fifth PR-8 review round on `199410b`: the accumulator now flushes the moment its buffered content reaches the 16,384-byte cap — including an exact-cap split remainder — implementing TN-1's "16,384 bytes or 500 ms, whichever occurs first" trigger exactly; PLB-2's flush-before-exceeding bound is unchanged and AC-2 now asserts the exact-cap and exact-multiple cases. No approved decision content changed. | @zcode |
| 2026-08-27 | Addressed the fourth PR-8 review round on `efc97b9`: a flush append that the durable arbitration replaces with an `Interrupted` terminal now stops the flush and closes the Turn instead of appending past the terminal and double-transitioning the lifecycle (P1, regression `interrupt_won_flush_append_closes_the_turn_without_double_transition`, observed RED as `Transition { from: Interrupted, to: Interrupted }`). No approved decision content changed. | @zcode |
