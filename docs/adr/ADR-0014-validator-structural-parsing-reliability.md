...
| T-3 | Replace ADR/OCR/ADD filename dispatch recognition, update the file marker, and verify the complete scoped result. | `validate` record dispatch; focused classification test; package and governance checks; a later accepted local SonarQube OCR. | In Progress | `536cf5b`; ADR marker and dispatch are implemented and AC-3/AC-4 pass. Accepted OCR-0011 found one residual scoped `validate.mjs` L584 row for `f9d08d5`, restored the `f2cc310` baseline, and requires a narrow follow-up correction before a new OCR. |
