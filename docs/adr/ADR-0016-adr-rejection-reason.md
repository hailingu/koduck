...
| --- | --- | --- | --- | --- |
| tools/governance-validator/validate.mjs | validateTerminalStages; validateRejectionMetadata; isCompleteValue | N/A — stable symbols are sufficient | In validateTerminalStages, keep the existing rejection metadata check for both record types and require a reason only when the path is an ADR; reuse isCompleteValue and the current stderr error collection | 06445f7a638dd280aa1a16d653b1180b33ee48ee — inspected baseline; record final implementation revision before completion |
| tools/governance-validator/lib/markdown-parsing.mjs | isRecordFilename | N/A — stable symbol is sufficient | Reuse the existing imported filename classifier for the ADR-only branch; no parser change is planned | 0f1fe3dbac8849c8aebae93741cff5a8628fb534 — inspected before this revision |
...
| framework or trust-boundary rejection | Applicable — a rejected record omits its reason or attempts to supply it through non-active or ambiguous metadata | Markdown-to-lifecycle validation boundary | Real CLI fixture regression groups for RR-2 and RR-3 | Exit 1 with path and ADR_REJECTION_REASON_REQUIRED on stderr for invalid input; exit 0 after valid correction | AC-1, AC-2, AC-3 | Not Started | Not run — implementation not started |
...
| A-8 | Governance validation passed | Repository governance validation exits 0 | npm run validate --prefix tools/governance-validator output | Complete | 2026-09-23: revised proposal and index validation exited 0; git diff --check exited 0; the earlier validator suite passed 184/184 and must be repeated against the implementation |
