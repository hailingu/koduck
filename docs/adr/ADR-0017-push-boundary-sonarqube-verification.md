...
| AC-6 | T-1 | Implementation and policy match the scoped decision | Committed implementation diff against 6bf6a225f2c8641d894d1f0d17919bd46d6b064a and retained-test map | Execute Scoped Verification Procedure and its listed routed commands; inspect the named symbols and policy anchors | Only Affected paths changed; PV-1 through PV-6 implemented, including markers and reporting eligibility; named retained protections unchanged; all listed commands exit 0 | Path diff, symbol/policy dispositions, test map and command results | In Progress | Working-tree review complete: only Affected paths changed; markers placed at `.githooks/pre-push` and `tools/sonarqube/install.sh` (comment after shebang), `gate.py`/`git_snapshot.py`/six modified test files (comment after module docstring), `tools/sonarqube/README.md` line 1 and `AGENTS.md` marker list; `gate.main` keeps `project_lock()`/`database_fixture()` lifetimes and the signal handler; `scan_runtime`, `sonar_api`, `coverage_report`, `postgres_fixture` and `scripts/sonar-quality-gate.sh` untouched; local suite, `ruff check` and `ruff format --check` green. The committed-revision `git diff --name-status` against `6bf6a22`, routed command reruns and `git diff --check` are recorded in the follow-up evidence change on the exact commit |
| AC-7 | T-1 | Real implementation revision satisfies the retained admission gate | Committed implementation revision, isolated fixture database and owner-authorized scanner environment | Pre-push evidence qualifying per target under PV-5 plus its read-only identity comparison, or python3 tools/sonarqube/gate.py check --revision HEAD | Zero incremental issues, Quality Gate OK, and at least 80% changed product-line coverage or proven-zero-line case; pre-push alternative has the complete admission line and matching recomputed revision/tree/base/policy; overall Git push result reported separately | Actual invocation and combined-output admission line with identities, analysis ID, new_issues, quality_gate, covered and coverable; required CI green and exact-revision review before review-ready | In Progress | Local live admission on the committed implementation revision and its read-only identity comparison are recorded in the follow-up evidence change; required CI and exact-revision review remain pending until the implementation pull request is pushed and reviewed |
...
| A-2 | Complete task delivered | T-1 Complete and AC-1 through AC-7 Pass with actual evidence | Subtask and acceptance rows | In Progress | T-1 In Progress on `zcode/adr-0017-push-boundary-sonar`; AC-1 through AC-5 Pass with test evidence; AC-6 and AC-7 await the committed-revision diff/routed commands, the local live admission and, for review-ready status, CI and exact-revision review on the implementation pull request |
...
| A-7 | Contracts and risks covered | PV-1 through PV-7 traced; applicable risk rows Pass; N/A rows cite actual scope review | Traceability, risk and acceptance evidence | In Progress | PV-1 through PV-7 each trace to passing checks AC-1 through AC-5; the three applicable Risk Coverage Matrix rows are Pass with test evidence and both N/A rows cite the AC-6 structured diff review; final closure follows AC-6/AC-7 committed-revision evidence |
