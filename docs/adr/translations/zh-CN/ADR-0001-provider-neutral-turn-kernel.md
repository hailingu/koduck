...
| CT-7 | 已认证 Interrupt 仅在 `started` Turn 仍有未 Fenced、未过期 Live Owner 时赢得 Terminal Arbitration；已脱离 Stream 的 `recovery-pending` 与 Expired Turn 拒绝 Interrupt，因为已无 Stream 可交付 `turn.interrupted`。Unknown/Non-owned 仍不可区分，Cancellation 保持独立。 | AC-7；`postgres_subject_ownership::production_postgres_contract` |
...
| Cancellation and interruption | 适用 — Interrupt 与 Provider Terminal、Failed-append Recovery、Lease Expiry 或 Downstream Disconnect 竞争。 | Runner、PostgreSQL History 与 HTTP/SSE Adapter | AC-7、AC-15 | Live-owner Interrupt 是唯一 `interrupted`；Recovery-pending 或 Expired Owner 拒绝 Interrupt，而不确认无法交付的 SSE Event；Dependency/Disconnect 仍为 `cancelled`；同步 409 Code 可区分。 | AC-7、AC-15 | Pass — Interrupt、Arbitration、生产 PostgreSQL Recovery-pending/Expired-owner Rejection 与 Sync Mapping Regression。 |
...
| AC-7 | T-2 | Interrupt Contract 精确，且已认证 Client Stop 与 Platform/Dependency Cancellation 可区分。 | 一个 Owner Live SSE Turn、一个 Recovery-pending Turn、一个 Expired Started Turn、一个未知 UUID、一个非 Owner Turn、一个已终态 Turn，以及第二个注入 Dependency Stop 的 Turn。 | 运行 `cargo test -p koduck-ai --test cand_1_contract interrupt_and_cancel_are_distinct -- --exact` 与生产 PostgreSQL Contract Test。 | Exit Code 0；仅未 Fenced/未过期的 `started` Owner 接受 Interrupt 并到达恰好一个 Durable `interrupted`；Recovery-pending 与 Expired Started Turn 拒绝 Interrupt；Unknown/Non-owned 返回不可区分的 `404`；Terminal 返回 `409 turn-already-terminal`；Dependency Cancellation 保持 `cancelled`；无其他 Terminal。 | Command Output、Normalized Response Hash、PostgreSQL Replay 和 Replay Fixture。 | Pass | Local Contract/Terminal-arbitration Test 通过；生产 PostgreSQL Test 验证 Live-owner Interrupt，并确定性拒绝 Recovery-pending 与 Expired-started Fixture。Commit `fe3beb9`/`a7258bc` 证明 Live Interrupt 赢过并发 Provider Terminal；Commit `31ef43f` 证明 Request Establishment 期间仍可观察。 |
...
- 当前 Worktree 的提交前分解评审：超过 400 行评审阈值的生产文件为
  `sqlx_executor.rs`（721）、`provider/mod.rs`（582）、`runner.rs`（591）、
