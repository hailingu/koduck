...
| T-3 | 实现 AI 自有 PostgreSQL History 与带 Fencing Liveness Adapter，并证明故障、恢复与无 Fallback 行为。 | 初始持久化接受、Append/Replay、Migration、Deadline/Buffer Cap、Lease Acquire/Renew/Fence、Orphan Reconciliation、Crash/Fault Test 和无旧运行依赖证据。 | In Progress | Commit `46f2a39` 与 `80fc2ff` 实现 Fail-closed Durability Policy、PostgreSQL Schema/Adapter Boundary、精确 Lease Timing 与确定性 Crash/Race Evidence；AC-8 至 AC-11 通过。Concrete PostgreSQL Executor、HTTP Runtime、Configuration 和 `main.rs` 尚缺，因此 AC-12 前置条件尚未满足。 |
