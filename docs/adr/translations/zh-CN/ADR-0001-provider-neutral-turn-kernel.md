...
- `POST /api/v1/ai/chat/stream` 接受同一 JSON，返回 `200` 与
  ...
  `item.created`，以及恰好一个 `turn.completed`、`turn.failed`、
  `turn.interrupted` 或 `turn.cancelled`。每个 Event JSON 包含一致的
  ...
  `turn.completed` 额外包含上述精确 `usage` Object。
...
- 缺失或无效 Identity 返回 `401`、`WWW-Authenticate: Bearer`，并用
  ...
  `400`；初始或中途持久化失败返回 `503` 与 `durability-unavailable`。每个 Error
...
| A-2 | 完整任务已交付 | 每个已声明子任务都有实际实施证据；每个适用验收检查均为 `Pass` 且有实际结果和证据；它们共同满足完整任务结果 | Implementation Plan 与 Acceptance Checks Row | Complete | T-1 至 T-3 均为 `Complete`；AC-1 至 AC-14 均为 `Pass`；Review Correction Commit `a7258bc` 上 `cargo fmt --all -- --check`、针对全部 `koduck-ai` Target 的严格 Clippy 与全部 45 个测试通过。 |
...
- `a7258bc` 分解评审：`sqlx_executor.rs` 为 742 个物理行，`postgres.rs`
  为 409 个物理行，两者均高于 400 行评审阈值、低于 800 行例外上限。
  Executor 保持内聚，因为 Acceptance、Append、Replay、Lease Fencing、Recovery
  与 Payload Translation 共享 Transaction/Row Invariant；`postgres.rs` 保持为该
  Executor 上内聚的 History/Liveness Adapter，抽取其中单一 Provider-terminal
  Delegation 只会形成透传式拆分。Recovery Thread Ownership 仍已拆至
  `postgres/recovery.rs`。Cyclomatic Complexity 为 `N/A — 未配置复杂度工具`；
  本次变更的函数均少于 60 行且嵌套深度不超过 4。本项不需要且未声明
