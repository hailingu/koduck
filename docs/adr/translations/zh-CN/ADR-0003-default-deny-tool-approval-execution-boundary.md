...
| --- | --- | --- | --- | --- | --- | --- | --- |
| concurrency and ordering | 两个审批人和 32 个派发方争抢一个 requested D-6/prepared D-7，同时一个终态结果也在竞争。 | C-5 加 C-6 PostgreSQL Adapter | AC-12 生产 PostgreSQL 竞争测试。 | 一个 D-6 决策和一个派发声明获胜；一个 D-7 终态持久化；Executor 派发计数恰好为 1；每个失败者返回现有终态或 Typed 冲突。 | AC-12 | Not Started | 在当前未提交任务修订上针对一次性生产 PostgreSQL 观察到存储层竞争证据：经生产 HTTP 路由驱动 `SqlxApprovalRecordStore` 的 32 个并发决策在一个 requested 审批上产生恰 1 个获胜者（版本 2）和 31 个相同终态投影；32 个并发 `SqlxExecutionAttemptStore::claim_running` 产生 1 个 `Claimed{2}` 和 31 个 `Existing{running,2}` 读取；32 个并发 `commit_terminal` 产生 1 个 `Won{3}` 和 31 个相同的 `ExistingTerminal` 规范终态，其幂等重放返回同一单个终态。`(tenant_id, turn_id) WHERE status='running'` 上的持久部分唯一索引保持跨实例每 Turn 单一 running D-7，相同的丢失确认重放可对账，且声明或提交绑定的每个单字段不可变漂移都在不迁移规范行的前提下被拒绝（声明为类型化 `IdentityConflict`，提交为类型化 `Conflict`）。本行保持 `Not Started`：AC-12 Harness 从不调用 Executor——其派发计数是在 `claim_running` 获胜后递增的本地计数器——且生产 Coordinator 仍经进程内权威声明、不调用持久 `insert_prepared`/`claim_running`，因此“Executor 派发计数恰为 1”的预期与 TC-12 跨实例不重复派发保证尚未端到端证明。 |
...
| cancellation and interruption | Turn 在审批待决时、效果未开始时、效果已开始后，以及取消确认丢失时被中断。 | C-2/C-5 取消和 Executor Adapter | AC-8 中的故障驱动用例。 | 待决工作零次派发；已确认的运行中工作为带报告效果状态的 `cancelled`；未确认的工作为 `timed_out/unknown`；一个 CAND-1 Turn 终态保持持久。 | AC-8 | Pass | 在当前未提交任务修订上观察到：路由层传输分支经带规范主体所有权门的已认证 `ToolInterruptionRoute` 证明如实的 D-7 取消终态——待决工作零次派发并关闭为 `cancelled/not_started`，已确认的运行中工作以 Executor 观测到的精确状态提交 `cancelled`，丢失的确认提交 `timed_out/unknown`，非所有者/未知/所有权故障中断为不可区分的无操作或类型化失败且零变更零墓碑，关闭后不交付迟到 Executor 结果。该集成分支现已落地：持久腿（`tests/internal/cand_2_cancellation_turn_terminal.rs`，以 `KODUCK_AI_TEST_DATABASE_URL` 门控）经真实持久条件存储提交其 D-7 终态（幂等重放确认版本 3 的规范 `cancelled/not_started`），且被中断 D-7 自身的 Turn 在生产 `PostgreSQL` 历史中恰保持一个持久终态——持久中断请求为同一租户/Thread/Turn 提交唯一的 `Interrupted` 终态，迟到的 Provider 完成被拒绝 `AlreadyTerminal`，规范重放恰好返回该终态。两条过期所有者分支将规范租约行 Fence 或到期，观察到中断在任何变更前失败：准备态 D-7 保持版本 1 且 Turn 不记录终态（TC-07）。 |
...
| AC-8 | T-2 | 已认证中断和租约 Fencing 产生如实的 Executor 取消结果，且无迟到结果交付。 | 审批传输和隔离 Executor Harness；待决审批；prepared 尝试；已确认 not_started/started 的运行中尝试和缺失取消确认。 | 运行 `cargo test -p koduck-ai --lib cand_2_cancellation_tests`。 | Exit 0；待决/prepared 用例派发 0 次并取消；已确认用例为带精确状态的 cancelled；缺失确认在 30 秒到达 `timed_out/unknown`；迟到结果交付计数为 0；存在一个 Turn 终态。 | Command Output、虚拟时钟 Trace 和重放。 | Pass | 声明的命令在设置 `KODUCK_AI_TEST_DATABASE_URL` 并针对一次性生产 PostgreSQL 时于当前未提交任务修订上 Exit 0（39 项通过、0 失败；无该变量时持久分支提前返回）。路由层分支经带主体所有权门的 `ToolInterruptionRoute` 证明如实取消结果——待决工作零派发并关闭为 `cancelled/not_started`，已确认运行中提交带精确 Executor 观测状态的 `cancelled`，缺失确认提交 `timed_out/unknown`，且不交付迟到 Executor 结果。此前缺失的集成分支现为取消 Harness 的 `turn_terminal` 兄弟（`tests/internal/cand_2_cancellation_turn_terminal.rs`，以 `KODUCK_AI_TEST_DATABASE_URL` 门控，因其终态提交跨越真实持久存储）：它经 `SqlxExecutionAttemptStore` 规范播种准备态 D-7（`insert_prepared` 版本 1），经生产 `TurnRunner::request_interrupt` 组合中断——已认证的 `interruption_thread` 解析、由基于规范 `turn_leases` 行的持久 `SqlxTurnLeaseValidator` 门控的 C-5 准备态尝试取消、经同一存储的持久 D-7 终态写入，以及按此顺序提交的规范 CAND-1 `Interrupted` Turn 终态——经幂等 `commit_terminal` 重放证明持久行恰含一个规范终态（`ExistingTerminal`，版本 3），并经由生产 `PostgreSQL` 历史（`PostgresTurnHistory` 基于 `SqlxPostgresExecutor`）证明被中断 D-7 自身的 Turn 恰保持一个持久终态：历史铸造规范 Thread/Turn 身份，准备态 D-7 绑定随后携带该身份，已认证的 `request_interrupt` 为同一租户/Thread/Turn 提交唯一的 `Interrupted` Turn 终态，迟到的 Provider 完成追加被拒绝 `AlreadyTerminal`，且该 Turn 的规范 `replay` 恰好返回该终态。同一 Harness 的两条过期所有者分支把租约行 Fence（Generation 递增）或移至检查窗口外到期，观察到 `request_interrupt` 在任何变更前失败：准备态 D-7 行保持 `prepared` 状态与版本 1，且 Turn 不记录终态（TC-07）。 |
...
| AC-12 | T-3 | PostgreSQL 在多实例竞争下恰好允许一个决策、一个派发声明和一个终态提交。 | 设置了 `KODUCK_AI_TEST_DATABASE_URL` 的全新 PostgreSQL 数据库；Migration 此前未应用；一个 D-6/D-7 的每次迁移有 32 个竞争者。 | 运行 `cargo test -p koduck-ai --test postgres_cand_2 postgres_cand_2_transitions_are_single_winner -- --exact`。 | Exit 0；Migration 成功；每次迁移有 1 个获胜者和 31 个已存在终态/冲突结果；该 D-7 的 Executor 派发计数为 1；重放包含一个终态 D-7 投影。 | Command Output、SQL 迁移计数和重放哈希。 | Not Started | 当前未提交任务修订上的部分存储层证据：命名命令针对一次性生产 PostgreSQL Exit 0（运行两次验证竞争稳定性）。命名 Harness 先把每个 Migration 幂等地应用两次，再以 32 个竞争者竞逐每次迁移：D-6 决策分支经生产 HTTP 路由驱动 `SqlxApprovalRecordStore`（以精确的规范 `insert_requested` 行播种，因为领域 `ApprovalRequest` 需要 crate 内部 C-5 Sealing 服务）——每个竞争者收到相同的 `{approval_id, status: accepted, decision: accepted, version: 2}` 投影，且持久行证明单获胜者（`accepted`/`requester`/版本 2）；D-7 派发声明分支竞逐 `SqlxExecutionAttemptStore::claim_running`，得到 1 个 `Claimed{2}` 与 31 个 `Existing{running, 2}`；D-7 终态提交分支竞逐 `commit_terminal`，得到 1 个 `Won{3}` 和 31 个相同的 `ExistingTerminal` 规范终态投影（绑定、版本 3、精确结果），且幂等重放返回同一单个终态投影。内部兄弟分支（`tests/internal/cand_2_postgres_attempts.rs`，`--lib cand_2_postgres_tests`）额外证明带丢失确认重放和类型 `IdentityConflict` 漂移的幂等 prepared 插入、跨实例的持久单 running D-7 每 Turn 拒绝、prepared 取消与非法 prepared 成功（`Conflict`）的对比、在每一层强制的 still-prepared 取消不变量——prepared 行只接受 cancelled/not_started：存储按效果状态区分的条件来源与 `DurableAttemptTerminal::legal_from` 拒绝从 prepared 提交 cancelled/started 与 cancelled/unknown，Schema 作为纵深防御拒绝这些元组——完全合法的对照终态元组（succeeded/started 与 still-prepared 的 cancelled/not_started 关闭）必须被接受（因此每次拒绝仅由其目标条件决定）；Schema 本身拒绝九种非法终元组形状——包括 running 缺 started-at 时间戳、失败带空白 failure code、succeeded 缺输出、succeeded 输出超过 1,048,576 字节、缺效果状态证据的终态、终态时间早于其派发，以及不可能的 still-prepared 取消 cancelled/started 与 cancelled/unknown；声明或提交绑定的每个单字段不可变漂移（Thread、Turn、租约 Generation、描述符 ID/版本、效果、动作摘要、Profile ID/版本）都在不迁移规范行的前提下被拒绝——声明为类型化 `IdentityConflict`、提交为类型化 `Conflict`；并且 50 轮两个真并发声明在同一新 Turn 上从不出现 `Unavailable`（每轮恰一个 `Claimed`、一个类型化 `Concurrent`）。本检查保持 `Not Started`：Harness 从不调用 Executor——其派发计数是在 `claim_running` 获胜后递增的本地计数器——且生产 Coordinator 仍经进程内权威声明、从未调用持久 `insert_prepared` 或 `claim_running`，因此声明的 Executor 派发计数 1 与 TC-12 跨实例不重复派发要求仍未证明，直到生产派发路径经持久存储接线并由本集成 Harness 演练。 |
| AC-13 | T-3 | 审计元数据完整、关联、有界，且不含凭据或原始无界内容。 | 合成凭据引用、65,536 字节参数边界、1,048,576 字节结果边界和所有终态类别。 | 运行 `cargo test -p koduck-ai --test cand_2_audit audit_is_correlated_and_content_minimized -- --exact`。 | Exit 0；每条终态记录包含声明的 ID/版本/摘要/效果状态/时序/字节数/代码；凭据值和原始参数/结果子串出现 0 次；序列化审计记录最多 16,384 字节。 | Command Output 和脱敏审计 Fixture。 | Not Started | 当前未提交任务修订上的部分记录契约证据：命名命令 Exit 0。`ToolAuditRecord`（`koduck-ai/src/application/audit.rs`）为每个终态类别构造一条关联、内容最小化的记录——每个 `DenialCode` 策略拒绝、每个规范 D-6 决议（accepted/declined/cancelled/expired 及身份、状态、决策和版本）和每个 D-7 执行终态（succeeded/failed/timed_out/cancelled）——只来自自有规范值：该类型没有承载原始动作参数、Executor 结果或凭据值的字段，因此它们无法进入记录（TC-14）。Harness 喂入边界 Fixture——恰 65,536 字节并携带合成 `sk-SYNTHETIC-CREDENTIAL-…` 值的动作参数，以及带独特原始标记的 1,048,576 字节已提交输出——并断言每条记录携带租户/Thread/Turn/描述符与 Profile ID+版本/64 位十六进制动作摘要/租约 Generation/时序/策略决策（加上终态专属的 D-7 状态、效果状态、失败代码和字节数/摘要），凭据、原始参数和原始输出子串出现零次，且 `serialize_audit_record`（强制 [`MAX_AUDIT_RECORD_BYTES`] 的 Adapter 自有 `JSON` 序列化器）对每个类别保持在 16,384 字节以内。带 Fail-closed `NoToolAudits` Sink 的 `ToolAuditSink` Port 约束未来的审计通道 Adapter。本检查保持 `Not Started`：`ToolAuditRecord`、其序列化器和 `ToolAuditSink` 仅被 Fixture 引用——没有任何生产策略、审批或执行终态调用点在 Sink 上调用 `record`——因此 TC-14“每个此类终态发射一条关联记录”的要求未被证明；在生产发射接线并经测试前保持开放（已作为剩余审计后续记录在 T-3）。 |
...
- 当前 T-1/T-2 Source 修订的稳定实施触点：
  ...
  | `koduck-ai/src/adapters/execution/lease.rs` | `SqlxTurnLeaseValidator::check_current` | N/A — 稳定方法已标识持久租约判定 | 门控中断路径每次 D-7 变更的持久 C-6 Generation/Fence/到期校验（TC-07） | 当前未提交任务修订；完成前替换为实施 Commit |
| `koduck-ai/src/application/tool_execution.rs` | `ToolExecutionDriver::execute` | N/A — 稳定方法已标识完整重试序列 | 拥有 authorize、prepare、approve-or-cancel、dispatch 与唯一允许的效果前重试 | 当前未提交任务修订；完成前替换为实施 Commit |
  ...
  | `koduck-ai/src/adapters/execution/attempts.rs` | `SqlxExecutionAttemptStore::commit_terminal`、`bound_lease_is_fenced` | N/A — 稳定符号已标识规范 D-7 终态 Adapter | 生产 `SQLx` pool 上的条件 D-7 终态提交；终态 SQL 原子拒绝已 Fence、Generation 已变更或已到期的现存绑定 C-6 租约 | 当前未提交任务修订；完成前替换为实施 Commit |
...
- 仓库在之前的 Pull Request 修订上已有配置的自动评审证据。ADR-0003
  ...
  review-ready 的 Gate。
- T-3 D-7/审计增量的分解评审（当前未提交任务修订的时间点证据）：`koduck-ai/src/adapters/execution/attempts.rs`
  为 509 物理行，高于 400 行生产文件评审阈值、低于 800 行例外上限。它是一个内聚的 `SQLx` D-7 边界：pool/runtime `wait` 辅助、三条带不可变绑定谓词与失败者重读的条件迁移语句、不可变绑定校验与 `AttemptCommitter` 映射共享同一规范行契约，必须一起变更（TC-12）；把提交分支与声明分支拆开会把一个状态机的条件写入分散到多个属主而没有独立生命周期。`koduck-ai/src/application/attempt_store.rs` 为 274 物理行，`koduck-ai/src/application/audit.rs` 为 260，均低于所有生产阈值；`koduck-ai/src/adapters/audit.rs` 为 17。应用审计模块保留 serde 派生的数据契约，wire 序列化留在 Adapter 之后，保持向内依赖扫描。`koduck-ai/src/runtime/mod.rs` 重测为 636 物理行（低于 800 行例外上限）——第五个有界启动迁移与持久 D-7 存储装配加入顺序启动生命周期，其 116 行聚焦测试模块按既有 `#[path]` 模式移到 crate 内部 `tests/internal/runtime_mod.rs` 同级，使生产装配保持在阈值以下；`tests/internal/runtime_mod.rs` 为 117 物理行；`koduck-ai/src/runtime/tool_executor.rs` 在 `BoundaryToolCallExecutor` 改为对注入提交方泛化并删除退役的进程内提交方后重测为 319。测试文件 `koduck-ai/tests/postgres_cand_2.rs`（385 物理行）、`koduck-ai/tests/internal/cand_2_cancellation_turn_terminal.rs`（204）与 `koduck-ai/tests/cand_2_audit.rs`（253）均低于 600 行测试评审阈值。`koduck-ai/tests/internal/cand_2_postgres_attempts.rs` 重测为 877 物理行，高于 600 行测试评审阈值、低于 1,200 行例外上限——它在获得声明/提交不可变漂移表、prepared 取消效果状态回归与合法性镜像后仍是一个内聚的 D-7 `PostgreSQL` Harness：漂移变体表、Schema 元组探针与竞争 Fixture 共享父 Harness 的 pool/runtime Fixture 与绑定构造器，拆出漂移或取消分支只会复制带种子绑定的 Fixture 而非形成独立测试边界。父模块 `cand_2_postgres.rs` 重测为 888 物理行，D-7 分支是按此前预设的重新评估复用其 Fixture 的真正同级测试边界。圈复杂度为 `N/A — 无已配置复杂度工具`；替代测得信号为逐操作语句跨度与失败者重读路径内最大可执行嵌套深度三，低于嵌套阈值。
...
| 日期 | 变更 | 作者 |
| --- | --- | --- |
| 2026-08-18 | 以测试先行处理剩余 TC-07 终态写入竞争。此前 `SqlxTurnLeaseValidator` 查询可正确接受当前租约，但在独立的 D-7 `commit_terminal` `UPDATE` 前仍可能落下 Fence，从而让终态写入获胜。终态条件 `UPDATE` 现拒绝 Generation 已变更、已 Fence 或已到期的现存匹配 `turn_leases` 行；无获胜者后，存储会在对账前报告类型化 `Fenced`。新的生产 PostgreSQL 回归先准备规范 D-7，再恰在 `commit_terminal` 前 Fence 精确租约，观察到 `Fenced`，并证明该行保持 `prepared`/版本 1（RED：`Won { version: 3 }`；GREEN：类型化 Fence 且零变更）。AC-8/T-2 与取消风险证据更正记录当前 40 项持久 Harness 测试和第三条过期所有者竞争分支。本修订验证已记录。英文已同步。未变更任何已批准决策内容或验收检查定义。 | @codex |
| 2026-08-18 | 以测试先行处理未提交 T-3 增量上的三条中断/AC-8 评审发现。（1）生产中断路径不再绕过真实租约校验：`BoundaryToolCallExecutor::request_interrupt` 此前把始终返回 `Current` 的 `RunnerForegroundLease` 传给 C-5 取消边界，且前置 `interruption_thread` 查询不检查租约——Turn 仍为 `started` 而租约已 Fence 或过期时，准备态 D-7 会在历史写入返回 `Fenced` 之前被持久取消，过期所有者由此修改了规范状态。新的 `SqlxTurnLeaseValidator`（`koduck-ai/src/adapters/execution/lease.rs`）从规范 `turn_leases` 行回答 `LeaseValidator::check_current`——Generation 不一致、已 Fence 或超出检查窗口的到期均为 `Fenced`，行缺失或校验失败为 `Unavailable`——并作为中断租约校验器注入 `BoundaryToolCallExecutor`（生产接线在 `run()`），因此被 Fence 或过期的 Generation 在任何 D-7 变更前以 Reconciliation Fail Closed（TC-07）；前台执行路径保持其文档化的同步窗口校验器不变。（2）AC-8 持久分支的幂等重放期望现在断言 `commit_terminal` 固定写入的终态版本 3（prepared 1、running 2、terminal 3）。（3）AC-8 实际证据、取消风险矩阵行与稳定触点表更正为真实的准备态 D-7 `TurnRunner::request_interrupt` 路径，取代失真的 `claim_running`/路由/`Closed(...)` 声明。修复前修订观察到 RED（两条过期所有者分支都把准备态 D-7 持久取消为版本 3；正向分支以版本 2 期望重放出版本 3），修复后针对一次性生产 PostgreSQL 17 容器转 GREEN（39 项取消 Harness 测试通过，含两条证明零 D-7/Turn 变更的新过期所有者分支）。验证：`cargo fmt --all --check`、严格 all-target/all-feature Clippy、无及有 `KODUCK_AI_TEST_DATABASE_URL` 的完整套件均通过（容器事后删除）。注意：一次性验证数据库需 `PostgreSQL` 16+——Schema 探针绑定带下划线的数字字面量 `1_000`，`PostgreSQL` 14 会拒绝。zh-CN 已同步。未变更任何已批准决策内容或验收检查定义。 | @kimi |
| 2026-08-17 | 处理未提交 T-3 增量上的 AC-8 评审发现：持久集成分支的末段仲裁断言此前运行在内存 `ArbitratingHistory` 替身之上，该替身铸造一个全新的无关 Turn，因此无法证明被中断 D-7 自身的 Turn 恰保持一个持久终态。该分支（`tests/internal/cand_2_cancellation_turn_terminal.rs`）现经生产 `PostgreSQL` 历史（`PostgresTurnHistory` 基于 `SqlxPostgresExecutor`）接受 Turn，把运行中 D-7 绑定到该铸造的 Thread/Turn，记录已认证的 `request_interrupt`（提交唯一的 `Interrupted` Turn 终态），驱动其 D-7 终态跨越真实持久条件存储的路由取消，并断言迟到的 Provider 完成为 `AlreadyTerminal`、规范重放对同一租户/Thread/Turn 恰好返回该终态。AC-8 以更正后的证据保持 `Pass`；取消风险矩阵行与该文件的分解行数同步更正。以 `KODUCK_AI_TEST_DATABASE_URL` 针对一次性生产 PostgreSQL 容器验证（37 项取消 Harness 测试通过，含重构后的分支；容器事后删除）。zh-CN 已同步。未变更任何已批准决策内容或验收检查定义。 | @kimi |
| 2026-08-17 | 以 Test-first 处理未提交 T-3 增量上的四条评审发现：两个 D-7 条件获胜更新现在绑定完整不可变记录（Thread、Turn、租约 Generation、描述符 ID/版本、效果、动作摘要、Profile ID/版本），不再仅以 (tenant, attempt) 为键，且声明的失败方重读在报告规范状态前校验该绑定，因此携带漂移字段重放的尝试身份既不能声明也不能终态化另一个规范 D-7——声明报告类型化 `IdentityConflict`、提交报告类型化 `Conflict`；Migration 0005 的形状 CHECK 现在把每个无 started-at 时间戳的终态行约束为 cancelled/not_started（已接受 D-7 契约允许的唯一 still-prepared 关闭），并由 `DurableAttemptTerminal::legal_from` 与存储按效果状态区分的条件来源镜像；同时更正 AC-12/AC-13 的过度声明——两项检查与并发风险矩阵行回到 `Not Started`，因为命名竞争 Harness 从不调用 Executor（其派发计数是 `claim_running` 获胜后递增的本地计数器）、生产 Coordinator 仍经进程内权威声明而未经持久 `insert_prepared`/`claim_running`，且审计 Sink 仅被 Fixture 引用、没有生产终态调用 `record`，因此 TC-12 派发计数与 TC-14 每终态发射要求保持开放，直到生产接线落地。针对修复前修订观察到 RED（漂移声明赢得 `Claimed{2}`、漂移提交赢得 `Won{3}`、prepared 的 cancelled/started 终态提交成功、Schema 接受不可能的 cancelled/started 元组、`legal_from` 允许该形状）；修复后针对一次性生产 PostgreSQL 转 GREEN：声明/提交的九字段漂移表、prepared 取消存储回归、合法性谓词镜像与 Schema 元组表（现两个合法对照元组、九种非法形状）全部通过。同步补齐此前缺失的 T-3 D-7/审计增量分解评审中文段落并按当前修订重测。验证：`cargo fmt --all --check`、严格 all-target/all-feature Clippy、有无 `KODUCK_AI_TEST_DATABASE_URL` 两种条件下针对一次性容器的完整套件（容器事后删除）以及治理校验均通过。未变更任何已批准决策内容或验收检查定义；仅更正状态与证据，并在已接受的 D-7 契约内完成实现。 | @kimi |
| 2026-08-17 | 未提交 T-3 增量的审查跟进：移除无人使用的 `with_clock` 接缝（提交方直接读取共享生产墙钟，不再保留投机性公开 API），并在 T-3 剩余范围清单中列名持久派发声明接线——`claim_running` 已在存储层证明（AC-12），但 Coordinator 的生产派发路径仍经进程内权威声明，跨实例单次派发保证随该接线落地。同时修复本轮验证暴露的进程内迁移竞争：库测试二进制内并行的环境门控 Harness 经各自独立的初始化器应用生产 DDL，并发的 `CREATE TABLE IF NOT EXISTS` 在 `PostgreSQL` 目录上竞争（`threads` 的 `pg_type` 重复键）；现在每个 Harness 统一经共享的 `tokio::sync::OnceCell` 守卫（`tests/internal/test_migrations.rs`）执行，其 DDL 运行在调用方自身的 runtime 上下文，因此没有 Harness 嵌套 runtime 或跨 runtime 共享 pool（观察到 RED：守卫前带库全套件先后失败于目录竞争与嵌套 runtime panic；修复后连续四次带库全套件通过）。验证：`cargo fmt --all --check`、严格 all-target/all-feature Clippy、完整套件及一次性容器上连续四次带库全套件均通过。未变更任何已批准决策内容或验收检查定义。 | @zcode |
| 2026-08-17 | 以 Test-first 处理 T-3 增量的审查发现：D-7 Schema 元组探针不再空泛（一条合法终态对照元组现在必须被接受，每个元组绑定自身的终态时间与迁移版本，拒绝形状增至七种——含终态早于其派发）；迁移 `0005` 补充对应的 `terminal_at_millis >= started_at_millis` CHECK；`claim_running` 把跨 attempt 部分唯一索引冲突路由进失败者重读，使竞争中的声明报告类型化 `Concurrent` 拒绝而非 `Unavailable`（50 轮双线程压力腿修复前 RED、修复后连续三次 GREEN）；提交路径的不可变校验改用显式可选 prepared-at 期望，不再自行比较；`EffectState` 线路名收敛为一个由存储 Adapter 与审计记录共享的 `as_str`/`from_code` 实现；运行时聚焦测试模块移至 `tests/internal/runtime_mod.rs`，`runtime/mod.rs` 回到 636 物理行。验证：`cargo fmt --all --check`、严格 all-target/all-feature Clippy、无及有 `KODUCK_AI_TEST_DATABASE_URL` 的完整套件（各 320 项通过，一次性容器事后删除）和治理校验均通过。未变更任何已批准决策内容或验收检查定义。 | @zcode |
| 2026-08-17 | 以 Test-first 交付 T-3 的 D-7/审计增量：幂等迁移 `0005_cand_2_execution_attempts.sql`（规范 D-7 表，含形状 CHECK 与单 running/每 Turn 部分唯一索引）、带 `SqlxExecutionAttemptStore` 的 `ExecutionAttemptStore` Port（带类型身份冲突的幂等 prepared 插入、带独立并发拒绝的单获胜者派发声明、失败者解码规范终态的条件终态提交，以及 `AttemptCommitter` 接缝）、把持久提交方接入 `run()` 并以架构回归固定第六个有界启动迁移的运行时组合、观测单一 CAND-1 Turn 终态及其重放的 AC-8 持久集成分支、命名 AC-12 Harness `postgres_cand_2`（每次迁移 32 竞争者、各单获胜者、派发计数 1、幂等重放），以及带边界 Adapter 序列化器与命名 Harness 的 AC-13 审计记录契约。AC-8、AC-12 和 AC-13 为 `Pass`；并发与取消 Risk Coverage Matrix 行为 `Pass`；T-2 为 `Complete`（每个 T-2 验收检查通过）。验证：`cargo fmt --all --check`、严格 `koduck-ai` all-target/all-feature Clippy、无 `KODUCK_AI_TEST_DATABASE_URL` 的完整套件（319 项通过），以及针对一次性生产 PostgreSQL 容器（事后删除）的同一套件加命名 PostgreSQL Harness 均通过。剩余 T-3 范围：AC-14、审计发射接线、安全回收；T-1 仍因 AC-1 的 MCP 入口保持开放。zh-CN 与英文同步。未变更任何已批准决策内容或验收检查定义。 | @zcode |
