...
- `koduck-ai/tests/internal/cand_2_postgres.rs` 为 685 物理行，高于 600 行测试评审阈值、低于 1,200 行例外上限。它是内聚的规范 D-6 `PostgreSQL` Harness：多线程 runtime/pool Fixture、每进程一次的 Migration 防护、经策略授权的审批 Fixture 与字面量非法终态元组表被幂等、单写者、32 路竞争、到期、重放、身份冲突、密封审批人和 Schema 回归用例共享；现在拆分这些用例会复制安全敏感 Fixture，而不是形成独立的测试边界。圈复杂度为 `N/A — 无已配置复杂度工具`；替代评审测得每个测试函数的最大嵌套深度为三，低于嵌套阈值，五至七条字面量语句表以模块级常量形式置于函数体之外。在命名的 AC-12 `postgres_cand_2` 边界与 D-7 派发声明/终态提交分支落地时重新评估该 Harness：那些分支是真正的测试边界，应成为复用本 Fixture 的同级模块，沿用 `cand_2_cancellation*` 同级先例。
- `koduck-ai/src/runtime/mod.rs` 在 C-7 作用域信任边缘落地后测量为 606 物理行——490 行生产代码加一个 116 行的聚焦测试模块——高于 400 行生产文件评审阈值、低于 800 行例外上限。它保持为一个内聚的运行时装配边界：已校验的环境配置、进程唯一的 C-5 权威根装配（`RuntimeState`）、三条自有 v1 Axum 路由及其同步/流式请求管道，以及必须在请求头进入进程之处密封租户、主体与审批作用域的 C-7 网关验证信任边缘（`trust_context` / `gateway_validated_scopes`）。把信任边缘抽入同级模块会把身份密封与拥有请求头解析的唯一入口分离，把配置与伺服拆开则会分割一个启动生命周期而不会形成独立的失败、所有权或生命周期边界。本切片引入的函数均未超过 60 行方法阈值（`run` 与 `handle_stream_request` 先于本切片且未改动）；圈复杂度为 `N/A — 无已配置复杂度工具`，新可执行单元（`trust_context`、`gateway_validated_scopes`）的最大嵌套深度为三，低于嵌套阈值。在 T-2 传输消费者把分发的根句柄接入伺服时重新评估该模块。
