              ...
              alt Earliest non-identical member conflict
                loop Bounded winner-first conflict recovery until the transaction succeeds or budget ends
                  ...
                  alt Winner covers target and fits with exact tail
                    ...
                    else Retry returns typed non-conflict zero-mutation failure
                ...
                loop Bounded request-drift recovery until the reassembled chain commits or the drift budget ends
                  ...
                  alt Reassembly fails or the drift or pass budget ends
                    ...
                    Store-->>Core: Final chain winner plus Turn committed, another drift or earliest conflict, or typed zero-mutation failure
                    alt Retry commits the complete chain plus new Turn
                      Core->>Core: Mark drift recovery complete and exit the drift loop
                    else Retry returns another request drift or earliest conflict winner
                      Core->>Core: Retain the current source or returned winner for the next bounded drift iteration
                    else Retry returns typed non-conflict zero-mutation failure
                ...
                alt Complete chain and new Turn did not commit
                  Core-->>C1: Typed drift, conflict, or durability failure with no D-9 or new Turn
                  C1-->>Client: Resume rejected with no side effect and sequence terminates
                else Complete chain and new Turn are durable
                  Note over Core,Store: Resume may enter active work using the final committed winner while the source terminal Turn remains unchanged
                end
        ...
        alt Reservation fails or client cancellation wins
          ...
          alt Child context is over budget
            loop Each bounded child seed or successor pass until snapshot plus tail fits or pass budget ends
              ...
              else Producer returns cancellation or typed failure
              ...
              alt Earliest non-identical member conflict
                loop Bounded winner-first child conflict recovery until the transaction succeeds or budget ends
                  ...
                  alt Winner covers child target and fits with exact tail
                    ...
                    alt Retry returns another earliest child conflict winner
                      ...
                      Core->>Core: Mark child recovery complete and exit the conflict loop
                    end
                    ...
                    loop Each remaining bounded child successor pass until replacement tip plus exact tail fits or pass budget ends
                      ...
                      External-->>Core: Replacement bounded child suffix member, cancellation, or typed failure
                      alt Producer returns replacement bounded child suffix member
                        Core->>Core: Derive canonical deterministic identity, append child replacement member, and promote it to current predecessor
                      else Producer returns cancellation
                        Core->>Core: Mark child conflict recovery cancelled and exit without a member or another producer call
                      else Producer returns typed failure
                        Core->>Core: Mark child conflict recovery failed and exit without a member or another producer call
                      end
                    end
                    alt Producer cancelled the replacement pass
                      Core->>Core: Mark child conflict recovery cancelled without retrying the atomic target transaction
                    else Producer fails, no next child closed delta exists, or pass budget ends before fit
                ...
                loop Bounded request-drift child recovery until the reassembled child chain commits or the drift budget ends
                  ...
                  alt Reassembly fails or the drift or pass budget ends
                    ...
                    Store-->>Core: Final child-chain winner and child state, another drift or earliest conflict, or typed zero-mutation failure
                    ...
                    else Retry returns another request drift or earliest child conflict winner
                      Core->>Core: Retain the current source or returned child winner for the next bounded drift iteration
                    else Retry returns typed non-conflict zero-mutation failure
...
- [ ] Repository owner and required approver `@linhai` must review the complete producer-failure exit, candidate-selection-gate, drift-reassembly, and child-cancellation corrections for automatic reviews `5085863664`, `5085923290`, and `5086121809` and respond with exact `Approve`; active approval metadata remains pending and Design Status is `Draft`.
