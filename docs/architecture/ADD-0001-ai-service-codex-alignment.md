            ...
            alt Typed request-source drift
              loop Bounded direct-path drift recovery until the direct commit succeeds or the drift budget ends
                ...
                alt Context still fits as direct history
                  ...
                  Core->>Core: Continue through bounded compaction assembly within the same drift budget
                end
              end
            ...
            alt Typed request-source drift
              loop Bounded direct-path child drift recovery until the direct child commit succeeds or the drift budget ends
                ...
                alt Child context still fits as direct history
                  ...
                  Core->>Core: Continue through bounded child compaction assembly within the same drift budget
                end
              end
...
- [ ] Repository owner and required approver `@linhai` must review the complete producer-failure exit, candidate-selection-gate, drift-reassembly, child-cancellation, retry-drift routing, direct-history drift-result, CF-1 durability-exit, CF-3 drift-recovery, Resume-cancellation, and direct-commit drift-retry corrections for automatic reviews `5085863664`, `5085923290`, `5086121809`, `5086269808`, `5086388217`, and `5086580275` and respond with exact `Approve`; active approval metadata remains pending and Design Status is `Draft`.
