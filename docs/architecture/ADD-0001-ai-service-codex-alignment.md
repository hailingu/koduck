...
- **Approval Time [Conditionally Required — Design Status is or has been `Current`]**: 2026-09-02T11:39:30+08:00
...
flowchart TB
  subgraph CF1 ["CF-1 Foreground turn lifecycle"]
    ...
    CF1CommitD9 -->|"No, earliest non-identical member winner"| CF1ChainConflict["No request member committed; discard conflicting member and descendants; read earliest winner"] --> CF1ChainRetry{"Bounded chain-conflict and suffix-regeneration budget remains?"}
  ...
  subgraph CF3 ["CF-3 Resume or fork"]
    ...
    CF3ResumeCommit -->|"No, earliest non-identical member winner"| CF3ChainConflict["No chain member or target state committed; discard conflicting member and descendants; read earliest winner"]
    ...
    CF3ChainConflict --> CF3ChainRetry{"Bounded chain-conflict and suffix-regeneration budget remains?"}
          ...
          alt Direct context or reusable D-9 plus exact tail fits
            ...
            alt Producer fails, no bounded closed delta fits, or pass budget ends
              ...
              alt Earliest non-identical member conflict
                loop Bounded conflicting-suffix regeneration until the complete chain commits or budget ends
                  Core->>Core: Discard conflicting member and descendants, then adopt returned committed winner
                  Core->>External: Regenerate next bounded successor from winner plus contiguous closed delta
        ...
        alt Reservation fails or client cancellation wins
          ...
          alt Child context is over budget
            ...
            alt Producer fails, no bounded closed delta fits, pass budget ends, or cancellation wins
              ...
              alt Earliest non-identical member conflict
                loop Bounded conflicting-suffix regeneration until the complete child chain commits or budget ends
                  Core->>Core: Discard conflicting child member and descendants, then adopt returned committed winner
                  Core->>External: Regenerate next bounded child successor from winner plus contiguous closed delta
...
| RK-12 | Automatic context compaction loses material detail, feeds an unbounded expanded prefix to the producer, reuses a stale prefix summary after a correction, invalidates a valid snapshot merely because the exact tail grows, fails to advance a still-valid active-Turn or Resume snapshot, partially commits a staged chain whose descendant names an uncommitted predecessor, uses a non-identical losing staged summary instead of the committed winner, exposes a Resume snapshot without its new Turn, dispatches after snapshot-winner/prefix/tail drift, splits a Tool round across prefix and tail, skips an active-loop budget check, crosses ownership or fork boundaries, exposes partial Fork state, or lets late producer output launch inference after interrupt/fencing. | Later model behavior may rely on incomplete, stale, foreign, partially committed, or non-durable context; producer inputs may exceed bounds; work may fail as history grows; a request may diverge from durable D-9 state; an orphan Tool result or partial Resume/Fork state may appear; or work may continue after terminal even though canonical history is intact. | CAND-15 atomically validates the selected committed anchor plus every staged predecessor/range/prefix link, commits/selects the complete request chain or none, returns the earliest non-identical conflicting winner, and provides final-winner request fences plus all-or-zero Resume/Fork persistence. CAND-16 provides one bounded producer step accepting either a bounded seed range or immediately preceding bounded snapshot content plus one bounded contiguous closed delta. CAND-14 orchestrates a bounded seed/successor pass budget, discards a conflicting member and descendants, regenerates the bounded suffix from the returned winner, binds only the final committed winner identity/content digest and exact tail into request provenance, reuses D-9 after tail-only append, recursively advances or rebuilds, checks every inference budget, permits only causally closed ranges, rejects post-terminal/stale writes, and fails visibly whenever complete current-source and committed-chain coverage cannot be proven. Resume and Fork atomically bind new visible state to the final chain winner; cancellation or failure exposes no staged-chain member. |
...
- [x] Repository owner and required approver `@linhai` reviewed the complete atomic staged-chain persistence correction for automatic review `5085259961`, then responded with exact `Approve` in the active task at `2026-09-02T11:39:30+08:00`; active approval metadata is complete and Design Status is `Current`.
