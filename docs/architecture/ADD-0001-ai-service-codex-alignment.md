...
- **Approval Time [Conditionally Required — Design Status is or has been `Current`]**: 2026-09-02T01:42:25+08:00
...
| D-9 | Context Compaction Snapshot | Reconstructable provider-context projection summarizing one causally closed canonical history prefix while retaining its source range, digest, policy version, producer identity, token accounting, and recent-tail boundary. A causally closed prefix ends at a durable Item boundary where no provider-visible request/result group, including a Tool call and its matching Tool result, is split between the summarized prefix and exact recent tail; it may end inside an active Turn only at such a boundary before its next inference. | C-2 context-assembly policy; C-6 stores the derived snapshot through a narrow owned port but does not make it canonical history. | Derived tenant/user conversation content; as sensitive as the summarized source. | Built automatically before any Turn-inference request whose effective context exceeds the soft budget, staged until the foreground Turn is revalidated as nonterminal with its current lease generation immediately before commitment and inference, committed idempotently, reused while its provenance remains valid, superseded by a later snapshot, invalidated by incompatible policy/version/lineage or causal-closure changes, discarded when its producer returns after interrupt/fencing/cancellation, and deleted with its owning Thread under the same content-retention policy. |
...
| C-2 | Agent core | Own thread/turn orchestration, state transitions, effective-context assembly, automatic context-compaction policy, budgets, cancellation, foreground lease heartbeats, orphan reconciliation, and provider-independent policy flow. | Owned turn input, instructions, canonical context references, D-9 provenance and causally closed recent-tail boundaries, capabilities, lease-expiry signals; bounded model context, staged and committed derived compaction snapshots, typed items, and terminal outcomes. | C-3, C-4, C-5, C-6, C-7. | Any healthy C-2 instance may reconcile an expired foreground lease, but only through C-6 generation fencing. C-2 checks context before every Turn-inference request; immediately before D-9 commitment and inference it revalidates that the Turn remains nonterminal, its lease generation remains current, and no authenticated interrupt or cancellation has won. Late compaction output is discarded. Compaction may create only a derived target-scoped D-9 projection, must keep each provider-visible request/result group wholly in the prefix or tail, and must fail visibly rather than silently omit uncovered history. Fork compaction follows child identity and lineage reservation and precedes the atomic child-state commit. No provider wire types, Web handlers, database types, or privileged host execution enter the core. |
...
flowchart LR
  ...
  C2 -->|"History and D-9 operations; child-lineage reservation / atomic commit; leases / checkpoints"| C6
  C6 -->|"Canonical operations plus reconstructable D-9 and atomic child state"| Stores
  Stores -->|"Durable history / lineage / D-9, lease expiry, and recovery input"| C6
  C6 -->|"Versioned semantic-memory projections and background-state contracts"| MemoryJobs
  C6 -->|"Durable replay, D-9 provenance, and child reservation / commit result"| C2
...
| --- | --- | --- | --- | --- | --- |
| CF-1 | Client starts or continues a turn / identity and thread access are valid | C-1 normalizes input; C-2 durably creates the started turn and input, acquires and renews its fenced foreground lease through C-6, and resolves capabilities and policy. Before every C-3 Turn-inference request — including after durable provider/tool output continues an active Turn — C-2 assembles effective context and checks the soft token budget. It uses direct effective history when within budget or a provenance-matching D-9 over a causally closed prefix plus exact recent tail when over budget. Immediately before committing any staged D-9 and issuing inference, C-2 revalidates through C-6 that the Turn is still nonterminal, its lease generation is current, and no authenticated interrupt or cancellation has won. It then appends every externally visible item and terminal outcome through C-6 before C-1 publishes it. | Missing, stale, corrupt, incompatible, provenance-mismatched, or causally open D-9 state is rejected and rebuilt from complete bounded canonical ranges at a causally closed boundary; producer output remains staged until the pre-inference fence passes and concurrent builders converge idempotently. Each Tool call/result group remains wholly summarized or wholly exact in the retained tail. Provider retry follows a bounded budget and the same pre-inference fence; no provider or legacy runtime fallback occurs. An authenticated interrupt cancels in-flight compaction when possible and yields `interrupted`; a platform, policy, dependency, or lease-generation stop yields `cancelled`. Any late producer result is ignored. | If initial durability fails, no turn is accepted. If compaction cannot prove complete and causally closed coverage, the producer fails, or no valid snapshot-plus-tail boundary can fit, C-2 issues no subsequent Turn-inference request and durably terminates the already accepted active Turn as `failed`; no context is silently discarded and no orphan Tool result is sent. If interrupt, terminal state, cancellation, or lease fencing wins during compaction, the pre-inference fence discards staged/late output, rejects the D-9 write, issues no inference, and preserves or appends the winning `interrupted` or `cancelled` terminal. A later append failure enters `recovery-pending` and closes `failed` when C-6 returns. Liveness expiry fences the old generation and exactly one reconciler appends `cancelled`. | The client sees rejection with no turn, a direct or causally closed compacted provider context for every fenced inference, the winning durable `interrupted`/`cancelled` terminal with no post-terminal D-9 or inference, a durable typed `failed` compaction terminal without a subsequent inference, a replayable prefix plus `durability-unavailable`, or an eventual durable `cancelled` orphan terminal. No expired or terminal owner can commit compaction, append, or report completion. |
...
| CF-3 | Thread is resumed or forked / caller has access | C-6 loads canonical ordered history and lineage. Resume binds context assembly to the existing Thread. Fork first reserves an unpublished child Thread identity and immutable lineage at the exact fork point, then binds all D-9 selection and construction to that child scope. C-2 uses direct effective history below budget or builds/stages a target-bound D-9 over a causally closed prefix plus exact recent tail above budget. Resume appends a new Turn; Fork atomically commits the child identity, lineage, staged D-9 when present, and first Turn. | Resume may reuse only an existing-Thread provenance and causal-closure match. Fork never reuses a parent-scoped snapshot; it may build a child-bound D-9 only from an authorized causally closed parent prefix ending at or before the fork point. Missing or rejected snapshots are rebuilt only from complete bounded source ranges at a boundary that keeps every Tool call/result group wholly in the prefix or tail; concurrent builders converge idempotently. Incompatible historical Item versions use a versioned adapter. Foreground orphan closure belongs to CF-1; background recovery belongs to CF-5. | Canonical corruption, unavailable source, producer failure, no causally closed boundary that fits, or final atomic-commit failure issues no Turn-inference request. Resume creates no new Turn; Fork aborts the reservation and exposes no child Thread, lineage, D-9, or Turn. No path silently truncates history, emits an orphan Tool result, mutates canonical Items, crosses scope, reuses parent D-9 as child data, or reactivates a terminal Turn. | The original terminal Turn and complete canonical replay remain unchanged. A successful Resume creates one new Turn on the existing Thread. A successful Fork exposes one child Thread with stable lineage and one child-bound direct or causally closed compacted context; every failed Fork has zero visible child state. |
...
flowchart TB
  subgraph CF1 ["CF-1 Foreground turn lifecycle"]
    ...
    CF1Budget -->|"Yes"| CF1PreInference
    CF1Budget -->|"No"| CF1Snapshot{"Valid provenance- and causal-closure-matching D-9 available?"}
    CF1Snapshot -->|"Yes, provenance and causal closure match"| CF1Compact["Reuse or advance D-9; keep each Tool round wholly summarized or wholly exact"]
    ...
    CF1Fits -->|"Yes"| CF1PreInference{"Turn nonterminal, lease generation current, and no interrupt or cancellation?"}
    CF1PreInference -->|"Yes"| CF1D9Pending{"Staged D-9 write pending?"}
    CF1D9Pending -->|"No"| CF1Dispatch{"Generation-bound inference dispatch establishment still permitted?"}
    CF1D9Pending -->|"Yes"| CF1CommitD9{"Conditional D-9 commit under current Turn and lease generation succeeds?"}
    CF1CommitD9 -->|"Yes"| CF1Dispatch
    ...
    CF1PreInference -->|"Authenticated interrupt won"| CF1Interrupt
    CF1PreInference -->|"Existing terminal"| CF1TerminalStop["Discard staged or late compaction output; preserve terminal; no inference"] --> CF1Done
    CF1PreInference -->|"Fenced, platform, policy, or dependency stop"| CF1Cancel
  ...
  subgraph CF3 ["CF-3 Resume or fork"]
    ...
    CF3Access -->|"Yes"| CF3Load["C-6 loads canonical history, lineage, and matching D-9 metadata"]
    ...
    CF3Snapshot -->|"Yes, provenance and causal closure match"| CF3Compact["Reuse or advance snapshot; keep each Tool round wholly summarized or wholly exact"]
    ...
    CF3Commit -->|"Resume"| CF3Resume["Append new Turn on existing Thread; prior terminal Turn stays terminal"]
    CF3Commit -->|"Fork"| CF3ForkCommit{"Atomic child, lineage, staged D-9, and first Turn commit succeeds?"}
...
| Reliability | Explicit lifecycle states, cancellation, timeout, retry and token budgets, idempotency, append-before-publish ordering, provenance- and causality-bound context compaction, pre-inference terminal/lease fencing, fenced foreground liveness leases, checkpoint ownership, and truthful partial/terminal outcomes. | Each externally visible item is durable before publication. Cancellation, timeout, dependency failure, storage failure, approval decline, duplicate input, foreground owner loss, snapshot mismatch, causally open boundary, compaction-producer failure, and late producer completion each have an exact state; lease expiry produces exactly one `cancelled` terminal and rejects stale-owner or post-terminal D-9 writes and inference dispatch; before every Turn-inference request, context compaction either proves complete, causally closed source coverage and passes the generation-bound dispatch fence within budget or prevents that request, durably fails an already accepted active Turn when compaction itself fails, preserves a winning `interrupted`/`cancelled` terminal when lifecycle state changes during compaction, and leaves an unaccepted Resume or Fork with zero visible new state. |
...
| Q-6 | How does a continuing Thread remain usable when its effective provider context exceeds the configured token budget? | @linhai | Resolved | R-1 requires reasoned adoption decisions from the fixed Codex reference baseline, whose E-11 implementation and tests demonstrate token-triggered automatic compaction across continued, resumed, and forked conversations. Repository owner `@linhai` confirmed on 2026-09-01 that CAND-14 is the independently reviewable adjusted-adoption slice for that in-scope Codex capability. C-2 owns a provenance-bearing D-9 derived snapshot plus bounded recent tail; canonical D-1/D-2/D-3 history remains complete and unchanged, and any unavailable, stale, corrupt, or failed compaction path fails visibly instead of silently truncating context. |
...
| RK-8 | Multi-agent expansion multiplies unfinished lifecycle and permission risks. | Concurrency, lineage, budget, and approval semantics become ambiguous. | Defer proactive multi-agent execution until CAND-1 through CAND-5 and CAND-7 through CAND-14 are complete and verified. |
...
| RK-12 | Automatic context compaction loses material detail, reuses a stale summary, splits a Tool call/result round across the summarized prefix and exact tail, skips a budget check during an active tool loop, crosses ownership or fork boundaries, or lets late producer output launch inference after interrupt/fencing. | Later model behavior may rely on incomplete or foreign context, receive an orphan provider Tool result, or continue after the Turn is terminal even though canonical history is intact. | CAND-14 checks the budget before every Turn-inference request, permits only causally closed boundaries that keep every provider-visible request/result group wholly summarized or wholly exact, stages producer output until a terminal/lease revalidation and generation-bound dispatch fence pass, conditionally rejects post-terminal or stale-generation D-9 writes, ignores late producer output, keeps D-9 derived and reconstructable, binds reuse to tenant/subject/Thread/lineage, source range and digest, policy and producer versions, retains an explicit recent tail, records token accounting, rejects mismatches, and fails visibly whenever complete causally closed coverage cannot be proven. Fork reserves child identity and lineage before staging a child-bound D-9 and commits all child state atomically. |
...
| CAND-14 | Before every Turn-inference request in an initial, resumed, forked, or already active tool-loop Turn, effective provider context above its configured soft token budget automatically uses one provenance-bearing derived compaction snapshot over a causally closed prefix plus a bounded recent tail, while complete canonical history remains unchanged and replayable. | Primary implementation boundary: domain and application context-assembly policy in C-2. Includes the per-inference soft-budget trigger; terminal, authenticated-interrupt, cancellation, and lease-generation revalidation immediately before D-9 commitment and generation-bound inference dispatch; staged-output and late-result rejection; active-Turn causally closed durable Item prefixes; D-9 snapshot lifecycle and provenance; bounded canonical range reads; conditional idempotent snapshot commitment through a narrow C-6 port; one owned compaction-producer port implemented by the configured C-3 adapter; recent-tail retention; correction invalidation; child identity/lineage reservation before Fork compaction; atomic child-state commitment; and typed fail-closed outcomes. Supporting C-3/C-6 interfaces remain limited to this C-2 policy. Excludes canonical history deletion or rewriting, semantic Memory ownership, provider fallback, UI, deployment, and changes to the independent per-Turn output budget. One context-compaction policy and its narrow supporting ports form one independently reviewable implementation pull request. | CAND-13 and CAND-7 complete so effective correction-aware provider input, aggregate limits, and immutable fork lineage are authoritative; CAND-1 canonical history and its interruptible, lease-fenced provider-request establishment remain authoritative. | Checks cover below/at/one-over soft token budgets before initial inference, provider retry, and each post-tool inference; initial and rolling active-Turn compaction; exact recent-tail retention; boundaries immediately before a Tool call and after its matching result; rejection of a boundary between that call and result or while any provider-visible request/result group is open; proof that each complete group is wholly summarized or wholly exact; snapshot provenance and reuse; concurrent-builder convergence; stale/corrupt/missing snapshot handling; correction boundaries; producer timeout/failure; tenant/subject isolation; bounded reads and memory; canonical replay equality; and inability to fit any causally closed snapshot-plus-tail boundary. Deterministic races hold the compaction producer open, then make an authenticated interrupt, terminal transition, cancellation, or lease expiry win before producer completion; every late completion is ignored, commits no D-9 under the stale/terminal Turn, establishes no Turn-inference request, and preserves exactly the winning `interrupted` or `cancelled` terminal. A final race covers fencing between revalidation, conditional D-9 commitment, and generation-bound provider-request establishment. Fork checks prove child identity and lineage precede child-bound D-9 selection/build, a parent D-9 is never reused as child state, and failed compaction or atomic commit exposes no child Thread, lineage, D-9, or Turn. A failed compaction path creates no subsequent Turn-inference request, durably fails an already accepted active Turn, creates no new Resume/Fork Turn, never emits an orphan Tool result, and never silently drops uncovered history. | Full | Ready | N/A — Ready; dependency-ordered after CAND-13 and CAND-7 | None |
| CAND-8 | The AI-owned store provides one durable checkpoint and idempotency boundary that can resume one logical background Turn after owner loss without process-local truth or duplicate commitment of non-idempotent effects. | Primary implementation boundary: persistence and data behavior in C-6. Includes checkpoint identity and monotonic state, resume ownership, idempotency records, lease fencing, and bounded recovery state; excludes Multitask transport, semantic Memory, context-compaction behavior, extension-registry snapshots, provider expansion, and deployment. The AI-owned resume state machine and its durable invariants fit one independently reviewable implementation pull request. | CAND-1 and CAND-2 complete. CAND-8 has no correction, fork-lineage, or context-compaction semantic dependency; it is scheduled after CAND-14 only to serialize additive C-6 evolution. | Checks cover monotonic checkpoint advance, same logical job for duplicate idempotency keys, owner-loss resume, stale-owner fencing, concurrent resume single-winner behavior, terminal idempotency, tenant isolation, ambiguous-effect fail-closed handling, and preservation of canonical Thread/Turn/Item history. | Full | Ready | N/A — Ready; planned after CAND-14 for C-6 evolution sequencing but semantically independent of CAND-7 and CAND-14 | None |
...
| AI to model provider | Provider-native HTTP/streaming for turn inference and the owned CAND-14 compaction-producer port | C-3 | Secrets stay at adapter boundary; provider/model selection is explicit; CAND-1 has no fallback; compaction output remains untrusted conversation content; provenance records the selected producer/model and policy version; redaction, token/time/retry limits, and normalized terminals apply. |
...
CAND-13, CAND-7, CAND-14, CAND-8, CAND-4, CAND-9, CAND-10, and CAND-5. CAND-6 remains
...
no correction-semantic dependency. CAND-14 follows CAND-13 and CAND-7 so its
derived snapshot policy operates on correction-aware provider input and stable
fork lineage without transferring canonical ownership. CAND-8 is independent
of CAND-7 and CAND-14 and follows them only for additive C-6 evolution
sequencing; CAND-4 owns its registry
snapshots and is independent of CAND-8; CAND-9 requires CAND-8; and CAND-10
requires CAND-7 and CAND-12 but neither CAND-8, CAND-13, nor CAND-14.
Repository-wide ADR serialization still permits only one unfinished ADR at a
time. CAND-11 through CAND-14 are `Ready` with no ADR and are eligible for
...
| 8 / CAND-14 | Before every initial or active-loop Turn-inference request, a continuing Thread automatically uses one provenance-bearing D-9 snapshot over a causally closed prefix plus its exact recent tail when effective provider context exceeds the soft token budget, without mutating canonical history; staged compaction output cannot cross a terminal/lease fence, and Fork compaction is child-scoped and committed atomically with child state. | Below/at/one-over budgets before initial inference, retry, and post-tool inference; active-Turn rolling compaction; boundaries before Tool call and after matching result; split-round and open-round rejection; each provider-visible request/result group wholly summarized or wholly exact; provenance reuse; concurrent builders; correction invalidation; interrupt/cancel/terminal/lease-expiry races while the producer is blocked and between revalidation, conditional D-9 commit, and generation-bound inference establishment; late-output rejection with no post-terminal D-9 or inference; child identity/lineage before child-bound D-9 construction; parent-snapshot non-reuse; atomic failed-Fork zero state; producer failure; isolation; bounded reads; canonical replay equality; and unable-to-fit outcomes are exact. | Disable new snapshot creation and provider consumption of D-9; retain complete canonical history and delete or rebuild only derived snapshots. If direct effective history cannot fit, fail visibly rather than truncate or emit an orphan Tool result; discard all staged/late compaction output and staged Fork state without exposing a child or crossing a terminal/lease fence. |
| 9 / CAND-8 | The AI-owned store gains durable checkpoint, idempotency, and fenced background-resume state without an external job transport or any dependency on fork lineage or context compaction. | Monotonic checkpoints, duplicate-key convergence, owner-loss resume, stale-owner rejection, single-winner recovery, terminal idempotency, and ambiguous-effect handling have exact outcomes. | Stop resume admission when ownership is ambiguous, preserve canonical state, and use only a verified new schema/artifact pair; no external fallback owns recovery. |
| 10 / CAND-4 | Instructions, profiles, skills, plugins, and MCP use one coherent C-4-owned extension snapshot and cannot widen permissions. | Precedence, provenance, invalid entry, source loss, stale snapshot, cross-tenant isolation, atomic publish, optional durable reconstruction, and permission non-escalation are deterministic. | Disable the registry and restore the static known-safe inventory; keep historical provenance evidence. |
| 11 / CAND-9 | An authenticated background submission crosses the minimal C-1/C-2 handoff and one versioned Multitask adapter that consumes the AI-owned checkpoint/idempotency boundary. | End-to-end submission/status, contract versioning, duplicate submission, lease loss, checkpoint handoff, cancellation, retry safety, terminal convergence, tenant isolation, and dependency loss have exact outcomes. | Disable the Multitask adapter and stop new background submissions; retain AI-owned canonical state for later recovery without route-back to predecessor behavior. |
| 12 / CAND-10 | Semantic Memory consumes versioned effective projections and returns bounded retrieval results without owning canonical history or D-9 context-compaction snapshots. | Projection provenance/versioning, effective correction and fork representation, idempotency, isolation, stale-version rejection, cache reconstruction, retention, and dependency loss have exact outcomes. | Disable projection and retrieval traffic, preserve canonical history, and discard or rebuild only reconstructable Memory projections and caches. |
| 13 / CAND-5 | Foreground/background and only providers already delivered by Accepted ADRs run on the new core and meet first-production-promotion gates; absent another provider candidate, this is the single CAND-1 provider. | Exact approved-provider inventory, owned-contract conformance, stream ordering, correction/provider/context-compaction behavior, background recovery, SLO thresholds, error budgets, and promotion-stop triggers are exact; no undeclared provider or fallback is active. | A failed first-release candidate is not promoted. After first promotion, rollback may target only the last verified new path while preserving the canonical store and owned external contract. |
...
- [x] Repository owner and required approver `@linhai` reviewed the post-compaction terminal/lease fence and late-result race correction, then responded with exact `Approve` in the active task at `2026-09-02T01:42:25+08:00`; active approval metadata is complete and Design Status is `Current`.
