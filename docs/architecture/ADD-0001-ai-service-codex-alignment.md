...
- **Approval Time [Conditionally Required — Design Status is or has been `Current`]**: 2026-09-02T10:15:12+08:00
...
| --- | --- | --- | --- | --- | --- |
| IX-1 | API client with authenticated principal and an existing or new thread | Start, steer, interrupt, resume, fork, or read a thread. | C-1 obtains trust context from C-7. Every lifecycle item is published only after C-6 confirms durability. Resume first loads canonical history and provenance, checks the context budget, and, when over budget, reuses a prefix-valid D-9 or requests bounded compaction before atomically committing any staged D-9 with the new Turn; the source Turn remains terminal. Fork first reserves an unpublished child identity and immutable lineage, evaluates the child-bound direct or compacted context without reusing parent-scoped D-9, and atomically commits the child, lineage, optional staged D-9, and first Turn. Source, producer, closed-boundary, cancellation, drift, or atomic-commit failure rejects Resume with no new D-9 or Turn and aborts Fork with zero visible child state. `Interrupted` reports an authenticated client stop; `cancelled` reports a platform, policy, dependency, or reconciled foreground-owner stop. | A durable terminal Turn, one atomic new Turn plus optional D-9 from Resume, one visible child with lineage and optional child D-9 from Fork, or a rejected/cancelled request with no side effect. Initial durability or pre-Turn Resume compaction failure creates no D-9 or Turn; a cancelled or failed Fork exposes no child identity, lineage, D-9, or Turn; a later outage exposes only the durable prefix plus `durability-unavailable`; an orphan becomes durably `cancelled` after the liveness window. | N/A — service/protocol interaction only; no UI is designed here. |
        ...
        alt Source is unavailable, corrupt, or incomplete
          ...
          alt Context is over budget and no reusable prefix-valid D-9 exists
            ...
            alt Producer fails or no closed boundary fits
              ...
              Core->>Store: Atomically commit staged D-9 and new Turn under request-wide provenance
...
| RK-12 | Automatic context compaction loses material detail, reuses a stale prefix summary after a correction, invalidates a valid snapshot merely because the exact tail grows, fails to advance a still-valid snapshot when its grown tail no longer fits, exposes a Resume snapshot without its new Turn, dispatches a request after prefix or tail drift, splits a Tool call/result round across the summarized prefix and exact tail, skips a budget check during an active tool loop, crosses ownership or fork boundaries, exposes partial Fork state, or lets late producer output launch inference after interrupt/fencing. | Later model behavior may rely on incomplete, stale, or foreign context, receive an orphan provider Tool result, incur unnecessary repeated compaction, fail a continuable active Turn, expose derived state from a rejected Resume or Fork, or continue after the Turn is terminal even though canonical history is intact. | CAND-15 provides distinct atomic prefix-version/digest D-9 fences, request-wide effective-context inference fences, atomic Resume D-9/new-Turn commitment, and all-or-zero Fork persistence; CAND-16 provides one bounded normalized producer adapter; and CAND-14 consumes those completed ports to reuse D-9 after tail-only append, route rolling advancement through the producer when its grown tail no longer fits, rebuild after prefix correction, reassemble after request-wide drift, check the budget before every Turn-inference request, permit only causally closed boundaries, reject post-terminal or stale-generation writes, ignore late output, retain an explicit exact tail and token accounting, and fail visibly whenever complete current-source coverage cannot be proven. Fork reserves child identity and lineage before staging a child-bound D-9 and commits all child state atomically; cancellation or failure exposes none. |
...
- [x] Repository owner and required approver `@linhai` reviewed the complete rolling-snapshot advancement, atomic Resume commit, and explicit Fork interaction correction for automatic review `5084828990`, then responded with exact `Approve` in the active task at `2026-09-02T10:15:12+08:00`; active approval metadata is complete and Design Status is `Current`.
