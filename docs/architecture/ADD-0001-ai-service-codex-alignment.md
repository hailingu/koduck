...
- **Approval Time [Conditionally Required — Design Status is or has been `Current`]**: 2026-09-02T01:29:54+08:00
...
| --- | --- | --- | --- | --- | --- |
| CF-1 | Client starts or continues a turn / identity and thread access are valid | C-1 normalizes input; C-2 durably creates the started turn and input, acquires and renews its fenced foreground lease through C-6, and resolves capabilities and policy. Before every C-3 Turn-inference request — including after durable provider/tool output continues an active Turn — C-2 assembles effective context and checks the soft token budget. It uses direct effective history when within budget or a provenance-matching D-9 over a causally closed prefix plus exact recent tail when over budget, then appends every externally visible item and terminal outcome through C-6 before C-1 publishes it. | Missing, stale, corrupt, incompatible, provenance-mismatched, or causally open D-9 state is rejected and rebuilt from complete bounded canonical ranges at a causally closed boundary; concurrent builders converge idempotently. Each Tool call/result group remains wholly summarized or wholly exact in the retained tail. Provider retry follows a bounded budget for the selected provider; no provider or legacy runtime fallback occurs. An authenticated client interrupt yields `interrupted`; a platform, policy, or dependency stop yields `cancelled`. | If initial durability fails, no turn is accepted. If compaction cannot prove complete and causally closed coverage, the producer fails, or no valid snapshot-plus-tail boundary can fit, C-2 issues no subsequent Turn-inference request and durably terminates the already accepted active Turn as `failed`; no context is silently discarded and no orphan Tool result is sent. A later append failure enters `recovery-pending` and closes `failed` when C-6 returns. Liveness expiry fences the old generation and exactly one reconciler appends `cancelled`. | The client sees rejection with no turn, a direct or causally closed compacted provider context for every inference, a durable typed `failed` compaction terminal without a subsequent inference, a replayable prefix plus `durability-unavailable`, or an eventual durable `cancelled` orphan terminal. No expired owner can append or report completion. |
...
flowchart TB
  subgraph CF1 ["CF-1 Foreground turn lifecycle"]
    ...
    CF1Budget -->|"Yes"| CF1Provider["Issue Turn-inference request through C-3"]
    ...
    CF1Snapshot -->|"No, complete bounded source and closed boundary available"| CF1Build["Build and idempotently commit D-9 from causally closed durable prefix"] --> CF1Compact
...
| Reliability | Explicit lifecycle states, cancellation, timeout, retry and token budgets, idempotency, append-before-publish ordering, provenance- and causality-bound context compaction, fenced foreground liveness leases, checkpoint ownership, and truthful partial/terminal outcomes. | Each externally visible item is durable before publication. Cancellation, timeout, dependency failure, storage failure, approval decline, duplicate input, foreground owner loss, snapshot mismatch, causally open boundary, and compaction-producer failure each have an exact state; lease expiry produces exactly one `cancelled` terminal and rejects stale-owner writes; before every Turn-inference request, context compaction either proves complete, causally closed source coverage within budget or prevents that request, durably fails an already accepted active Turn, and leaves an unaccepted Resume or Fork with zero visible new state. |
...
| 8 / CAND-14 | Before every initial or active-loop Turn-inference request, a continuing Thread automatically uses one provenance-bearing D-9 snapshot over a causally closed prefix plus its exact recent tail when effective provider context exceeds the soft token budget, without mutating canonical history; Fork compaction is child-scoped and committed atomically with child state. | Below/at/one-over budgets before initial inference, retry, and post-tool inference; active-Turn rolling compaction; boundaries before Tool call and after matching result; split-round and open-round rejection; each provider-visible request/result group wholly summarized or wholly exact; provenance reuse; concurrent builders; correction invalidation; child identity/lineage before child-bound D-9 construction; parent-snapshot non-reuse; atomic failed-Fork zero state; producer failure; isolation; bounded reads; canonical replay equality; and unable-to-fit outcomes are exact. | Disable new snapshot creation and provider consumption of D-9; retain complete canonical history and delete or rebuild only derived snapshots. If direct effective history cannot fit, fail visibly rather than truncate or emit an orphan Tool result; discard staged Fork state without exposing a child. |
...
- [x] Repository owner and required approver `@linhai` reviewed the causally closed compaction-boundary rule and architecture-diagram flow correction, then responded with exact `Approve` in the active task at `2026-09-02T01:29:54+08:00`; active approval metadata is complete and Design Status is `Current`.
