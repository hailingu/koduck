...
| C-2 | Agent core | Own thread/turn orchestration, state transitions, effective-context assembly, automatic context-compaction policy, budgets, cancellation, foreground lease heartbeats, orphan reconciliation, and provider-independent policy flow. | Owned turn input, instructions, canonical context references, prefix-scoped D-9 provenance and committed identity/content digest, request-wide effective-context provenance, and causally closed recent-tail boundaries, capabilities, lease-expiry signals; bounded model context, staged and committed derived compaction snapshots, typed items, and terminal outcomes. | C-3, C-4, C-5, C-6, C-7. | Any healthy C-2 instance may reconcile an expired foreground lease, but only through C-6 generation fencing. C-2 checks context before every Turn-inference request. Seed construction consumes one bounded closed canonical range; reconstruction and rolling advancement recursively use the prior bounded snapshot content plus one bounded contiguous closed delta per C-3 call, never the complete expanded prefix, and stop at a configured pass budget. For D-9 commitment C-2 supplies predecessor/absorbed-range and target-prefix provenance. C-6 returns the committed winner; non-identical losing staged output is discarded and C-2 rereads the winner's identity, digest, and content before assembling inference. Final inference establishment separately revalidates request-wide provenance containing the committed winner identity/content digest, prefix provenance, and exact tail, or a direct-history marker, while also checking that the Turn remains nonterminal, its lease generation remains current, and no authenticated interrupt or cancellation has won. Tail-only drift reassembles the request while retaining D-9 when its prefix still matches; when the valid snapshot plus exact tail no longer fits, C-2 performs bounded successor steps; prefix drift rejects D-9 and performs bounded seed-and-successor reconstruction. All drift recovery is bounded, and late or losing compaction output is discarded. Compaction may create only a derived target-scoped D-9 projection, must keep each provider-visible request/result group wholly in the prefix or tail, and must fail visibly rather than silently omit uncovered history. Resume compaction atomically commits or selects the winner D-9 with the new Turn and binds that Turn to the winner; Fork compaction follows child identity and lineage reservation and precedes the atomic child-state commit. No provider wire types, Web handlers, database types, or privileged host execution enter the core. |
...
| C-6 | Thread-store port and AI-owned durable adapter | Persist and retrieve canonical history, metadata, lineage, foreground liveness leases, checkpoints, idempotency state, and reconstructable D-9 compaction snapshots in a shared durable store owned by the AI service boundary. | Versioned Thread/turn/item appends and bounded range queries, target-scoped derived snapshot reads and predecessor-/prefix-conditional writes, committed-winner identity/content reads, generation- and request-wide-source/snapshot-bound inference-establishment checks, atomic Resume staged-D-9 winner selection/new-Turn commit, Fork child-identity/lineage reservation and atomic child-state commit, lease acquire/renew/expire operations; ordered history, prefix-scoped and request-wide effective-history versions/digests, D-9 identity/content/provenance, metadata, and fenced lease generations. | AI-owned PostgreSQL datastore; later semantic Memory and background Multitask adapters consume owned projections or commands. | Lease expiry and orphan terminal transition are conditional and idempotent by turn plus generation. A stale generation or terminal Turn rejects foreground D-9 commitment and inference establishment; changed predecessor, absorbed range, or prefix provenance rejects D-9 commitment, while any changed request-wide effective-context provenance or selected committed snapshot identity/content digest rejects inference establishment. For the same target-prefix key, identical content converges idempotently; non-identical output loses to one committed snapshot and returns that winner descriptor so C-2 must discard the loser and reread the winner content. Tail-only appends do not invalidate a prefix-matching D-9. A failed Resume compaction or final commit exposes neither its staged D-9 nor a new Turn; a successful atomic Resume binds the Turn to the selected committed winner rather than a losing staged summary. A cancelled or failed Fork compaction or final commit exposes no child identity, lineage, D-9, or Turn. The AI-owned store is canonical for Thread/Turn/Item; D-9 and process-local state are reconstructable only and never authorize canonical-history mutation. |
...
flowchart LR
  ...
  C2 -->|"Predecessor/range/prefix D-9 operations; winner-bound inference; atomic Resume and Fork state; leases / checkpoints"| C6
  C6 -->|"Canonical operations plus committed D-9 winner identity/content and request-fenced atomic Resume / Fork state"| Stores
  Stores -->|"Durable source versions/digests, committed D-9 identity/content/provenance, lineage / Turn state, lease expiry, and recovery input"| C6
  C6 -->|"Versioned semantic-memory projections and background-state contracts"| MemoryJobs
  C6 -->|"Durable replay, committed D-9 winner identity/content, source provenance, inference fence, and child commit result"| C2
...
| CF-3 | Thread is resumed or forked / caller has access | C-6 loads canonical ordered history, committed D-9 identity/content/prefix provenance when present, request-wide provenance, and lineage. Resume binds context assembly to the existing Thread. Fork first reserves an unpublished child Thread identity and immutable lineage at the exact fork point, then binds all D-9 selection and construction to that child scope. C-2 uses direct effective history below budget, a fitting committed target-bound D-9 plus exact recent tail, or a bounded recursive seed/successor chain above budget. Each seed consumes one bounded closed range; each successor consumes only its immediately preceding bounded snapshot content plus one bounded contiguous closed delta. Resume atomically compares predecessor/prefix and request-wide provenance, converges any staged D-9 to the committed winner, and commits the new Turn bound to that winner, or commits neither. Fork atomically compares that provenance while committing the child identity, lineage, selected committed child D-9 when present, and first Turn. | Resume may reuse only an existing-Thread prefix-provenance and causal-closure match. A tail-only append retains that committed snapshot and triggers exact-tail reassembly with its identity/content digest; if it no longer fits, bounded successor steps absorb later ranges. A prefix correction rejects it and starts bounded recursive reconstruction. Concurrent identical results converge idempotently; a non-identical losing builder discards its output and rereads the committed winner before assembly or atomic Turn creation. Fork never reuses a parent-scoped snapshot; it may seed and recursively advance a child-bound D-9 only from authorized contiguous closed parent ranges ending at or before the fork point. Request-wide drift after assembly retries with the same committed D-9 only while its identity/content/prefix remains selected and fits; otherwise C-2 rereads the winner or advances within bounded budgets. Incompatible historical Item versions use a versioned adapter. Foreground orphan closure belongs to CF-1; background recovery belongs to CF-5. | Canonical corruption, unavailable or changed request source, seed/successor producer failure or cancellation, no bounded causally closed next range, pass-budget exhaustion, committed-winner read failure, or final atomic-commit failure issues no Turn-inference request. Resume exposes neither a staged D-9 nor a new Turn; Fork aborts the reservation and exposes no child Thread, lineage, D-9, or Turn. No path silently truncates history, emits an orphan Tool result, mutates canonical Items, crosses scope, reuses parent D-9 as child data, commits D-9 against stale predecessor/prefix provenance, uses a losing staged summary, commits a Turn against stale winner/request provenance, or reactivates a terminal Turn. | The original terminal Turn and complete canonical replay remain unchanged. A successful Resume atomically exposes one new Turn bound to the exact committed D-9 winner when compaction was required. A successful Fork exposes one child Thread with stable lineage and one child-bound direct or committed compacted context; every cancelled or failed Fork has zero visible child state. |
...
flowchart TB
  subgraph CF1 ["CF-1 Foreground turn lifecycle"]
    ...
    CF1BuildResult -->|"Yes"| CF1Built["Stage seed or successor D-9 with predecessor and absorbed-range provenance"] --> CF1BuiltFits{"Staged D-9 plus exact recent tail fits budget?"}
    ...
    CF1CommitD9 -->|"Yes or existing winner"| CF1Winner["C-6 returns committed winner identity and content digest"] --> CF1WinnerRead["Discard non-identical loser and read committed winner content"] --> CF1RequestFence
    ...
    CF1Dispatch -->|"No, winner, prefix, or tail changed"| CF1RequestDrift["Reread winner and exact tail; retain D-9 only if identity, digest, and prefix still match"]
  ...
  subgraph CF3 ["CF-3 Resume or fork"]
    ...
    CF3BuildResult -->|"Yes"| CF3Built["Stage seed or successor D-9 with predecessor and absorbed-range provenance"] --> CF3BuiltFits{"Staged D-9 plus exact recent tail fits budget?"}
...
| --- | --- | --- | --- | --- | --- |
| IX-1 | API client with authenticated principal and an existing or new thread | Start, steer, interrupt, resume, fork, or read a thread. | C-1 obtains trust context from C-7. Every lifecycle item is published only after C-6 confirms durability. Resume first loads canonical history and any committed D-9 winner, checks the context budget, and uses direct history or that winner only when it plus the exact tail fits. Otherwise C-2 runs bounded recursive production: a seed consumes one bounded closed range; each successor consumes the immediately preceding bounded snapshot content plus one bounded contiguous closed delta, never the complete expanded prefix. Before creating the new Turn, C-6 fences predecessor/range/prefix/request provenance, converges concurrent output to one committed winner, and binds the Turn to that winner identity/content digest; non-identical losing output is discarded. Fork applies the same bounded child-scoped scheme after reserving unpublished child identity and immutable lineage, never reusing parent-scoped D-9. Source, producer, closed-range, pass-budget, committed-winner read, cancellation, drift, or atomic-commit failure rejects Resume with no new D-9 or Turn and aborts Fork with zero visible child state. `Interrupted` reports an authenticated client stop; `cancelled` reports a platform, policy, dependency, or reconciled foreground-owner stop. | A durable terminal Turn, one atomic new Turn bound to the selected committed D-9 winner when Resume compaction is needed, one visible child bound to its selected child D-9 winner when Fork compaction is needed, or a rejected/cancelled request with no side effect. Initial durability, recursive producer/pass, winner selection/read, or atomic-commit failure creates no D-9/Turn for Resume and exposes no child identity, lineage, D-9, or Turn for Fork; a later outage exposes only the durable prefix plus `durability-unavailable`; an orphan becomes durably `cancelled` after the liveness window. | N/A — service/protocol interaction only; no UI is designed here. |
        ...
        alt Source is unavailable, corrupt, or incomplete
          ...
          alt Direct context or reusable D-9 plus exact tail fits
            ...
            loop Each bounded seed or successor pass until snapshot plus tail fits or pass budget ends
              ...
              Core->>Core: Stage successor with predecessor and absorbed-range provenance
            end
            alt Producer fails, no bounded closed delta fits, or pass budget ends
              ...
              C1-->>Client: Resume rejected with no side effect and sequence terminates
            else Staged seed or successor context is complete
              Core->>Store: Atomically fence predecessor, range, prefix, and request provenance, select committed winner, and append new Turn bound to winner
              Store-->>Core: Winner identity/content plus Turn committed, or typed zero-mutation failure
              alt Winner or source drifted or atomic commit failed
                Core-->>C1: Typed source or durability failure with no D-9 or new Turn
                C1-->>Client: Resume rejected with no side effect and sequence terminates
              else Committed winner and new Turn are durable
                Core->>Core: Discard any non-identical staged loser and use committed winner content
                Note over Core,Store: Resume may enter active work and source terminal Turn remains unchanged
              end
            end
          end
        ...
        alt Reservation fails or client cancellation wins
          ...
          alt Child context is over budget
            loop Each bounded child seed or successor pass until snapshot plus tail fits or pass budget ends
              ...
              Core->>Core: Stage child successor with predecessor and absorbed-range provenance
            end
            ...
            else Child-bound staged context is complete
              Core->>Store: Atomically fence source, select committed child D-9 winner, and bind child, lineage, and first Turn to winner
              Store-->>Core: Winner and all child state committed or typed zero-mutation failure
              alt Winner or request source drifted or atomic commit failed
                Core->>Store: Abort child reservation and staged D-9
                Store-->>Core: Zero visible child state acknowledged
...
| Reliability | Explicit lifecycle states, cancellation, timeout, per-step/pass/retry/token budgets, idempotency, append-before-publish ordering, predecessor/range/prefix/winner/request-bound context compaction, pre-inference source/snapshot/terminal/lease fencing, fenced foreground liveness leases, checkpoint ownership, and truthful partial/terminal outcomes. | Each externally visible item is durable before publication. Cancellation, timeout, dependency/storage failure, approval decline, duplicate input, owner loss, predecessor/snapshot mismatch, correction/source drift, causally open range, seed/successor producer failure, non-identical losing output, and late completion each have an exact state. Every producer call is bounded to a seed range or predecessor snapshot plus one contiguous closed delta; pass exhaustion fails visibly. C-6 selects one committed winner, and inference assembly rereads and binds that winner identity/content digest plus exact tail before the generation-bound source/snapshot/terminal/lease fence. Tail appends reuse a valid winner, grown tails use successor steps, prefix corrections use bounded recursive rebuild, and winner/prefix/tail drift reassembles. Resume and Fork atomically bind visible state to the committed winner; a failed active Turn closes durably, while failed Resume/Fork exposes zero new state. Lease expiry produces exactly one `cancelled` terminal and rejects stale-owner writes/inference. |
...
| RK-12 | Automatic context compaction loses material detail, feeds an unbounded expanded prefix to the producer, reuses a stale prefix summary after a correction, invalidates a valid snapshot merely because the exact tail grows, fails to advance a still-valid active-Turn or Resume snapshot, uses a non-identical losing staged summary instead of the committed winner, exposes a Resume snapshot without its new Turn, dispatches after snapshot-winner/prefix/tail drift, splits a Tool round across prefix and tail, skips an active-loop budget check, crosses ownership or fork boundaries, exposes partial Fork state, or lets late producer output launch inference after interrupt/fencing. | Later model behavior may rely on incomplete, stale, foreign, or non-durable context; producer inputs may exceed bounds; work may fail as history grows; a request may diverge from durable D-9 state; an orphan Tool result or partial Resume/Fork state may appear; or work may continue after terminal even though canonical history is intact. | CAND-15 provides predecessor/range/prefix D-9 fences, deterministic committed-winner selection, winner identity/content reads, winner-bound request fences, atomic Resume winner/new-Turn commitment, and all-or-zero Fork persistence. CAND-16 provides one bounded producer step accepting either a bounded seed range or immediately preceding bounded snapshot content plus one bounded contiguous closed delta. CAND-14 orchestrates a bounded seed/successor pass budget, discards non-identical losers and rereads the committed winner before assembly, binds winner identity/content digest and exact tail into request provenance, reuses D-9 after tail-only append, recursively advances or rebuilds, checks every inference budget, permits only causally closed ranges, rejects post-terminal/stale writes, and fails visibly whenever complete current-source and committed-winner coverage cannot be proven. Resume and Fork atomically bind new visible state to the committed winner; cancellation or failure exposes none. |
...
| CAND-15 | The AI-owned store exposes one reconstructable, target-scoped D-9 persistence contract whose bounded reads and conditional operations fence predecessor/range/prefix provenance, select one committed snapshot winner, and bind request-wide inference plus atomic Resume/Fork state to that winner identity/content digest. | Primary implementation boundary: persistence and data behavior in C-6. Includes bounded versioned source/delta reads; stable D-9 identity/content digest and predecessor/absorbed-range metadata; target-prefix keyed reads and conditional writes; identical-content idempotency and deterministic non-identical committed-winner selection; winner descriptor/content reads; atomic comparison of predecessor, range, and prefix provenance; separate generation- and request-wide-source/snapshot-bound inference establishment; atomic Resume winner selection plus new-Turn binding; child identity/lineage reservation; atomic Fork winner selection plus child, lineage, and first-Turn binding; reconstructable deletion; and typed conflict results. Excludes soft-budget/pass policy, provider calls, summary generation, canonical-history rewriting, routes, UI, deployment, Memory, and Multitask. One durable transaction and storage-port boundary forms one independently reviewable implementation pull request. | CAND-13 and CAND-7 complete so correction-aware source identity, aggregate limits, and immutable Fork lineage are authoritative; CAND-11 correction admission remains the source-changing transaction whose affected range determines whether only request assembly or also D-9 provenance is stale. | Checks cover bounded seed/delta reads; predecessor identity/content digest and absorbed-range equality; target-prefix and request-wide source versions/digests; tail-only append preserving a committed snapshot while invalidating request assembly; prefix correction invalidation; correction or append between D-9 commit and inference establishment; identical-write convergence; concurrent non-identical builders selecting one winner and returning its descriptor/content; rejection of inference assembled from losing output or stale winner identity/digest; terminal/lease races; tenant/subject/Thread/lineage isolation; reconstructable deletion; atomic Resume Turn binding to the committed winner with all-or-zero state; parent-snapshot rejection; and atomic all-or-zero Fork winner/child state after success, cancellation, producer failure, source/winner drift, or commit failure. | Full | Ready | N/A — Ready; dependency-ordered after CAND-13 and CAND-7 | None |
...
| CAND-14 | Before every Turn-inference request in an initial, resumed, forked, or already active tool-loop Turn, C-2 automatically assembles context within the configured soft token budget from exact effective history or one committed D-9 winner plus a causally closed bounded recent tail, while complete canonical history remains unchanged and replayable. | Primary implementation boundary: domain and application context-assembly policy in C-2. Includes the per-inference soft-budget trigger; bounded seed-and-successor pass orchestration using predecessor snapshot content plus one bounded contiguous closed delta; pass/retry budgets; predecessor/range/prefix provenance; committed-winner selection and reread; request-wide provenance binding the winner identity/content digest or direct marker plus exact tail; D-9 reuse/recursive reconstruction/rolling advancement; terminal, interrupt, cancellation, lease-generation, winner, prefix, and request-wide revalidation; staged/losing/late-result rejection; atomic winner-bound Resume and Fork policy; and typed fail-closed outcomes. Excludes C-3 adapter implementation, C-6 persistence implementation, canonical-history deletion or rewriting, semantic Memory ownership, provider fallback, UI, deployment, and changes to the independent per-Turn output budget. One C-2 context-assembly policy forms one independently reviewable implementation pull request. | CAND-15 and CAND-16 complete; therefore CAND-13 correction-aware provider input, CAND-7 immutable fork lineage, CAND-11 source-changing correction admission, and CAND-1 interruptible lease-fenced inference establishment are available through their owned contracts. | Checks cover below/at/one-over soft budgets before initial inference, provider retry, and post-tool inference; seed-only, multi-successor initial/rebuild, active-Turn, Resume, and Fork compaction where the complete prefix exceeds producer input limits but every predecessor-plus-delta step remains bounded; exact-tail retention and Tool-round causal boundaries; tail append reuse; prefix correction recursive rebuild; winner/prefix/tail drift; one-under/at/one-over step and pass budgets; no complete expanded-prefix producer request; concurrent identical builders; concurrent non-identical builders where only the committed winner identity/content is reread, included in request provenance, and dispatched; explicit rejection of losing staged content; blocked producer and commitment/inference races; stale/corrupt/missing snapshot; timeout/failure; isolation; bounded memory; canonical replay equality; and unable-to-fit outcomes. Resume checks cover fitting reuse and multi-step valid-but-insufficient advancement, proving the Turn binds atomically to the committed winner or neither appears. Fork checks prove child reservation precedes child-bound recursive compaction and cancellation/failure/winner drift exposes no child state. Every late or losing completion is ignored and no stale inference is established. | Full | Ready | N/A — Ready; dependency-ordered after CAND-15 and CAND-16 | None |
...
| 8 / CAND-15 | C-6 provides bounded source/delta reads, predecessor/range/prefix-fenced D-9 storage, deterministic committed-winner selection and reads, winner-bound inference establishment, atomic Resume winner/new-Turn state, and atomic Fork winner/child state without owning compaction policy. | Predecessor identity/digest, absorbed-range, target-prefix, and request-wide provenance equality; tail append and correction races; identical convergence; non-identical single-winner selection; rejection of losing/stale-winner inference; terminal/lease fencing; isolation; reconstructable deletion; all-or-zero winner-bound Resume and Fork state have exact transactional outcomes. | Disable D-9 writes and winner-bound inference establishment while retaining canonical history; remove or rebuild only reconstructable snapshots. |
...
| 10 / CAND-14 | C-2 checks every inference budget and orchestrates direct history or one committed D-9 winner plus exact tail through bounded seed/successor CAND-16 steps and CAND-15 winner/fence operations; staged, losing, or late output cannot cross winner, prefix, request-wide, terminal, or lease fences. | Multi-pass initial/rebuild/active-Turn/Resume/Fork cases whose complete prefix exceeds one producer call; per-step/pass boundaries; predecessor-plus-delta causality; tail reuse; prefix rebuild; committed-winner reread; non-identical concurrent output; winner-bound request provenance; rejection of losing summaries; atomic winner-bound Resume/Fork state; drift/race/failure/isolation/bounded-memory/canonical-replay/unable-to-fit outcomes are exact. | Disable C-2 snapshot creation and consumption; retain complete canonical history and only derived D-9 state. If direct effective history cannot fit, fail visibly rather than truncate, use a losing snapshot, or emit an orphan Tool result. |
