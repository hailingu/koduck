...
- **Approval Time [Conditionally Required — Design Status is or has been `Current`]**: 2026-09-02T09:14:00+08:00
...
| D-9 | Context Compaction Snapshot | Reconstructable provider-context projection summarizing one causally closed canonical history prefix while retaining its source range, effective-history version and digest, policy version, producer identity, token accounting, and recent-tail boundary. A causally closed prefix ends at a durable Item boundary where no provider-visible request/result group, including a Tool call and its matching Tool result, is split between the summarized prefix and exact recent tail; it may end inside an active Turn only at such a boundary before its next inference. | C-2 context-assembly policy; C-6 stores the derived snapshot through a narrow owned port but does not make it canonical history. | Derived tenant/user conversation content; as sensitive as the summarized source. | Built automatically before any Turn-inference request whose effective context exceeds the soft budget, staged until the foreground Turn and exact effective-history source are atomically revalidated immediately before commitment and inference, committed idempotently, reused while its provenance remains valid, superseded by a later snapshot, invalidated by an admitted correction or other incompatible source-version, digest, policy, lineage, or causal-closure change, discarded when its producer returns after source drift, interrupt, fencing, or cancellation, and deleted with its owning Thread under the same content-retention policy. |
...
| C-2 | Agent core | Own thread/turn orchestration, state transitions, effective-context assembly, automatic context-compaction policy, budgets, cancellation, foreground lease heartbeats, orphan reconciliation, and provider-independent policy flow. | Owned turn input, instructions, canonical context references, D-9 provenance and causally closed recent-tail boundaries, capabilities, lease-expiry signals; bounded model context, staged and committed derived compaction snapshots, typed items, and terminal outcomes. | C-3, C-4, C-5, C-6, C-7. | Any healthy C-2 instance may reconcile an expired foreground lease, but only through C-6 generation fencing. C-2 checks context before every Turn-inference request; immediately before D-9 commitment and inference it supplies the exact expected effective-history version and digest and revalidates that the source still matches, the Turn remains nonterminal, its lease generation remains current, and no authenticated interrupt or cancellation has won. Source drift or late compaction output is discarded and rebuilt only within a bounded retry budget. Compaction may create only a derived target-scoped D-9 projection, must keep each provider-visible request/result group wholly in the prefix or tail, and must fail visibly rather than silently omit uncovered history. Fork compaction follows child identity and lineage reservation and precedes the atomic child-state commit. No provider wire types, Web handlers, database types, or privileged host execution enter the core. |
...
flowchart LR
  ...
  C2 -->|"Versioned source and D-9 operations; source-bound inference establishment; child reservation / atomic commit; leases / checkpoints"| C6
  C6 -->|"Canonical operations plus source-fenced reconstructable D-9 and atomic child state"| Stores
  Stores -->|"Durable history versions / digests, lineage / D-9, lease expiry, and recovery input"| C6
  C6 -->|"Versioned semantic-memory projections and background-state contracts"| MemoryJobs
  C6 -->|"Durable replay, source version / digest, D-9 provenance, inference fence, and child commit result"| C2
...
| --- | --- | --- | --- | --- | --- |
| CF-1 | Client starts or continues a turn / identity and thread access are valid | C-1 normalizes input; C-2 durably creates the started turn and input, acquires and renews its fenced foreground lease through C-6, and resolves capabilities and policy. Before every C-3 Turn-inference request — including after durable provider/tool output continues an active Turn — C-2 assembles effective context with an exact source version/digest and checks the soft token budget. It uses direct effective history when within budget or a provenance-matching D-9 over a causally closed prefix plus exact recent tail when over budget. Immediately before committing any staged D-9 and issuing inference, C-2 revalidates through C-6 that the same effective-history version/digest remains current, the Turn is still nonterminal, its lease generation is current, and no authenticated interrupt or cancellation has won. It then appends every externally visible item and terminal outcome through C-6 before C-1 publishes it. | Missing, stale, corrupt, incompatible, provenance-mismatched, source-drifted, or causally open D-9 state is rejected and rebuilt from complete bounded canonical ranges at a causally closed boundary; producer output remains staged until the pre-inference fence passes and concurrent builders converge idempotently. An admitted correction or any other effective-history change during production or between conditional D-9 commitment and dispatch invalidates the staged/committed candidate for this request and restarts assembly only within a bounded source-drift retry budget. Each Tool call/result group remains wholly summarized or wholly exact in the retained tail. Provider retry follows a bounded budget and the same pre-inference fence; no provider or legacy runtime fallback occurs. An authenticated interrupt cancels in-flight compaction when possible and yields `interrupted`; a platform, policy, dependency, or lease-generation stop yields `cancelled`. Any late producer result is ignored. | If initial durability fails, no turn is accepted. If compaction cannot prove complete and causally closed coverage, the producer fails, no valid snapshot-plus-tail boundary can fit, or source drift exhausts its bounded retry budget, C-2 issues no subsequent Turn-inference request and durably terminates the already accepted active Turn as `failed`; no context is silently discarded and no orphan Tool result is sent. If interrupt, terminal state, cancellation, lease fencing, or source drift wins during compaction, the pre-inference fence discards staged/late output, rejects a stale D-9 write and inference establishment, and either rebuilds from the new source or preserves the winning durable terminal. A later append failure enters `recovery-pending` and closes `failed` when C-6 returns. Liveness expiry fences the old generation and exactly one reconciler appends `cancelled`. | The client sees rejection with no turn, a direct or causally closed compacted provider context for the exact source version/digest of every fenced inference, the winning durable terminal with no post-terminal or stale-source D-9/inference, a durable typed `failed` compaction terminal without a subsequent inference, a replayable prefix plus `durability-unavailable`, or an eventual durable `cancelled` orphan terminal. No expired, terminal, or source-stale owner can commit compaction, append, or report completion. |
...
| CF-3 | Thread is resumed or forked / caller has access | C-6 loads canonical ordered history, its effective-history version/digest, and lineage. Resume binds context assembly to the existing Thread. Fork first reserves an unpublished child Thread identity and immutable lineage at the exact fork point, then binds all D-9 selection and construction to that child scope. C-2 uses direct effective history below budget or builds/stages a target-bound D-9 over a causally closed prefix plus exact recent tail above budget. Resume appends a new Turn only if the expected source version/digest remains current; Fork atomically compares that source provenance while committing the child identity, lineage, staged D-9 when present, and first Turn. | Resume may reuse only an existing-Thread provenance and causal-closure match. Fork never reuses a parent-scoped snapshot; it may build a child-bound D-9 only from an authorized causally closed parent prefix ending at or before the fork point. Missing, rejected, or source-drifted snapshots are rebuilt only from complete bounded source ranges at a boundary that keeps every Tool call/result group wholly in the prefix or tail; concurrent builders converge idempotently. Incompatible historical Item versions use a versioned adapter. Foreground orphan closure belongs to CF-1; background recovery belongs to CF-5. | Canonical corruption, unavailable or changed source, producer failure, no causally closed boundary that fits, or final atomic-commit failure issues no Turn-inference request. Resume creates no new Turn; Fork aborts the reservation and exposes no child Thread, lineage, D-9, or Turn. No path silently truncates history, emits an orphan Tool result, mutates canonical Items, crosses scope, reuses parent D-9 as child data, commits against stale source provenance, or reactivates a terminal Turn. | The original terminal Turn and complete canonical replay remain unchanged. A successful Resume creates one new Turn on the existing Thread from the exact validated source. A successful Fork exposes one child Thread with stable lineage and one child-bound direct or causally closed compacted context; every failed Fork has zero visible child state. |
...
flowchart TB
  subgraph CF1 ["CF-1 Foreground turn lifecycle"]
    ...
    CF1Fits -->|"Yes"| CF1PreInference{"Effective-history version/digest current, Turn nonterminal, lease current, and no interrupt or cancellation?"}
    CF1PreInference -->|"Yes"| CF1D9Pending{"Staged D-9 write pending?"}
    CF1D9Pending -->|"No"| CF1Dispatch{"Generation-bound inference establishment with same source version/digest permitted?"}
    CF1D9Pending -->|"Yes"| CF1CommitD9{"Conditional D-9 commit under current Turn, lease, and source version/digest succeeds?"}
    CF1CommitD9 -->|"Yes"| CF1Dispatch
    CF1CommitD9 -->|"No, terminal or fence won"| CF1FenceLost{"Winning durable state"}
    CF1CommitD9 -->|"No, source changed"| CF1SourceDrift
    CF1Dispatch -->|"Yes"| CF1Provider["Issue Turn-inference request through C-3"]
    CF1Dispatch -->|"No, terminal or fence won"| CF1FenceLost
    CF1Dispatch -->|"No, source changed"| CF1SourceDrift["Discard staged output and reject stale D-9 for this request"]
    CF1SourceDrift --> CF1DriftBudget{"Bounded source-drift retry remains?"}
    ...
    CF1PreInference -->|"Source changed"| CF1SourceDrift
  ...
  subgraph CF3 ["CF-3 Resume or fork"]
    ...
    CF3Access -->|"Yes"| CF3Load["C-6 loads canonical history, effective-history version/digest, lineage, and matching D-9 metadata"]
    ...
    CF3Commit -->|"Resume"| CF3ResumeCommit{"Source-bound new-Turn append succeeds?"}
    CF3ResumeCommit -->|"No, source changed"| CF3Fail
    CF3ResumeCommit -->|"Yes"| CF3Resume["Append new Turn on existing Thread; prior terminal Turn stays terminal"]
    CF3Commit -->|"Fork"| CF3ForkCommit{"Atomic source check, child, lineage, staged D-9, and first Turn commit succeeds?"}
...
| Reliability | Explicit lifecycle states, cancellation, timeout, retry and token budgets, idempotency, append-before-publish ordering, source-version-, provenance-, and causality-bound context compaction, pre-inference source/terminal/lease fencing, fenced foreground liveness leases, checkpoint ownership, and truthful partial/terminal outcomes. | Each externally visible item is durable before publication. Cancellation, timeout, dependency failure, storage failure, approval decline, duplicate input, foreground owner loss, snapshot mismatch, admitted correction or other source drift, causally open boundary, compaction-producer failure, and late producer completion each have an exact state; lease expiry produces exactly one `cancelled` terminal and rejects stale-owner or post-terminal D-9 writes and inference dispatch; before every Turn-inference request, context compaction either proves complete, causally closed coverage for the exact current effective-history version/digest and passes the generation-bound source/terminal/lease dispatch fence within budget or prevents that request, durably fails an already accepted active Turn when compaction or bounded source-drift recovery fails, preserves any winning durable terminal, and leaves an unaccepted Resume or Fork with zero visible new state. |
...
| RK-12 | Automatic context compaction loses material detail, reuses a stale summary after a correction or other source change, splits a Tool call/result round across the summarized prefix and exact tail, skips a budget check during an active tool loop, crosses ownership or fork boundaries, or lets late producer output launch inference after interrupt/fencing. | Later model behavior may rely on incomplete, stale, or foreign context, receive an orphan provider Tool result, or continue after the Turn is terminal even though canonical history is intact. | CAND-15 provides atomic source-version/digest, Turn, lease, D-9, and Fork persistence fences; CAND-16 provides one bounded normalized producer adapter; and CAND-14 consumes those completed ports to check the budget before every Turn-inference request, permit only causally closed boundaries, stage producer output until source/terminal/lease revalidation and generation-bound dispatch pass, reject post-terminal, stale-generation, or source-drifted D-9 writes, ignore late output, retain an explicit recent tail and token accounting, and fail visibly whenever complete current-source coverage cannot be proven. Fork reserves child identity and lineage before staging a child-bound D-9 and commits all child state atomically. |
...
| CAND-15 | The AI-owned store exposes one reconstructable, target-scoped D-9 persistence contract whose bounded source reads carry an effective-history version/digest and whose conditional writes, inference-establishment checks, and Fork commits reject source drift, terminal or lease-generation loss, and partial child visibility. | Primary implementation boundary: persistence and data behavior in C-6. Includes bounded versioned source reads; target/provenance keyed D-9 reads and idempotent conditional writes; atomic comparison of expected effective-history version/digest, Turn state, and lease generation; generation- and source-bound inference-establishment fencing; child identity/lineage reservation; atomic child, lineage, optional D-9, and first-Turn commit; reconstructable deletion; and typed conflict results. Excludes soft-budget and compaction policy, provider calls, summary generation, canonical-history rewriting, routes, UI, deployment, Memory, and Multitask. One durable transaction and storage-port boundary forms one independently reviewable implementation pull request. | CAND-13 and CAND-7 complete so correction-aware source identity, aggregate limits, and immutable Fork lineage are authoritative; CAND-11 correction admission remains the source-changing transaction that the D-9 fence must detect. | Checks cover bounded source versions/digests, identical-write convergence, concurrent builders, correction admitted before and during conditional commit, source change between D-9 commit and inference establishment, terminal and lease-generation races, post-terminal/stale-source rejection, tenant/subject/Thread/lineage isolation, reconstructable deletion, child reservation, parent-snapshot rejection, and atomic all-or-zero Fork child state. | Full | Ready | N/A — Ready; dependency-ordered after CAND-13 and CAND-7 | None |
...
only the source-fenced D-9 persistence boundary; CAND-16 follows CAND-13 to add
...
| 8 / CAND-15 | C-6 provides versioned bounded source reads, target-scoped D-9 storage, source/Turn/lease conditional commitment, source-bound inference establishment, and atomic Fork child state without owning compaction policy. | Source version/digest equality and drift, correction races, identical-write convergence, terminal/lease fencing, isolation, reconstructable deletion, parent-snapshot rejection, and all-or-zero Fork state have exact transactional outcomes. | Disable D-9 writes and source-bound inference establishment while retaining canonical history; remove or rebuild only reconstructable snapshots. |
...
- [x] Repository owner and required approver `@linhai` reviewed the effective-history source-provenance fence and CAND-14/CAND-15/CAND-16 implementation-boundary split, then responded with exact `Approve` in the active task at `2026-09-02T09:14:00+08:00`; active approval metadata is complete and Design Status is `Current`.
