...
- **Approval Time [Conditionally Required — Design Status is or has been `Current`]**: 2026-09-02T10:25:47+08:00
...
flowchart LR
  ...
  C2 -->|"Prefix-scoped D-9 operations; request-wide inference establishment; atomic Resume and Fork state; leases / checkpoints"| C6
  C6 -->|"Canonical operations plus prefix-fenced D-9 and request-fenced atomic Resume / Fork state"| Stores
  Stores -->|"Durable prefix and request-wide versions / digests, lineage / D-9 / Turn state, lease expiry, and recovery input"| C6
...
| CF-3 | Thread is resumed or forked / caller has access | C-6 loads canonical ordered history, prefix-scoped D-9 provenance when present, request-wide effective-context provenance, and lineage. Resume binds context assembly to the existing Thread. Fork first reserves an unpublished child Thread identity and immutable lineage at the exact fork point, then binds all D-9 selection and construction to that child scope. C-2 uses direct effective history below budget, a fitting target-bound D-9 plus exact recent tail, or one newly produced and staged target-bound D-9 over a causally closed prefix above budget. If a Resume snapshot remains prefix-valid but its grown exact tail no longer fits, C-2 invokes C-3 to produce and stage an advanced D-9 over a later causally closed prefix. Resume atomically compares the prefix and request-wide versions/digests while committing any staged initial or advanced D-9 and the new Turn, or commits neither. Fork atomically compares that provenance while committing the child identity, lineage, prefix-valid staged D-9 when present, and first Turn. | Resume may reuse only an existing-Thread prefix-provenance and causal-closure match. A tail-only append retains that snapshot and triggers exact-tail reassembly under a new request-wide version/digest; if the snapshot plus grown exact tail no longer fits, Resume selects a complete later causally closed prefix and advances D-9 only through bounded C-3 production, staging, prefix fencing, and failure handling. A prefix correction rejects the snapshot. When Resume builds or advances D-9, the snapshot never becomes visible independently of its new Turn. Fork never reuses a parent-scoped snapshot; it may build a child-bound D-9 only from an authorized causally closed parent prefix ending at or before the fork point. Missing, rejected, or prefix-drifted snapshots are rebuilt only from complete bounded source ranges at a boundary that keeps every Tool call/result group wholly in the prefix or tail; request-wide drift after assembly retries without rebuilding D-9 when its prefix remains valid and still fits, otherwise it advances the snapshot within the bounded drift budget. Concurrent builders converge idempotently. Incompatible historical Item versions use a versioned adapter. Foreground orphan closure belongs to CF-1; background recovery belongs to CF-5. | Canonical corruption, unavailable or changed request source, initial or advanced producer failure or cancellation, no causally closed boundary that fits, or final atomic-commit failure issues no Turn-inference request. Resume exposes neither a staged D-9 nor a new Turn; Fork aborts the reservation and exposes no child Thread, lineage, D-9, or Turn. No path silently truncates history, emits an orphan Tool result, mutates canonical Items, crosses scope, reuses parent D-9 as child data, commits D-9 against stale prefix provenance, commits a Turn against stale request-wide provenance, or reactivates a terminal Turn. | The original terminal Turn and complete canonical replay remain unchanged. A successful Resume atomically exposes one new Turn and its newly built or advanced D-9 when one was required on the existing Thread from the exact validated prefix and request-wide source. A successful Fork exposes one child Thread with stable lineage and one child-bound direct or causally closed compacted context; every cancelled or failed Fork has zero visible child state. |
...
flowchart TB
  subgraph CF1 ["CF-1 Foreground turn lifecycle"]
    ...
    CF1Snapshot -->|"No, complete bounded source and closed boundary available"| CF1Build["Request bounded initial D-9 production through C-3"]
    ...
    CF1BuildResult -->|"Yes"| CF1Built["Stage new D-9 from causally closed durable prefix"] --> CF1BuiltFits{"New D-9 plus exact recent tail fits budget?"}
    CF1BuiltFits -->|"No"| CF1CompactFail
    ...
    CF1Fits -->|"No"| CF1AdvanceSource{"Complete bounded later prefix and closed boundary available?"}
    CF1AdvanceSource -->|"No"| CF1CompactFail
    CF1AdvanceSource -->|"Yes"| CF1Advance["Request bounded advanced D-9 production through C-3"]
    CF1Advance --> CF1AdvanceResult{"Producer succeeds with valid bounded output?"}
    CF1AdvanceResult -->|"No"| CF1CompactFail
    CF1AdvanceResult -->|"Yes"| CF1Advanced["Stage advanced D-9 over later causally closed prefix"] --> CF1AdvancedFits{"Advanced D-9 plus exact recent tail fits budget?"}
    CF1AdvancedFits -->|"No"| CF1CompactFail
  ...
  subgraph CF3 ["CF-3 Resume or fork"]
    ...
    CF3Snapshot -->|"No, complete bounded source and closed boundary available"| CF3Build["Request bounded initial target-bound D-9 production through C-3"]
    ...
    CF3BuildResult -->|"Yes"| CF3Built["Stage initial target-bound D-9 from causally closed prefix"] --> CF3BuiltFits{"Initial D-9 plus exact recent tail fits budget?"}
    CF3BuiltFits -->|"No"| CF3Fail
    ...
    CF3Fits -->|"No"| CF3AdvanceSource{"Complete bounded later prefix and closed boundary available?"}
    CF3AdvanceSource -->|"No"| CF3Fail
    CF3AdvanceSource -->|"Yes"| CF3Advance["Request bounded advanced target-bound D-9 production through C-3"]
    CF3Advance --> CF3AdvanceResult{"Producer succeeds with valid bounded output?"}
    CF3AdvanceResult -->|"No"| CF3Fail
    CF3AdvanceResult -->|"Yes"| CF3Advanced["Stage advanced D-9 over later causally closed prefix"] --> CF3AdvancedFits{"Advanced D-9 plus exact recent tail fits budget?"}
    CF3AdvancedFits -->|"No"| CF3Fail
    CF3AdvancedFits -->|"Yes"| CF3CompactContext
    CF3Direct --> CF3Commit{"Commit Resume or Fork target?"}
    CF3CompactContext --> CF3Commit
    CF3Commit -->|"Resume"| CF3ResumeCommit{"Atomic request-wide source check, optional staged D-9, and new-Turn commit succeeds?"}
    CF3ResumeCommit -->|"No, prefix or tail changed"| CF3Fail
    CF3ResumeCommit -->|"Yes"| CF3Resume["Expose new Turn and any newly built D-9 together; prior terminal Turn stays terminal"]
...
| --- | --- | --- | --- | --- | --- |
| IX-1 | API client with authenticated principal and an existing or new thread | Start, steer, interrupt, resume, fork, or read a thread. | C-1 obtains trust context from C-7. Every lifecycle item is published only after C-6 confirms durability. Resume first loads canonical history and provenance, checks the context budget, and uses direct history or a prefix-valid D-9 only when it plus the exact tail fits. When no valid snapshot exists it requests bounded initial compaction; when a valid snapshot's grown exact tail no longer fits it requests bounded C-3 advancement over a later causally closed prefix. Both producer paths stage D-9, fence prefix and request-wide provenance, and atomically commit the staged D-9 with the new Turn; the source Turn remains terminal. Fork first reserves an unpublished child identity and immutable lineage, evaluates the child-bound direct or compacted context without reusing parent-scoped D-9, and atomically commits the child, lineage, optional staged D-9, and first Turn. Source, initial or advanced producer, closed-boundary, cancellation, drift, or atomic-commit failure rejects Resume with no new D-9 or Turn and aborts Fork with zero visible child state. `Interrupted` reports an authenticated client stop; `cancelled` reports a platform, policy, dependency, or reconciled foreground-owner stop. | A durable terminal Turn, one atomic new Turn plus optional initial or advanced D-9 from Resume, one visible child with lineage and optional child D-9 from Fork, or a rejected/cancelled request with no side effect. Initial durability or pre-Turn Resume initial/rolling compaction failure creates no D-9 or Turn; a cancelled or failed Fork exposes no child identity, lineage, D-9, or Turn; a later outage exposes only the durable prefix plus `durability-unavailable`; an orphan becomes durably `cancelled` after the liveness window. | N/A — service/protocol interaction only; no UI is designed here. |
    ...
    alt Storage unavailable
      ...
      alt Operation is Resume
        ...
        alt Source is unavailable, corrupt, or incomplete
          ...
          Core->>Core: Check soft budget and whether a reusable D-9 plus exact tail fits before creating the new Turn
          alt Direct context or reusable D-9 plus exact tail fits
            Core->>Store: Atomically validate request provenance and append new Turn
            ...
            alt No reusable prefix-valid D-9 exists
              Core->>Core: Select complete bounded initial causally closed prefix
            else Prefix-valid D-9 exists but its grown exact tail no longer fits
              Core->>Core: Select complete bounded later causally closed prefix for rolling advancement
            end
            Core->>External: Request bounded initial or advanced target-bound D-9 production through C-3
            External-->>Core: Compaction output or typed producer failure
            alt Producer fails or no selected closed boundary fits
              Core-->>C1: Typed context-compaction failure with no D-9 or new Turn
              C1-->>Client: Resume rejected with no side effect and sequence terminates
            else Prefix-valid initial or advanced compacted context is complete
              Core->>Store: Atomically fence prefix and request provenance and commit staged D-9 plus new Turn
              Store-->>Core: Both committed or typed zero-mutation failure
              alt Prefix or request source drifted or atomic commit failed
                Core-->>C1: Typed source or durability failure with no D-9 or new Turn
                C1-->>Client: Resume rejected with no side effect and sequence terminates
              else D-9 and new Turn are durable
                Note over Core,Store: Resume may enter active work and source terminal Turn remains unchanged
              end
            end
          end
        ...
        alt Reservation fails or client cancellation wins
          ...
          alt Child context is over budget
            Core->>External: Request bounded child-bound compaction through C-3
            External-->>Core: Child-bound compaction output, cancellation, or typed failure
            alt Producer fails, no closed boundary fits, or cancellation wins
            ...
            else Child-bound compacted context is complete
              Core->>Store: Atomically commit child, lineage, staged D-9, and first Turn
...
| RK-12 | Automatic context compaction loses material detail, reuses a stale prefix summary after a correction, invalidates a valid snapshot merely because the exact tail grows, fails to advance a still-valid active-Turn or Resume snapshot when its grown tail no longer fits, exposes a Resume snapshot without its new Turn, dispatches a request after prefix or tail drift, splits a Tool call/result round across the summarized prefix and exact tail, skips a budget check during an active tool loop, crosses ownership or fork boundaries, exposes partial Fork state, or lets late producer output launch inference after interrupt/fencing. | Later model behavior may rely on incomplete, stale, or foreign context, receive an orphan provider Tool result, incur unnecessary repeated compaction, fail a continuable active Turn or resumable Thread, expose derived state from a rejected Resume or Fork, or continue after the Turn is terminal even though canonical history is intact. | CAND-15 provides distinct atomic prefix-version/digest D-9 fences, request-wide effective-context inference fences, atomic Resume D-9/new-Turn commitment, and all-or-zero Fork persistence; CAND-16 provides one bounded normalized producer adapter; and CAND-14 consumes those completed ports to reuse D-9 after tail-only append, route active-Turn and pre-Turn Resume rolling advancement through the producer when a valid snapshot's grown tail no longer fits, rebuild after prefix correction, reassemble after request-wide drift, check the budget before every Turn-inference request, permit only causally closed boundaries, reject post-terminal or stale-generation writes, ignore late output, retain an explicit exact tail and token accounting, and fail visibly whenever complete current-source coverage cannot be proven. Resume stages initial or advanced D-9 and atomically commits it with the new Turn; Fork reserves child identity and lineage before staging a child-bound D-9 and commits all child state atomically; cancellation or failure exposes none. |
...
| 8 / CAND-15 | C-6 provides versioned bounded source reads, target-scoped D-9 storage, prefix/Turn/lease conditional commitment, request-wide-source-bound inference establishment, atomic Resume D-9/new-Turn state, and atomic Fork child state without owning compaction policy. | Prefix version/digest equality, request-wide version/digest equality, tail-only append, prefix and tail correction races, identical-write convergence, terminal/lease fencing, isolation, reconstructable deletion, all-or-zero Resume snapshot/new-Turn state, parent-snapshot rejection, and all-or-zero Fork state under success, cancellation, failure, and drift have exact transactional outcomes. | Disable D-9 writes and request-wide-source-bound inference establishment while retaining canonical history; remove or rebuild only reconstructable snapshots. |
...
| 10 / CAND-14 | C-2 checks every initial or active-loop Turn-inference budget and orchestrates exact history or one causally closed D-9 plus recent tail through the completed CAND-15/CAND-16 ports; staged output cannot cross prefix, request-wide, terminal, or lease fences. | Budget boundaries, producer-backed active-Turn and pre-Turn Resume snapshot advancement when a valid D-9 plus grown tail no longer fits, Tool-round causality, prefix-provenance reuse after tail-only append, D-9 rebuild after prefix correction, request reassembly after tail correction or append, atomic Resume initial/advanced D-9 and new-Turn state, Fork success/cancellation/failure feedback and zero-visible-state guarantees, bounded drift recovery, interrupt/cancel/terminal/lease races, late-output rejection, producer failure, isolation, bounded reads, canonical replay equality, and unable-to-fit outcomes are exact. | Disable C-2 snapshot creation and consumption; retain complete canonical history and only derived D-9 state. If direct effective history cannot fit, fail visibly rather than truncate or emit an orphan Tool result. |
...
- [x] Repository owner and required approver `@linhai` reviewed the complete pre-Turn Resume valid-but-insufficient snapshot advancement correction for automatic review `5084952972`, then responded with exact `Approve` in the active task at `2026-09-02T10:25:47+08:00`; active approval metadata is complete and Design Status is `Current`.
