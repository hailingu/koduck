...
- **Approval Time [Conditionally Required — Design Status is or has been `Current`]**: 2026-09-02T00:56:45+08:00
...
or inheriting removed predecessor infrastructure. When a resumed Thread would
exceed its configured provider-context token budget, C-2 automatically builds
or reuses a provenance-bearing derived compaction snapshot plus a bounded
recent tail; canonical Thread/Turn/Item history remains complete and unchanged.
...
| F-2 | Agent core | A turn is accepted | Assemble instructions, context, available capabilities, model input, and policy into one observable orchestration lifecycle. | Provider-specific types do not enter the domain model; state transitions have one owner; partial results and terminal errors remain distinguishable. A resumed Thread below the configured soft token budget uses its effective history directly; a Thread above that budget automatically uses one valid derived compaction snapshot plus a bounded recent tail. Missing, stale, corrupt, or failed compaction never silently discards context. | R-1 |
...
| D-9 | Context Compaction Snapshot | Reconstructable provider-context projection summarizing one closed canonical history prefix while retaining its source range, digest, policy version, producer identity, token accounting, and recent-tail boundary. | C-2 context-assembly policy; C-6 stores the derived snapshot through a narrow owned port but does not make it canonical history. | Derived tenant/user conversation content; as sensitive as the summarized source. | Built automatically before the provider-context soft budget is exceeded, committed idempotently, reused while its provenance remains valid, superseded by a later snapshot, invalidated by incompatible policy/version/lineage changes, and deleted with its owning Thread under the same content-retention policy. |
...
| Context Compaction Snapshot summarizes Item range | One D-9 snapshot covers one contiguous, closed prefix of effective canonical history and retains a bounded unsummarized tail after that prefix. | The source Thread, tenant, subject, fork lineage, inclusive source range, source digest, policy version, and producer identity must match before reuse. D-9 is untrusted conversation content and cannot become instructions, policy, identity, approval, or execution authority. It never replaces or authorizes deletion of D-1/D-2/D-3 data; concurrent builders converge to one committed snapshot for the same provenance key. |
...
| C-2 | Agent core | Own thread/turn orchestration, state transitions, effective-context assembly, automatic context-compaction policy, budgets, cancellation, foreground lease heartbeats, orphan reconciliation, and provider-independent policy flow. | Owned turn input, instructions, canonical context references, D-9 provenance and recent-tail boundaries, capabilities, lease-expiry signals; bounded model context, derived compaction snapshots, typed items, and terminal outcomes. | C-3, C-4, C-5, C-6, C-7. | Any healthy C-2 instance may reconcile an expired foreground lease, but only through C-6 generation fencing. Context compaction may create only a derived D-9 projection and must fail visibly rather than silently omit uncovered history. No provider wire types, Web handlers, database types, or privileged host execution enter the core. |
...
| CF-3 | Thread is resumed or forked / caller has access | C-6 loads canonical ordered history, lineage, and any provenance-matching D-9 snapshot; C-2 reconstructs effective model context under the configured token budget. Below the soft budget it uses effective history directly. Above the soft budget it automatically builds or advances one D-9 snapshot over a closed prefix and retains a bounded recent tail. Resume starts a new turn on the same thread; fork creates a child thread and then a new turn. | A missing snapshot is built from bounded canonical ranges; a stale, corrupt, incompatible, or provenance-mismatched snapshot is rejected and rebuilt only when the complete required source range is available. Concurrent builders converge idempotently. Incompatible historical item versions are translated by a versioned adapter. Foreground orphan closure belongs to CF-1; process recovery of a still-active background turn belongs to CF-5. Neither is Resume. | Canonical-history corruption, an unavailable required source range, compaction-producer failure, or inability to fit the snapshot plus retained tail fails visibly and creates no new Turn or provider request. No path silently truncates uncovered history, mutates canonical Items, crosses tenant/subject/lineage scope, or reactivates a terminal Turn. | The original terminal Turn and complete canonical replay remain unchanged. The new Turn or child Thread has stable lineage and receives either direct effective history or one provenance-bearing compaction snapshot plus its exact recent tail within budget. |
...
flowchart TB
  ...
  subgraph CF3 ["CF-3 Resume or fork"]
    ...
    CF3Version -->|"No, adapter exists"| CF3Translate["Apply versioned translation"] --> CF3Budget
    CF3Version -->|"No adapter"| CF3Fail
    CF3Version -->|"Yes"| CF3Budget{"Effective context within soft token budget?"}
    CF3Budget -->|"Yes"| CF3Direct["Use effective history directly"]
    CF3Budget -->|"No"| CF3Snapshot{"Valid provenance-matching D-9 snapshot available?"}
    CF3Snapshot -->|"Yes"| CF3Compact["Reuse or advance snapshot and retain exact recent tail"]
    CF3Snapshot -->|"No, complete bounded source available"| CF3Build["Build and idempotently commit derived D-9 snapshot"] --> CF3Compact
    ...
    CF3Direct --> CF3Operation{"Resume or fork?"}
    CF3CompactContext --> CF3Operation
...
| RK-12 | Automatic context compaction loses material detail, reuses a stale summary, or crosses ownership or fork boundaries. | Later model behavior may rely on incomplete or foreign context even though canonical history is intact. | CAND-14 keeps D-9 derived and reconstructable, binds reuse to tenant/subject/Thread/lineage, source range and digest, policy and producer versions, retains an explicit recent tail, records token accounting, rejects mismatches, and fails visibly whenever complete coverage cannot be proven. |
...
| CAND-14 | A continuing Thread whose effective provider context exceeds its configured soft token budget automatically uses one provenance-bearing derived compaction snapshot plus a bounded recent tail, while complete canonical history remains unchanged and replayable. | Primary implementation boundary: domain and application context-assembly policy in C-2. Includes the soft-budget trigger, D-9 snapshot lifecycle and provenance, bounded canonical range reads, idempotent snapshot commitment through a narrow C-6 port, one owned compaction-producer port implemented by the configured C-3 adapter, recent-tail retention, correction/fork invalidation, and typed fail-closed outcomes. Supporting C-3/C-6 interfaces remain limited to this C-2 policy. Excludes canonical history deletion or rewriting, semantic Memory ownership, provider fallback, UI, deployment, and changes to the independent per-Turn output budget. One context-compaction policy and its narrow supporting ports form one independently reviewable implementation pull request. | CAND-13 and CAND-7 complete so effective correction-aware provider input, aggregate limits, and immutable fork lineage are authoritative; CAND-1 canonical history remains the sole source of truth. | Checks cover below/at/one-over soft token budgets, initial and rolling compaction, exact recent-tail retention, snapshot provenance and reuse, concurrent-builder convergence, stale/corrupt/missing snapshot handling, correction and fork boundaries, producer timeout/failure, tenant/subject isolation, Tool-round causality, bounded reads and memory, canonical replay equality, and inability to fit the snapshot plus tail. A failed compaction-producer request creates no subsequent Turn-inference provider request or new Turn, and no failure silently drops uncovered history. | Full | Ready | N/A — Ready; dependency-ordered after CAND-13 and CAND-7 | None |
...
| 8 / CAND-14 | A continuing Thread automatically uses one provenance-bearing D-9 snapshot plus its exact recent tail when effective provider context exceeds the soft token budget, without mutating canonical history. | Below/at/one-over budgets, initial/rolling compaction, provenance reuse, concurrent builders, correction/fork invalidation, producer failure, isolation, bounded reads, Tool causality, canonical replay equality, and unable-to-fit outcomes are exact. | Disable new snapshot creation and provider consumption of D-9; retain complete canonical history and delete or rebuild only derived snapshots. If direct effective history cannot fit, fail visibly rather than truncate. |
...
- [x] Repository owner and required approver `@linhai` reviewed the R-1/Codex evidence disposition and CAND-14 Turn-inference failure clarification, then responded with exact `Approve` in the active task at `2026-09-02T00:56:45+08:00`; active approval metadata is complete and Design Status is `Current`.
