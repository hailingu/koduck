...
`koduck-ai` is therefore the evidence baseline for current behavior, fixed at
commit `c414ddccdbc45a99fcd3d606ca0fe1f75730b7fe`. It is a Rust AI gateway and
...
Keep the existing northbound REST/SSE and southbound Memory/Tool/Multitask
contracts behind adapters during migration. Move capability discovery, policy
...
protocols as independently replaceable adapters. Preserve Koduck's
multi-provider, tenant, distributed-memory, and background-task semantics
rather than adopting Codex's product-specific local persistence and auth model.

**Migration operating model**: The predecessor `koduck-ai` deployment remains
the rollback target while replacement Koduck services are introduced
side-by-side behind APISIX. Traffic moves by an explicitly bounded cohort or
capability slice; both paths use only the canonical history representation
declared compatible for that slice. Before CAND-1's ADR can be accepted, it must
confirm an immutable legacy artifact, a routable old path, the shared-history
compatibility boundary, and a tested route-back condition. If those preconditions
are unavailable, the side-by-side slices in this ADD are not executable and
must be redesigned rather than treating a repository checkout as a rollback.
...
- Establish one auditable current-state baseline and one immutable Codex reference baseline.
...
- Preserve externally consumed Koduck behavior through compatibility boundaries while enabling incremental replacement.
...
| F-3 | Model adapter | The core requests inference | Translate owned model input and streamed output to one configured provider without leaking provider wire types into the core. | Provider fallback is explicit; time, token, retry, and output budgets are bounded; usage and terminal status are preserved. | R-1 |
...
| F-5 | Thread-store adapter | Thread state changes | Persist canonical thread/turn/item history and metadata through an owned store port backed by Koduck services. | One canonical owner per datum; appends are ordered and idempotent; local caches are reconstructable and never silently become truth. | R-1 |
...
| --- | --- | --- | --- | --- | --- |
| C-1 | Presentation and compatibility boundary | Expose existing REST/SSE behavior, the authenticated approval protocol, and future typed application protocols; translate them to owned domain requests, approval decisions, and events. | Client and approver requests, gateway context, thread/turn operations, approval decisions; typed lifecycle events and compatibility responses. | C-2, C-5, C-7. | C-1 delegates signed-claim validation and trust-context construction to C-7; it never validates identity by itself. Existing public behavior remains stable until a separately Accepted ADR authorizes change; UI design is out of scope. |
...
| C-6 | Thread-store port and adapters | Persist and retrieve canonical history, metadata, lineage, foreground liveness leases, checkpoints, and idempotency state using Koduck services. | Thread/turn/item appends and queries, lease acquire/renew/expire operations; ordered history, metadata, and fenced lease generations. | Memory and Multitask services. | Lease expiry and orphan terminal transition are conditional and idempotent by turn plus generation; a stale generation cannot append after expiry or reassignment. Koduck distributed stores remain canonical; local persistence is a reconstructable cache only. |
...
flowchart LR
  ...
  subgraph Runtime ["Koduck AI runtime boundary"]
    C1["C-1 Presentation and compatibility boundary"]
    ...
    C6["C-6 Thread-store port and adapters"]
  ...
  C1 -->|"Compatibility responses and lifecycle events"| Client
...
flowchart TB
  subgraph CF1 ["CF-1 Foreground turn lifecycle"]
    ...
    CF1Outcome -->|"Explicit configured fallback"| CF1Fallback["Select fallback provider"] --> CF1Provider
...
| Compatibility | Existing northbound REST/SSE client contracts and southbound gRPC service contracts remain behind their respective adapters; new internal types are versioned and provider neutral. | Contract comparison separately verifies C-1 client behavior and C-6 southbound service behavior without implying a client-facing gRPC API; incompatible changes require separate accepted decisions. |
...
| --- | --- | --- | --- | --- |
| Q-1 | What is the current AI-service baseline when this repository has no service code? | @linhai | Resolved | Use the predecessor `koduck-quant` `koduck-ai` tree at commit `c414ddccdbc45a99fcd3d606ca0fe1f75730b7fe`. The current repository [README](../../README.md) identifies Koduck as its from-scratch rebuild and states that no service has been added. |
...
| Q-5 | What operational coexistence makes the migration rollback paths real? | @linhai | Resolved | This design uses a side-by-side predecessor/replacement deployment behind APISIX, with bounded cohort or capability routing and a shared-history compatibility boundary per slice. CAND-1's ADR cannot be accepted until it verifies the immutable legacy artifact, routable old path, compatible canonical-history subset, and route-back condition. |
...
| RK-2 | Compatibility adapters become permanent duplication. | Two domain models and two event semantics increase maintenance cost. | Give each adapter an explicit retirement criterion and remove it only after contract consumers migrate under an Accepted ADR. |
...
| RK-5 | History and memory ownership overlap. | Duplicate truth, inconsistent replay, or cross-tenant leakage. | Freeze one logical ownership matrix in CAND-3 before migration and treat caches as reconstructable. |
| RK-6 | New typed protocol breaks current REST/SSE clients. | Product regression during structural migration. | Introduce it behind C-1, verify semantic parity, and keep legacy contracts until a separate retirement decision. |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| CAND-1 | A provider-neutral thread/turn/item orchestration kernel can execute one authenticated, tool-free turn through the legacy REST/SSE compatibility boundary with durable ordered history and explicit completion, failure, interruption, durability-outage, and foreground-owner-loss outcomes. | Includes the owned lifecycle model, core ports, compatibility mapping, one provider path, and a minimal C-6 compatibility adapter over the current canonical-history path sufficient for append-before-publish, replay, and fenced foreground liveness leases; excludes final Memory/Multitask ownership, fork/checkpoint/idempotency migration, privileged tools, extensions, background work, public-contract retirement, and deployment. | This ADD must be `Current`; external contract baselines, immutable legacy artifact, APISIX side-by-side route, compatible canonical-history subset, and route-back condition must be verified before ADR acceptance. | Binary mapping for one non-tool turn; deterministic state, Resume-as-new-Turn, append-before-publish, bounded append/backpressure and liveness windows, replay, provider/store failure, process crash, lease expiry, stale-owner fencing, concurrent-reconciler idempotency, exactly-one orphan `cancelled` terminal, and rollback checks. | Full | Selected | Selected by the Proposed, Not Started project Full ADR at `docs/adr/ADR-0001-provider-neutral-turn-kernel.md` | `docs/adr/ADR-0001-provider-neutral-turn-kernel.md` |
| CAND-2 | Every tool and MCP invocation passes through one default-deny policy and isolated D-7 execution boundary; any required approval uses one canonical exact-action D-6 record, with cancellation, timeout, output-cap, lease fencing, and an auditable terminal result. | Includes C-1/C-7 approval transport, C-5 authority, C-6 foreground-lease validation, D-3 status projections, and adapters for current Tool/MCP paths; excludes reusable session/turn grants, UI design, and expansion of allowed privileged capabilities. | CAND-1 complete; authenticated approval protocol and current tool-effect inventory available. | Checks cover allow without approval, deny, invalid approver identity, decline, cancel, expiry, scope/attempt/lease drift, stale-owner dispatch and result rejection, pre-effect retry reapproval, timeout, cancellation, and untrusted output; rollback disables the new dispatcher and restores the prior approved tool path. | Full | Ready | N/A — Ready; dependency prevents selection before CAND-1 completes | None |
| CAND-3 | Canonical thread, turn, item, lineage, liveness lease, checkpoint, and idempotency ownership is served through a replaceable store port backed by Memory/Multitask without process-local truth. | Replaces CAND-1's minimal history/liveness compatibility adapter with the complete logical ownership and adapter model, including replay, foreground orphan reconciliation, fork, checkpoint, background-resume, and projection semantics; excludes physical database redesign and unrelated memory ranking behavior. | CAND-1 complete; Memory and Multitask contract owners participate. | Replay/order/append-only-correction/lease-fencing/tenant-isolation and duplicate-submission checks; migration preserves CAND-1 history and foreground terminal semantics, while rollback keeps canonical data and discards only reconstructable projections. | Full | Ready | N/A — Ready; dependency prevents selection before CAND-1 completes | None |
...
| CAND-5 | Foreground and background model turns use the new core across supported providers while existing REST/SSE consumers observe compatible behavior and the superseded orchestration path has a verified retirement boundary. | Includes provider adapters, background lifecycle integration, compatibility evidence, controlled cutover, and old-path retirement; excludes new product features and UI redesign. | CAND-1 through CAND-4 complete and verified; consumer migration inventory complete. | Provider/stream/background parity, recovery, SLO, and rollback-trigger checks; rollback routes new traffic to the last verified path without rewriting canonical history. | Full | Ready | N/A — Ready; final migration candidate | None |
...
| Application protocol | REST/SSE routes directly expose orchestration-specific behavior. | Typed bidirectional app-server protocol with generated schemas. | Keep REST/SSE as compatibility facade and add a versioned provider-neutral application protocol when a consumer requires it. | Adjusted adoption. |
...
| Storage | Memory and Multitask are distributed service dependencies; process-local registries and checkpoints also exist. | Replaceable `ThreadStore`; local rollout JSONL and SQLite-backed metadata. | Owned store port backed by Memory/Multitask; local state is cache/projection only. | Adopt store abstraction; do not adopt local persistence as truth. |
| Provider layer | Multi-provider Rust adapters, explicit routing, normalized types, streaming, retry, and fallback policy already exist. | Core client and model-provider packages focus strongly on OpenAI/Codex product needs. | Preserve multi-provider adapters behind an owned interface and converge lifecycle events at the core boundary. | Preserve and refine; do not replace with ChatGPT-specific behavior. |
...
| AD-3 | Replaceable thread store | Adopt after adjustment | The port is valuable; CAND-1 starts with a minimal compatibility adapter over existing canonical history, then CAND-3 establishes complete Memory/Multitask ownership instead of local JSONL/SQLite. |
...
| AD-7 | Codex local rollout files and SQLite as canonical state | Do not adopt | Koduck already has distributed Memory/Multitask ownership and requires shared multi-instance state. |
...
| --- | --- | --- | --- |
| Client or approver to AI | REST/SSE chat, stream, session, memory BFF, task, tool, and approval protocol routes | C-1 with C-7 identity validation | Preserve observable behavior during migration; C-1 delegates signed-claim validation to C-7 before state/model/tool/approval work; bounded bodies, ordered durable stream terminals, and exact D-6 identities are mandatory. |
...
| AI to Memory | gRPC memory/session/history/fact/plan operations | C-6 adapter | Tenant/user/thread ownership, deadlines, idempotency, version negotiation, and exact data ownership are mandatory. |
| AI to Multitask | Background submission, lease, checkpoint, retry, and terminal-state operations | C-6 adapter | Duplicate submissions map to one logical job; lease loss cannot duplicate non-idempotent effects; credentials never enter history. |
...
| AI to model provider | Provider-native HTTP/streaming | C-3 | Secrets stay at adapter boundary; explicit provider/model/fallback; redaction, token/time/retry limits, and normalized terminals. |
...
### Ordered Migration Slices, Validation, And Rollback Boundaries

The candidate order is CAND-1, CAND-2, CAND-3, CAND-4, and CAND-5.
CAND-6 remains deferred. CAND-1 introduces a minimal C-6 compatibility adapter
over the currently verified canonical-history path; CAND-3 replaces that
temporary boundary with the complete Memory/Multitask ownership model. All
slices assume side-by-side predecessor and replacement deployments behind
APISIX until CAND-5 closes the rollback window. Each selected candidate requires
one reciprocal Full ADR with no more than three implementation subtasks and
deterministic checks.

| Slice | Minimum architecture outcome | Architecture-level validation | Rollback boundary |
| --- | --- | --- | --- |
| 1 / CAND-1 | One tool-free authenticated turn crosses compatibility → core → provider → a minimal C-6 compatibility adapter and reaches one explicit durable terminal state. | Legacy mapping, Resume-as-new-Turn, append-before-publish latency/backpressure, foreground liveness window, process crash, lease expiry, stale-owner fencing, exactly-one orphan cancellation, ordered replay, provider/store failure, and APISIX route-back all have binary states. | Route the bounded cohort to the verified predecessor deployment; retain only the canonical-history subset both paths understand. A checkout or rebuild is not a rollback target. |
| 2 / CAND-2 | All current tool/MCP effects cross one C-5 authority and isolated one-attempt D-7 boundary; required approvals use exact D-6 records and D-3 carries projections only. | Demonstrate allow without approval, deny, invalid approver, accept/decline/cancel/expiry, scope or attempt drift, retry reapproval, timeout, output cap, and untrusted-result handling. | Disable the new dispatcher and return to the last approved legacy tool path; no pending projection or partially authorized scope can execute. |
| 3 / CAND-3 | Memory/Multitask replace the minimal CAND-1 adapter with the complete canonical thread store and recovery model. | Ordered replay, append-only correction, fork lineage, tenant isolation, checkpoint recovery, duplicate submission, cache loss, and CAND-1 history migration have exact outcomes. | Preserve prior canonical data; discard or rebuild only new projections/caches; stop recovery when ownership is ambiguous. |
...
| 5 / CAND-5 | Foreground/background and supported providers run on the new core with compatibility evidence; legacy orchestration is retired only after cutover gates pass. | Contract parity, stream ordering, provider behavior, background recovery, SLO thresholds, error budgets, and rollback triggers are exact. | Shift traffic to the last verified path while keeping the same canonical store and external contract; old code removal occurs only after the rollback window closes under an Accepted decision. |
...
- [x] Every `Selected` or `Complete` candidate has an exact reciprocal ADR path; CAND-1 is `Selected` by `docs/adr/ADR-0001-provider-neutral-turn-kernel.md`, whose Architecture Source points back to this ADD and candidate ID.
