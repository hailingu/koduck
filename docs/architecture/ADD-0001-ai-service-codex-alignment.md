...
- [ ] Repository owner and required approver `@linhai` must review the complete producer-failure exit and candidate-selection-gate corrections for automatic reviews `5085863664` and `5085923290` and respond with exact `Approve`; active approval metadata remains pending and Design Status is `Draft`.
