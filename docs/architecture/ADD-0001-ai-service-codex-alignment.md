...
flowchart TB
  ...
  subgraph CF3 ["CF-3 Resume or fork"]
    ...
    CF3ForkScope --> CF3ForkExisting{"Child already committed for this reservation?"}
        ...
        alt Reservation fails or client cancellation wins
          ...
          alt Child context is over budget
            ...
            alt Producer cancelled the pass
              ...
              Store-->>Core: Zero visible child state acknowledged, or already-committed child state when a concurrent attempt committed it
              Core-->>C1: Fork cancelled with no child state, or the already-committed child returned as the idempotent result when concurrently committed
              C1-->>Client: Typed cancellation or idempotent existing-child result and sequence terminates
            else Producer fails, no bounded closed delta fits, or pass budget ends
              Core->>Store: Abort child reservation and discard staged output, rechecking whether this reservation's child was already committed
              Store-->>Core: Zero visible child state acknowledged, or already-committed child state when a concurrent attempt committed it
              Core-->>C1: Fork failed with no child state, or the already-committed child returned as the idempotent result when concurrently committed
              C1-->>Client: Typed failure or idempotent existing-child result and sequence terminates
              ...
              alt Child state for this reservation is already committed
                ...
                Core->>Store: Abort child reservation and discard staged chain, rechecking whether this reservation's child was already committed
                Store-->>Core: Zero visible child state acknowledged, or already-committed child state when a concurrent attempt committed it
                Core-->>C1: Fork rejected with no child state, or the already-committed child returned as the idempotent result when concurrently committed
                C1-->>Client: Typed validation failure or idempotent existing-child result and sequence terminates
              else Transaction cancelled or fenced before commit
                Core->>Store: Abort child reservation and discard staged chain, rechecking whether this reservation's child was already committed
                Store-->>Core: Zero visible child state acknowledged, or already-committed child state when a concurrent attempt committed it
                Core-->>C1: Fork cancelled with no child state, or the already-committed child returned as the idempotent result when concurrently committed
