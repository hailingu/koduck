...
- **Approver [Conditionally Required — Design Status is or has been `Current`]**: Pending — reapproval required
- **Approval Time [Conditionally Required — Design Status is or has been `Current`]**: Pending — reapproval required
- **Approval Evidence [Conditionally Required — Design Status is or has been `Current`]**: Pending — reapproval required
- **Retired By [Conditionally Required — Design Status is `Deprecated` or `Superseded`]**: N/A — Design Status is `Draft`; the document has not been retired
- **Retirement Time [Conditionally Required — Design Status is `Deprecated` or `Superseded`]**: N/A — Design Status is `Draft`; the document has not been retired
- **Retirement Evidence [Conditionally Required — Design Status is `Deprecated` or `Superseded`]**: N/A — Design Status is `Draft`; the document has not been retired
- **Retirement Reason [Conditionally Required — Design Status is `Deprecated` or `Superseded`]**: N/A — Design Status is `Draft`; the document has not been retired
...
flowchart TB
  ...
  subgraph CF3 ["CF-3 Resume or fork"]
    ...
    CF3ForkWinner -->|"Matches reservation"| CF3Fork
    CF3ForkWinner -->|"Missing or mismatched"| CF3Fail
        ...
        alt Reservation fails or client cancellation wins
          ...
          alt Child context is over budget
            ...
            alt Producer cancelled the pass
              ...
              C1-->>Client: Typed cancellation with zero visible child state and sequence terminates
            else Producer fails, no bounded closed delta fits, or pass budget ends
              Core->>Store: Abort child reservation and discard staged output
              Store-->>Core: Zero visible child state acknowledged
              Core-->>C1: Fork failed with no child state
              C1-->>Client: Typed failure with zero visible child state and sequence terminates
              ...
              Store-->>Core: Final child-chain winner and all child state, earliest non-identical conflict, typed request-source drift, typed cancellation or fenced stop, typed zero-mutation durability failure, or typed zero-mutation validation failure
              alt Earliest non-identical member conflict
                ...
                alt Existing child state matches this reservation
                  ...
                  C1-->>Client: Fork succeeded with the existing visible child as the idempotent result
                ...
                loop Bounded request-drift child recovery until the reassembled child chain commits or the drift budget ends
                  ...
                  alt Cancellation wins during child reassembly
                    ...
                    Store-->>Core: Final child-chain winner and child state, another request drift, earliest child conflict winner, typed cancellation or fenced stop, typed zero-mutation durability failure, or typed zero-mutation validation failure
...
time. CAND-11 through CAND-16 are `Ready` with no ADR; they are eligible for
dependency-ordered selection only while this ADD is `Current`, and no candidate
may be selected while the ADD is `Draft` pending reapproval. No slice assumes a predecessor deployment, legacy route,
