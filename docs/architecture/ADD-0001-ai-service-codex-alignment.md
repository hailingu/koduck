...
- **Approval Time [Conditionally Required — Design Status is or has been `Current`]**: 2026-09-02T01:15:29+08:00
...
| D-9 | Context Compaction Snapshot | Reconstructable provider-context projection summarizing one closed canonical history prefix while retaining its source range, digest, policy version, producer identity, token accounting, and recent-tail boundary. A closed prefix ends at a durable Item boundary and may end inside an active Turn before its next inference. | C-2 context-assembly policy; C-6 stores the derived snapshot through a narrow owned port but does not make it canonical history. | Derived tenant/user conversation content; as sensitive as the summarized source. | Built automatically before any Turn-inference request whose effective context exceeds the soft budget, committed idempotently, reused while its provenance remains valid, superseded by a later snapshot, invalidated by incompatible policy/version/lineage changes, and deleted with its owning Thread under the same content-retention policy. |
...
| Context Compaction Snapshot summarizes Item range | One D-9 snapshot covers one contiguous, durably closed prefix of effective canonical history and retains a bounded unsummarized tail after that prefix. The prefix may end at a stable Item boundary inside an active Turn before its next inference. | The target Thread, tenant, subject, fork lineage, inclusive source range, source digest, policy version, and producer identity must match before reuse. A Fork reserves its child Thread identity and immutable lineage before snapshot selection; a parent-scoped D-9 is never reused as child-scoped data. The child-bound snapshot may be built only from the authorized parent prefix ending at the exact fork point and is committed atomically with the child lineage and first Turn, or none of them becomes visible. D-9 is untrusted conversation content and cannot become instructions, policy, identity, approval, or execution authority. It never replaces or authorizes deletion of D-1/D-2/D-3 data; concurrent builders converge to one committed snapshot for the same provenance key. |
...
| CF-3 | Thread is resumed or forked / caller has access | C-6 loads canonical ordered history and lineage. Resume binds context assembly to the existing Thread. Fork first reserves an unpublished child Thread identity and immutable lineage at the exact fork point, then binds all D-9 selection and construction to that child scope. C-2 uses direct effective history below budget or builds/stages a target-bound D-9 plus exact recent tail above budget. Resume appends a new Turn; Fork atomically commits the child identity, lineage, staged D-9 when present, and first Turn. | Resume may reuse only an existing-Thread provenance match. Fork never reuses a parent-scoped snapshot; it may build a child-bound D-9 from the authorized parent prefix ending at the fork point. Missing or rejected snapshots are rebuilt only from complete bounded source ranges; concurrent builders converge idempotently. Incompatible historical Item versions use a versioned adapter. Foreground orphan closure belongs to CF-1; background recovery belongs to CF-5. | Canonical corruption, unavailable source, producer failure, unable-to-fit context, or final atomic-commit failure issues no Turn-inference request. Resume creates no new Turn; Fork aborts the reservation and exposes no child Thread, lineage, D-9, or Turn. No path silently truncates history, mutates canonical Items, crosses scope, reuses parent D-9 as child data, or reactivates a terminal Turn. | The original terminal Turn and complete canonical replay remain unchanged. A successful Resume creates one new Turn on the existing Thread. A successful Fork exposes one child Thread with stable lineage and one child-bound direct or compacted context; every failed Fork has zero visible child state. |
...
flowchart TB
  subgraph CF1 ["CF-1 Foreground turn lifecycle"]
    ...
    CF1Budget -->|"No"| CF1Snapshot{"Valid provenance-matching D-9 available?"}
    CF1Snapshot -->|"Yes"| CF1Compact["Reuse or advance D-9 and retain exact recent tail"]
    CF1Snapshot -->|"No, complete bounded source available"| CF1Build["Build and idempotently commit D-9 from durable prefix"] --> CF1Compact
    CF1Snapshot -->|"No, source unavailable or producer fails"| CF1CompactFail["Prepare failed terminal; no next Turn-inference request"] --> CF1Persist
    CF1Compact --> CF1Fits{"D-9 plus recent tail fits budget?"}
  ...
  subgraph CF3 ["CF-3 Resume or fork"]
    ...
    CF3Budget -->|"No"| CF3Snapshot{"Valid target-scope D-9 available? Never reuse parent D-9 for Fork"}
    CF3Snapshot -->|"Yes"| CF3Compact["Reuse or advance snapshot and retain exact recent tail"]
    CF3Snapshot -->|"No, complete bounded source available"| CF3Build["Build or stage target-bound D-9 from exact authorized prefix"] --> CF3Compact
    CF3Snapshot -->|"No, source unavailable or producer fails"| CF3Fail
    CF3Compact --> CF3Fits{"Snapshot plus retained tail fits budget?"}
    CF3Fits -->|"No"| CF3Fail
...
| Reliability | Explicit lifecycle states, cancellation, timeout, retry and token budgets, idempotency, append-before-publish ordering, provenance-bound context compaction, fenced foreground liveness leases, checkpoint ownership, and truthful partial/terminal outcomes. | Each externally visible item is durable before publication. Cancellation, timeout, dependency failure, storage failure, approval decline, duplicate input, foreground owner loss, snapshot mismatch, and compaction-producer failure each have an exact state; lease expiry produces exactly one `cancelled` terminal and rejects stale-owner writes; before every Turn-inference request, context compaction either proves complete source coverage within budget or prevents that request, durably fails an already accepted active Turn, and leaves an unaccepted Resume or Fork with zero visible new state. |
...
| RK-12 | Automatic context compaction loses material detail, reuses a stale summary, skips a budget check during an active tool loop, or crosses ownership or fork boundaries. | Later model behavior may rely on incomplete or foreign context even though canonical history is intact. | CAND-14 checks the budget before every Turn-inference request, keeps D-9 derived and reconstructable, binds reuse to tenant/subject/Thread/lineage, source range and digest, policy and producer versions, retains an explicit recent tail, records token accounting, rejects mismatches, and fails visibly whenever complete coverage cannot be proven. Fork reserves child identity and lineage before staging a child-bound D-9 and commits all child state atomically. |
...
| CAND-14 | Before every Turn-inference request in an initial, resumed, forked, or already active tool-loop Turn, effective provider context above its configured soft token budget automatically uses one provenance-bearing derived compaction snapshot plus a bounded recent tail, while complete canonical history remains unchanged and replayable. | Primary implementation boundary: domain and application context-assembly policy in C-2. Includes the per-inference soft-budget trigger, active-Turn durable Item prefixes, D-9 snapshot lifecycle and provenance, bounded canonical range reads, idempotent snapshot commitment through a narrow C-6 port, one owned compaction-producer port implemented by the configured C-3 adapter, recent-tail retention, correction invalidation, child identity/lineage reservation before Fork compaction, atomic child-state commitment, and typed fail-closed outcomes. Supporting C-3/C-6 interfaces remain limited to this C-2 policy. Excludes canonical history deletion or rewriting, semantic Memory ownership, provider fallback, UI, deployment, and changes to the independent per-Turn output budget. One context-compaction policy and its narrow supporting ports form one independently reviewable implementation pull request. | CAND-13 and CAND-7 complete so effective correction-aware provider input, aggregate limits, and immutable fork lineage are authoritative; CAND-1 canonical history remains the sole source of truth. | Checks cover below/at/one-over soft token budgets before initial inference, provider retry, and each post-tool inference; initial and rolling active-Turn compaction; exact recent-tail retention; snapshot provenance and reuse; concurrent-builder convergence; stale/corrupt/missing snapshot handling; correction boundaries; producer timeout/failure; tenant/subject isolation; Tool-round causality; bounded reads and memory; canonical replay equality; and inability to fit the snapshot plus tail. Fork checks prove child identity and lineage precede child-bound D-9 selection/build, a parent D-9 is never reused as child state, and failed compaction or atomic commit exposes no child Thread, lineage, D-9, or Turn. A failed compaction path creates no subsequent Turn-inference request, durably fails an already accepted active Turn, creates no new Resume/Fork Turn, and never silently drops uncovered history. | Full | Ready | N/A — Ready; dependency-ordered after CAND-13 and CAND-7 | None |
...
| 8 / CAND-14 | Before every initial or active-loop Turn-inference request, a continuing Thread automatically uses one provenance-bearing D-9 snapshot plus its exact recent tail when effective provider context exceeds the soft token budget, without mutating canonical history; Fork compaction is child-scoped and committed atomically with child state. | Below/at/one-over budgets before initial inference, retry, and post-tool inference; active-Turn rolling compaction; provenance reuse; concurrent builders; correction invalidation; child identity/lineage before child-bound D-9 construction; parent-snapshot non-reuse; atomic failed-Fork zero state; producer failure; isolation; bounded reads; Tool causality; canonical replay equality; and unable-to-fit outcomes are exact. | Disable new snapshot creation and provider consumption of D-9; retain complete canonical history and delete or rebuild only derived snapshots. If direct effective history cannot fit, fail visibly rather than truncate; discard staged Fork state without exposing a child. |
...
- [x] Repository owner and required approver `@linhai` reviewed the active-turn per-inference budget gate and Fork child-scope atomicity correction, then responded with exact `Approve` in the active task at `2026-09-02T01:15:29+08:00`; active approval metadata is complete and Design Status is `Current`.
