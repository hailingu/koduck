...
- [x] 每个 `Selected`/`Complete` 候选项均有精确双向 ADR 路径；CAND-1 已通过 `docs/adr/ADR-0001-provider-neutral-turn-kernel.md` 完成，CAND-2 已通过 `docs/adr/ADR-0003-default-deny-tool-approval-execution-boundary.md` 选中；两份 ADR 的 Architecture Source 均指回本 ADD 与对应候选项 ID。
- [x] 必填和条件触发内容完整。
...
| 2026-08-11 | 为 CAND-1 增加第七个 Review Correction Evidence Commit `31ef43f`，覆盖可中断的 Provider Request Establishment 与 1-MiB 未终止 Frame 上限；Candidate 保持 `Complete`，已接受的 Outcome 与 Scope 不变。 | @codex |
| 2026-08-11 | 为 CAND-1 增加第八个 Review Correction Evidence Commit `d444cf3`，覆盖按 Turn 连续的并发 Thread History 与自有超大 Body/Method Rejection Problem；Candidate 保持 `Complete`，已接受的 Outcome 与 Scope 不变。 | @codex |
| 2026-08-12 | 通过 Proposed、Not Started 的项目级 Full ADR `docs/adr/ADR-0003-default-deny-tool-approval-execution-boundary.md` 选中 CAND-2，并同步双向路径。 | @codex |
| 2026-08-12 | `@linhai` 接受 `docs/adr/ADR-0003-default-deny-tool-approval-execution-boundary.md` 后同步 CAND-2 证据；ADR 为 `Accepted`、`Not Started` 时，候选项保持 `Selected`。 | @codex |
| 2026-08-12 | 关联 Accepted ADR 为测试优先的 T-1 实施进入 `In Progress` 后，同步 CAND-2。 | @codex |
| 2026-08-17 | 关联 ADR 因枚举带内 SSE `error` Transport Diagnostic Event 的使审批失效 Wire Contract 调和而回到 `Proposed`、`Not Started`，据此同步 CAND-1；候选项恢复为 `Selected`，历史完成证据予以保留，待 ADR 重新审批与重新验证。 | @kimi |
| 2026-08-17 | `@linhai` 于 `2026-08-17T08:57:39Z` 重新批准关联 ADR 且修订后验收检查重新执行通过，同步 CAND-1 为 `Complete`；关联 ADR 为 `Accepted`、`Complete`。 | @kimi |
