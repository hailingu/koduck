...
| --- | --- | --- | --- | --- |
| T-1 | Add and characterize the private strict-JSON validator. | `koduck-ai/src/adapters/strict_json.rs`; private module declaration; focused unit tests for SJ-01/SJ-02. | Not Started | Pending |
| T-2 | Migrate both consumers without changing their owned semantics. | `koduck-ai/src/adapters/tool.rs`; `koduck-ai/src/adapters/provider/stream_state.rs`; existing Tool and provider regressions for SJ-03/SJ-04. | Not Started | Pending |
| T-3 | Verify service-wide compatibility and governance evidence. | Routed Rust checks, contract traceability, risk matrix, source markers, and stable implementation evidence. | Not Started | Pending |
...
| --- | --- | --- | --- | --- |
| `koduck-ai/src/adapters/tool.rs` | `parse_action_parameters`; `parse_input_schema`; current private `UniqueJson` deserializer | N/A — stable functions and type identify the decisive validation call sites. | Applies size limits, strict syntax validation, semantic conversion, and Tool-owned error mapping. | `d396c8c99916d25c2fbd47663455496761ac3add` |
| `koduck-ai/src/adapters/provider/stream_state.rs` | `StreamState::parse_frame`; `ensure_unique_members`; current private `UniqueJson` deserializer | N/A — stable methods and functions identify the decisive validation and lifecycle call sites. | Rejects duplicated terminal evidence before `serde_json::Value` and maps it to provider code `INVALID_FRAME`. | `d396c8c99916d25c2fbd47663455496761ac3add` |
| `koduck-ai/tests/cand_2_policy.rs` | `duplicate_json_schema_members_fail_closed`; `duplicate_action_parameter_members_fail_closed_at_every_depth` | N/A — stable test symbols identify Tool contract evidence. | Proves exact Tool consumer mappings for nested duplicate members. | `d396c8c99916d25c2fbd47663455496761ac3add` |
| `koduck-ai/tests/openai_provider.rs` | `invalid_clean_end_sequences_fail_closed` | N/A — stable test symbol identifies provider contract evidence. | Proves duplicated provider terminal evidence fails as `INVALID_FRAME` without completion. | `d396c8c99916d25c2fbd47663455496761ac3add` |
...
| A-7 | Contract and baseline risks covered, when applicable | Every normative contract clause maps to an explicit check or deterministic test, and every required Risk Coverage Matrix row is complete before approval and reaches Pass or specific N/A before review-ready or completion | Contract-To-Check Traceability, Risk Coverage Matrix, acceptance checks, and stable evidence | Pass | SJ-01 through SJ-05 map to AC-1 through AC-4; all five baseline risk dimensions have exact scenarios or N/A reasons and linked checks |
| A-8 | Governance validation passed | The independent validator reports no required-section, template-field, lifecycle-status, index, reciprocal-link, or Mermaid contract error for this record and repository | `npm run validate --prefix tools/governance-validator` output | Pass | Exit 0 on 2026-08-25; governance validation passed for the repository with this Proposed service ADR and central index row |
