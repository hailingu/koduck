...
No engineering-rule exception is required or waived: no production file,
function, or complexity limit is exceeded, existing large files receive only
narrow type, delegation, registration, or exhaustive-match compatibility
edits, and new codec behavior lives in one cohesive child module.

One decomposition-review threshold is crossed and the required review is
recorded here (PR-7 automatic-review P1):

| Affected unit | Measured | Review outcome and cohesion rationale | Why extraction would worsen the unit | Owner | Review date |
| --- | --- | --- | --- | --- | --- |
| `koduck-ai/tests/postgres_cand_3.rs` | 743 physical lines — above the 600-line maintained-test review threshold, below the 1,800-line exception limit. | One named acceptance check (AC-3) as one end-to-end narrative: the harness, the stalled-startup leg, the exhaustive pre-0009 seed table, the expected replay it must mirror exactly, the shared raw-row insert helper, the four verification legs, and the schema assertions. The seed rows and the expected replay justify each other row by row, and every constraint-rejection case is bred from the same fixture, so the file has one cohesive responsibility: proving migration 0009's additive, idempotent, fail-closed behavior against real PostgreSQL. | Splitting the seeding, expectations, or legs into separate files would separate the expected replay from the seed rows it must equal, duplicate fixture plumbing across files for one test's private state, and make the single-transaction recovery narrative (seed → migrate → replay → corrupt → roll back) unreadable in review order; the shared-test threshold exists as a review signal, not a mechanical split mandate. | @zcode | 2026-08-26 |
...
| Resource bounds and backpressure | Applicable — malformed or oversized stored correction payloads must not create a second unbounded replay buffer. | Payload codec and C-6 replay | Run at-limit/over-limit durable payload fixtures under the existing Item/replay resource contract and inspect allocations introduced by the diff. | Existing bounds and failure outcomes remain unchanged; the correction decoder allocates only the owned payload and replay adds no second accumulated history. | AC-1, AC-4 | Pass | Commits `c5211311e34bf` and the PR-7 review-fix commits: 1 MiB and 1 MiB+1 byte stored correction contents decode exactly once with no new replay-side byte bound; the correction payload is deserialized in a single pass by a strict document type that constructs only the owned content, so no full-document `Value` or second parse exists on the correction path; `validate_raw_replay` uses two bounded hash sets over the replayed slice and the routed diff adds no accumulated history buffer. |
...
| A-6 | Engineering exceptions governed, when applicable | Every exceeded or waived engineering rule has one complete exception row, an accountable owner, a lifecycle, and verification evidence before approval; otherwise the conditional subsection records `N/A — <reason>` | Engineering Exceptions subsection and affected-file evidence | N/A — no exception required | No rule is waived and no exception limit is exceeded; existing large files receive narrow declarations/delegation and new codec behavior is isolated. The new 743-line AC-3 test harness crosses only the 600-line decomposition-review threshold, and that review is recorded in the Engineering Exceptions subsection with its cohesion rationale, extraction trade-off, owner, and date (PR-7 automatic-review P1). |
| A-7 | Contract and baseline risks covered, when applicable | Every normative contract clause maps to an explicit check or deterministic test, and every required Risk Coverage Matrix row is complete before approval and reaches Pass or specific N/A before review-ready or completion | Contract-To-Check Traceability, Risk Coverage Matrix, acceptance checks, and stable evidence | Complete | CR-01 through CR-08 map to AC-1 through AC-4; every baseline risk dimension reached `Pass` or a reasoned `N/A` with runtime evidence recorded in the matrix at implementation completion. |
| A-8 | Governance validation passed | The independent validator reports no required-section, template-field, lifecycle-status, index, reciprocal-link, or Mermaid contract error for this record and repository | `npm run validate --prefix tools/governance-validator` output | Complete | Exit 0 on 2026-08-25 after ADD-0001 reapproval and again after acceptance, after completion-evidence recording, and after each PR-7 review-fix change; the latest run passes with this ADR `Accepted, Complete`, CAND-3 `Complete` in the `Current` ADD-0001, CAND-11 through CAND-13 `Ready`, and the central ADR index synchronized. |
...
| 2026-08-26 | Addressed the three automatic-review P2 findings of the second Codex review: duplicate correction members now fail closed through the strict duplicate-member validator before canonicalization, the standalone PostgreSQL ownership harness applies the complete production migration list so it no longer depends on binary execution order, and the A-1/A-2/A-3/A-7/A-8 completion evidence was synchronized with the final lifecycle states. Evidence-only update, not approval-invalidating. | @zcode |
| 2026-08-26 | Addressed the automatic-review P2 finding of the third Codex review: correction payload decoding became a single-pass strict document deserialization that rejects duplicate and unknown members while constructing the owned content, removing the extra full-document validation parse and allocation the two-pass strict-validator approach introduced. Evidence-only update, not approval-invalidating. | @zcode |
| 2026-08-26 | Addressed the automatic-review P1 finding of the fourth Codex review: recorded the required decomposition review for the 743-line AC-3 PostgreSQL harness in the Engineering Exceptions subsection — cohesion rationale, extraction trade-off, owner, and date — and synchronized A-6. Evidence-only update, not approval-invalidating. | @zcode |
| 2026-08-26 | Addressed the automatic-review P1 finding of the seventh Codex review by reducing `encode_payload` from a 124-line span back to 114 lines, below the 120-line engineering-exception limit: the D-3 projection payload JSON constructors moved into the `tool_projections` child module beside their decode counterparts, no exception row is required, and the no-production-limit claim is true again. Behavior-preserving refactor verified by the full routed suite. | @zcode |
