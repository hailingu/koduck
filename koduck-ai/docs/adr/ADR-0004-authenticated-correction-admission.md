...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Concurrency and ordering | Applicable — 4 writers target one tip; retries overlap; independent chains share a Turn. | C-6 PostgreSQL correction transaction | Real PostgreSQL barriers under the measured AC-3 timing precondition; compare result vectors, sequence counter, original rows, and raw replay; AC-4 separately forces deadline exhaustion. | With AC-3 timing satisfied, one winner/3 tip conflicts and identical requests return the same Item; independent chains commit distinct sequences. Deadline-exhausted calls follow CA-07 without duplicate successors. | AC-3, AC-4 | Pass | AC-3 at `849b0c2`: one winner plus 31 `PredecessorConflict` under the measured bound, identical calls converged on one Item, distinct increasing sequences per chain; AC-4's 32 deadline-exhausted writers kept at most one durable successor and deduplicated on retry. |
...
a prewarmed pool with at least 4 available connections, and no unrelated
...
each 4-call case, record a conservative serialized bound
`W + 3 * L + S < 2,000 ms`: `W` bounds the winner's lock-held transaction and
...
| A-3 | Reciprocal ADD link synchronized | CAND-11 names this ADR as its Architecture Source and, with this ADR Verified, is Complete. | ADD candidate, ADR Architecture Source and central indexes. | Complete | Final reciprocal state confirmed: this ADR is Accepted/Verified, the ADD's CAND-11 row is `Complete` with the implementation commit recorded, both central index rows agree, and the reciprocal paths were revalidated by governance validation on 2026-09-05 at the review-fix revisions. |
...
`01a08909-759c-78b3-948a-367161280447` authorizes reducing 32-way race tests to
four contenders and directly updating affected documents without another
decision record or approval ritual. Original approval metadata, statuses, and
dated 32-way results remain historical implementation evidence; they do not
assert approval or verification of this amendment. Current AC-3 uses four
connections and the bound `W + 3 * L + S < 2,000 ms`. AC-4 uses four writers;
its pool retains eight slots for writers, fault injection, and reconciliation.
Production deadlines, attempt limits, Tool-call bounds, and digest widths are
unchanged. This is semantic concurrency coverage, not a 32-client load claim.

| State / precondition | Action and entry point | Observable outcome and invariant | Owner / verification |
| --- | --- | --- | --- |
| One expired owner, unavailable store, then recovery | Four CAND-1 reconcilers | Four unavailable results before recovery; one cancellation and three terminal/fenced results after recovery; no duplicate terminal | History store; `concurrent_reconcilers_are_idempotent` |
| One requested D-6 or prepared/running D-7 | Four approval, dispatch, or terminal contenders, including HTTP and production executor entry points | One durable transition and one executor dispatch; other callers observe canonical state or typed conflict | C-5 / C-6; `postgres_cand_2_transitions_are_single_winner` and internal PostgreSQL tests |
| Fifteen durable attempts already occupy a 16-slot Turn budget | Four distinct insertions race for the last slot | Durable count equals 15 plus successful insertions and never exceeds 16; overflow remains typed limit or fail-closed unavailability | C-6; `concurrent_distinct_inserts_never_exceed_the_sixteen_attempt_budget` |
| One valid correction tip, measured timing precondition | Four fresh identities, then four identical retries | One successor and three tip conflicts; identical requests converge on the same Item with one counter increment | Correction store; `concurrency_and_retry` |
| Four correction identities blocked by held locks | Four writers exhaust deadlines, then retry after release | Unavailable with no mutation while blocked; released exact retry deduplicates | Correction settlement; `settlement_and_cancellation` |

Verification on 2026-09-10 passed for all matrix rows above. From the repository
root, with an isolated PostgreSQL 18 database supplied through
`KODUCK_AI_TEST_DATABASE_URL` and a disposable `CARGO_TARGET_DIR`:

- `cargo test -p koduck-ai --all-targets --all-features -- --test-threads=1`:
  486 tests passed across 25 test binaries, including all named matrix cases.
- `cargo fmt --all --check` and
  `cargo clippy -p koduck-ai --all-targets --all-features -- -D warnings`:
  exit 0.
- `npm test --prefix tools/governance-validator`: 184 tests passed.
- `npm run validate --prefix tools/governance-validator` and
  `git diff --check`: exit 0; structured review confirmed four-way current
  criteria, preserved historical evidence, and unchanged production bounds.
- `python3 tools/sonarqube/gate.py pre-commit`, using a temporary alternate
  Git index containing only this task's changes over `426b128`: zero incremental
  unresolved issues, Quality Gate `OK`, changed-line coverage 1,066/1,279
  (83.35%). Analysis `5ea7675e-18e2-450b-9895-a5c947179992`, temporary snapshot
  revision `2d82dcbe90fa0758fdd2d4fab1963782d92adafa`, tree
  `fab74dd6604e94f7ab1a4c5ce5eaef015ce54baf`, baseline `8e432bd`.
  This scan covers the unchanged final test source; subsequent documentation
  edits correct the AC-11 loser count and record verification evidence.
  A later commit or push still runs its normal exact-snapshot gate.

The disposable database and compilation output were removed; command logs are
retained under `/private/tmp/koduck-four-way-*.log`. Historical Pass rows above
are not current-run evidence. Tests retain normal, failure, recovery,
alternative entry-point, and durable-state assertions. Higher-load scheduling
coverage is intentionally reduced by the owner's request.
...
unchanged. Two corrections from the automatic review of the installation
revision are recorded here: ancestor payloads are now decoded for every
walked ancestor, streamed one row at a time so at most one decoded payload is
retained (the earlier decode-only-the-direct-predecessor reading did not
satisfy CA-03's malformed-anywhere requirement), and a `#[cfg(test)]`
commit-acknowledgement-loss seam was added for the AC-4 commit-arm harness —
it is never read in production builds and authorizes no production behavior.
