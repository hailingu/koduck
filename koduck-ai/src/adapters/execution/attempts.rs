...
impl ExecutionAttemptStore for SqlxExecutionAttemptStore {
    ...
    fn commit_fenced_after_dispatch(
        ...
        // Only the executor-observed states of a post-dispatch fence may
        // persist as the canonical failure; every other combination keeps the
        // current-generation transitions (ADR-0003 lines 309-314).
        if !matches!(effect_state, EffectState::Started | EffectState::Unknown) {
            return Ok(AttemptTerminalResolution::Conflict);
        }
        self.wait(async {
            if fenced_failure_winner(&self.pool, binding, effect_state, terminal_at_millis).await? {
