...
impl StreamState {
    ...
    fn parse_frame(&mut self, frame: &str) -> Result<Option<ProviderEvent>, ProviderError> {
        ...
        ensure_unique_members(data).map_err(|_| protocol_error("INVALID_FRAME"))?;
        let document: Value =
...
/// Rejects duplicate JSON object members anywhere in one provider frame.
///
/// `serde_json::Value` silently keeps the last duplicate member, so the
/// frame is first rejected when any object member is duplicated — two
/// `finish_reason` values must never collapse into one validated finish
/// (ADR-0004 PSC-3/PSC-5). Mirrors the C-5 JSON adapter's duplicate-property
/// rejection (ADR-0003).
fn ensure_unique_members(serialized: &str) -> Result<(), serde_json::Error> {
