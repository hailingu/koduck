...
pub fn parse_action_parameters(serialized: &str) -> Result<ActionParameters, ToolAdapterError> {
    ...
    unique_json::ensure_unique(serialized).map_err(|_| ToolAdapterError::InvalidJson)?;
    let value: Value =
...
pub fn parse_input_schema(serialized: &str) -> Result<InputSchema, ToolAdapterError> {
    ...
    unique_json::ensure_unique(serialized).map_err(|_| ToolAdapterError::InvalidSchema)?;
    let schema: Value =
